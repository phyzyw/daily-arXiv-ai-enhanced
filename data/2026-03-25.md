...
### [1] [Intelligence Inertia: Physical Principles and Applications](https://arxiv.org/abs/2603.22347)
*Jipeng Han*

Main category: cs.AI

TL;DR: 本文提出了“智能惯性”的概念，并从物理原理出发，解释了高级智能系统在重新配置过程中所产生的超线性计算和能量成本，揭示了静态信息理论模型无法捕捉到的“计算墙”。


<details>
  <summary>Details</summary>
Motivation: 现有信息理论框架（如Landauer原理和Fisher信息）无法解释高级智能系统在结构调整过程中所产生的巨大计算和能量成本，尤其是在维护符号可解释性的情况下。需要一种新的框架来量化智能的计算权重。

Method: 作者将智能系统分解为规则(R)和状态(S)两个组成部分，提出了规则-状态对偶的概念，并基于规则和状态的非对易性，构建了一个数学框架。通过分析实际适应成本与静态信息理论估计之间的差异，推导出一个非线性成本公式，并进行了三组实验验证：与Fisher信息模型的比较、神经网络架构演化的几何分析、以及基于惯性意识的调度器wrapper的实现。

Result: 实验结果表明，实际适应成本呈现J形曲线的“计算墙”效应，验证了“智能惯性”的存在，并证明了基于惯性的调度器wrapper能够优化深度网络的训练。

Conclusion: 本文提供了一种基于物理原理的智能成本统一描述，为理解智能体中的结构适应计算和可解释性维护开销提供了新的视角，并为人工智能领域的研究提供了新的理论基础。
...
TL;DR: 本文提出了一种基于风格的量子生成对抗网络（QGAN）架构，用于药物设计，旨在克服经典GAN训练困难，并利用量子计算提高模型泛化能力。


<details>
  <summary>Details</summary>
Motivation: 经典GAN在药物设计中面临训练困难（如 barren plateaus 和 mode collapse），且参数量大，泛化能力受限。量子计算有望通过减少参数量来改善GAN的性能。

Method: 该方法结合了变分自编码器（VAE）将分子结构编码到潜在空间，然后使用QGAN生成新的分子结构。QGAN架构在电路的每个旋转门中实现噪声编码，并在损失函数中使用梯度惩罚以减轻模式坍塌。在量子模拟器和IBM Heron量子计算机上进行了验证。

Result: 在量子模拟器上使用高达15个量子比特验证了架构，并在5个量子比特的IBM Heron量子计算机上进行了推理，以研究真实量子硬件的影响。实验结果与MOSES基准套件中的经典模型进行了比较。

Conclusion: 该研究表明，基于风格的量子GAN架构在药物设计中具有潜力，有望通过利用量子计算优势，提高药物设计效率和模型泛化能力。
