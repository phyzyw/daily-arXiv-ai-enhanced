<div id=toc></div>

# Table of Contents

- [physics.chem-ph](#physics.chem-ph) [Total: 3]
- [cs.LG](#cs.LG) [Total: 2]


<div id='physics.chem-ph'></div>

# physics.chem-ph [[Back]](#toc)

### [1] [Particle-Guided Diffusion for Gas-Phase Reaction Kinetics](https://arxiv.org/abs/2603.05139)
*Andrew Millard, Henrik Pedersen*

Main category: physics.chem-ph

TL;DR: 本文将基于扩散模型的引导采样方法应用于气体相化学反应动力学，通过训练扩散模型来预测反应-传输方程的解，并在未见过的参数值下准确预测流出浓度。


<details>
  <summary>Details</summary>
Motivation: 传统数值求解偏微分方程（PDE）在处理参数范围广或需要重复参数扫描的问题时，计算成本高且缺乏灵活性。现有基于扩散模型的生成模型主要集中在理想化的基准问题上，对真实、参数变化的物理系统的应用有限。

Method: 研究人员训练了一个基于扩散模型的引导采样方法，使其能够生成反应-传输方程的解。训练数据涵盖了不同参数值，并通过逆向扩散过程生成物理上一致的浓度场。

Result: 实验结果表明，该方法能够生成准确的PDE解，即使在训练数据中未见过的参数值下也能准确预测流出浓度，并能够模拟多个时间尺度的气体相反应动力学。

Conclusion: 这项工作证明了基于扩散模型的引导采样方法在反应传输问题中的潜力，为在复杂物理系统中进行推断提供了一种新的方法，并为稀疏观测的浓度场估计提供了支持。

Abstract: Physics-guided sampling with diffusion model priors has shown promise for solving partial differential equation (PDE) governed problems, but applications to chemically meaningful reaction-transport systems remain limited. We apply diffusion-based guided sampling to gas-phase chemical reactions by training on solutions of the advection-reaction-diffusion (ARD) equation across varying parameters. The method generates physically consistent concentration fields and accurately predicts outlet concentrations, including at unseen parameter values, demonstrating the potential of diffusion models for inference in reactive transport.

</details>


### [2] [Escaping the Hydrolysis Trap: An Agentic Workflow for Inverse Design of Durable Photocatalytic Covalent Organic Frameworks](https://arxiv.org/abs/2603.05188)
*Iman Peivaste, Nicolas D. Boscher, Ahmed Makradi, Salim Belouettar*

Main category: physics.chem-ph

TL;DR: 本文提出了一种基于大型语言模型（LLM）的智能代理Ara，用于逆向设计耐水性强的光催化共价有机框架（COFs），有效解决了COFs在水解条件下稳定性与活性难以兼顾的问题。


<details>
  <summary>Details</summary>
Motivation: 目前COFs作为光催化剂具有潜力，但常用的亚胺连接基团容易水解，限制了其应用。寻找同时具有高活性和高耐水性的COFs设计方案是一个巨大的挑战。

Method: Ara代理利用预训练的化学知识、给体-受体理论、共轭效应和连接基团稳定性等级，引导搜索满足带隙、能带边缘和水解稳定性标准的COFs。该代理结合了GFN1-xTB片段流程进行筛选，并与随机搜索和贝叶斯优化进行对比。

Result: Ara代理的命中率达到52.7%（随机搜索的11.5倍，p=0.006），在第12次迭代时找到第一个命中，显著优于随机搜索和贝叶斯优化。分析表明，Ara代理优先选择乙烯基和β-酮烯胺连接基团以提高稳定性，并根据电子吸引特性选择节点，系统地优化R基团以使带隙居于2.0 eV。

Conclusion: 研究表明，LLM化学先验知识可以显著加速多标准材料的发现过程，为逆向设计高性能、高稳定性的COFs光催化剂提供了新的思路，并提示了代理和贝叶斯优化相结合的混合策略可能更有效。

Abstract: Covalent organic frameworks (COFs) are promising photocatalysts for solar hydrogen production, yet the most electronically favorable linkages, imines, hydrolyze rapidly in water, creating a stability--activity trade-off that limits practical deployment. Navigating the combinatorial design space of nodes, linkers, linkages, and functional groups to identify candidates that are simultaneously active and durable remains a formidable challenge. Here we introduce Ara, a large-language-model (LLM) agent that leverages pretrained chemical knowledge, donor--acceptor theory, conjugation effects, and linkage stability hierarchies, to guide the search for photocatalytic COFs satisfying joint band-gap, band-edge, and hydrolytic-stability criteria. Evaluated against random search and Bayesian optimization (BO) over a space consisting of candidates with various nodes, linkers, linkages, and r-groups, screened with a GFN1-xTB fragment pipeline, Ara achieves a 52.7\% hit rate (11.5$\times$ random, p = 0.006), finds its first hit at iteration 12 versus 25 for random search, and significantly outperforms BO (p = 0.006). Inspection of the agent's reasoning traces reveals interpretable chemical logic: early convergence on vinylene and beta-ketoenamine linkages for stability, node selection informed by electron-withdrawing character, and systematic R-group optimization to center the band gap at 2.0 eV. Exhaustive evaluation of the full search space uncovers a complementary exploitation--exploration trade-off between the agent and BO, suggesting that hybrid strategies may combine the strengths of both approaches. These results demonstrate that LLM chemical priors can substantially accelerate multi-criteria materials discovery.

</details>


### [3] [Projected Hessian Learning: Fast Curvature Supervision for Accurate Machine-Learning Interatomic Potentials](https://arxiv.org/abs/2603.04523)
*Austin Rodriguez, Justin S. Smith, Sakib Matin, Nicholas Lubbers, Kipton Barros, Jose L. Mendoza-Cortes*

Main category: physics.chem-ph

TL;DR: 本文提出了一种名为Projected Hessian Learning (PHL) 的新型机器学习原子间势 (MLIP) 训练框架，它利用Hessian-vector product (HVP) 来近似Hessian矩阵，从而在保持高精度的同时，显著降低了计算和存储成本。


<details>
  <summary>Details</summary>
Motivation: 传统的机器学习原子间势训练通常仅使用能量和力进行训练，而Hessian矩阵包含更丰富的信息，可以显著提高精度和鲁棒性。然而，显式构建和存储Hessian矩阵的二次计算和存储成本限制了其应用。

Method: PHL通过避免显式Hessian矩阵的构建，而是使用随机探针方向进行曲率投影，将二阶导数监督的成本降低到接近力级别的复杂度。它采用Hutchinson trace估计器构建随机损失函数，实现可扩展的曲率信息训练。

Result: 实验结果表明，与全Hessian训练相比，基于HVP的方法在能量、力、Hessian精度上具有统计上不显著的差异，同时实现了超过24倍的加速。在固定向量模式下，Hutchinson投影优于单列探测，尤其是在远离平衡几何构型时。

Conclusion: PHL有效地将显式Hessian监督替换为力复杂度曲率训练，保留了全二阶方法的大部分精度优势，同时实现了可扩展的MLIP开发，适用于更大、更复杂的分子系统。

Abstract: The Hessian matrix (second derivatives) encodes far richer local curvature of the potential energy surface than energies and forces alone. However, training machine-learning interatomic potentials (MLIPs) with full Hessians is often impractical because explicitly forming and storing Hessian matrices scales quadratically in cost and memory.   We introduce Projected Hessian Learning (PHL), a scalable second-order training framework that injects curvature information using only Hessian-vector products (HVPs). Rather than constructing the Hessian, PHL projects curvature along stochastic probe directions and uses an unbiased stochastic trace-based loss with favorable system-size scaling, enabling curvature-informed training without quadratic memory growth.   We benchmark PHL on a chemically diverse dataset of reactants, products, transition states, intrinsic reaction coordinates, and normal-mode sampled geometries computed at omegaB97XD/6-31G(d). We compare energy-force training (E-F), two HVP-based schemes (E-F-HVP with one-hot or randomized probes), and full energy-force-Hessian training (E-F-H). With randomized probes per minibatch, both HVP schemes match full-Hessian training in energy, force, and Hessian accuracy while delivering >24x epoch speedups for the small molecular systems studied. In a fixed-probe regime with one HVP per molecule, randomized projections consistently outperform one-column probing, especially for far-from-equilibrium geometries.   Overall, PHL replaces explicit Hessian supervision with force-complexity curvature training, retaining most second-order accuracy gains while scaling to larger, more complex molecular systems.

</details>


<div id='cs.LG'></div>

# cs.LG [[Back]](#toc)

### [4] [Specialization of softmax attention heads: insights from the high-dimensional single-location model](https://arxiv.org/abs/2603.03993)
*M. Sagitova, O. Duranthon, L. Zdeborová*

Main category: cs.LG

TL;DR: 本文提出了一种理论模型，用于解释多头softmax注意力机制中头部的专业化现象，揭示了训练过程中的分阶段专业化过程，并表明softmax-1和Bayes-softmax注意力可以提高性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明，Transformer模型中的注意力头并非同时发展，而是呈现分阶段专业化的现象，并且存在大量冗余头部。本文旨在研究驱动这种分阶段头部出现和持续冗余的理论机制。

Method: 本文构建了一个高维的合成序列到token回归任务模型，其中一个token携带结构化信号，其余token为噪声。该模型使用多头softmax注意力层进行训练，并通过分析参数演化过程，将问题简化为低维的序参量系统。

Result: 研究结果表明，多头softmax注意力训练过程经历了两个阶段：一个快速的初始阶段，头部发展；以及一个随后的多阶段专业化阶段，不同的头部依次与潜在信号方向对齐。此外，softmax-1显著降低了无关头部的噪声，Bayes-softmax注意力实现了最佳预测性能。

Conclusion: 本文的研究结果揭示了多头注意力机制中头部专业化和冗余的动态过程，为理解Transformer模型训练机制提供了新的视角，并为设计更有效的注意力机制提供了理论指导。

Abstract: Multi-head attention enables transformer models to represent multiple attention patterns simultaneously. Empirically, head specialization emerges in distinct stages during training, while many heads remain redundant and learn similar representations. We propose a theoretical model capturing this phenomenon, based on the multi-index and single-location regression frameworks. In the first part, we analyze the training dynamics of multi-head softmax attention under SGD, revealing an initial unspecialized phase followed by a multi-stage specialization phase in which different heads sequentially align with latent signal directions. In the second part, we study the impact of attention activation functions on performance. We show that softmax-1 significantly reduces noise from irrelevant heads. Finally, we introduce the Bayes-softmax attention, which achieves optimal prediction performance in this setting.

</details>


### [5] [Solving adversarial examples requires solving exponential misalignment](https://arxiv.org/abs/2603.03507)
*Alessandro Salvatore, Stanislav Fort, Surya Ganguli*

Main category: cs.LG

TL;DR: 本文研究了对抗样本的本质，发现神经网络对类别的感知空间（Perceptual Manifold, PM）维度远高于人类，导致机器与人类的感知存在指数级错位，从而解释了对抗样本的产生。


<details>
  <summary>Details</summary>
Motivation: 对抗样本长期以来是机器学习中的一个难题，其产生原因至今仍不清楚。本文旨在揭示对抗样本的起源，并探讨其与模型对齐问题之间的联系。

Method: 本文定义并分析了神经网络的感知空间（PM），通过比较神经网络和人类对类别的感知空间维度，量化了机器与人类的感知错位程度。研究还验证了PM维度与稳健准确率和距离PM的距离之间的负相关关系。

Result: 研究发现，神经网络的PM维度远高于人类，存在指数级的感知错位。即使是最稳健的网络也存在这种错位，只有少数PM维度接近人类概念的维度时，才能实现对人类感知的对齐。对18个不同网络的研究证实了预测。

Conclusion: 本文将对抗样本问题与对齐问题联系起来，指出机器PM的高维度是实现对抗鲁棒性的主要障碍。研究结果表明，解决对抗样本问题需要实现机器与人类感知空间的维度对齐。

Abstract: Adversarial attacks - input perturbations imperceptible to humans that fool neural networks - remain both a persistent failure mode in machine learning, and a phenomenon with mysterious origins. To shed light, we define and analyze a network's perceptual manifold (PM) for a class concept as the space of all inputs confidently assigned to that class by the network. We find, strikingly, that the dimensionalities of neural network PMs are orders of magnitude higher than those of natural human concepts. Since volume typically grows exponentially with dimension, this suggests exponential misalignment between machines and humans, with exponentially many inputs confidently assigned to concepts by machines but not humans. Furthermore, this provides a natural geometric hypothesis for the origin of adversarial examples: because a network's PM fills such a large region of input space, any input will be very close to any class concept's PM. Our hypothesis thus suggests that adversarial robustness cannot be attained without dimensional alignment of machine and human PMs, and therefore makes strong predictions: both robust accuracy and distance to any PM should be negatively correlated with the PM dimension. We confirmed these predictions across 18 different networks of varying robust accuracy. Crucially, we find even the most robust networks are still exponentially misaligned, and only the few PMs whose dimensionality approaches that of human concepts exhibit alignment to human perception. Our results connect the fields of alignment and adversarial examples, and suggest the curse of high dimensionality of machine PMs is a major impediment to adversarial robustness.

</details>
