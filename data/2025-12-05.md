...
### [1] [NORi: An ML-Augmented Ocean Boundary Layer Parameterization](https://arxiv.org/abs/2512.04452)
*Xin Kai Lee, Ali Ramadhan, Andre Souza, Gregory LeClaire Wagner, Simone Silvestri, John Marshall, Raffaele Ferrari*

Main category: physics.ao-ph

TL;DR: NORi是一种基于物理的海洋底层湍流参数化方法，利用神经网络增强了 Richardson 数相关的扩散和粘度，通过对时间积分变量进行后验校准，实现了数值稳定性和预测能力。


<details>
  <summary>Details</summary>
Motivation: 气候模型难以模拟海洋中1-100米尺度的小尺度混合过程，这些过程对海洋与大气之间的热量、盐度等交换至关重要，影响气候预测。

Method: NORi结合了传统的物理方程和现代机器学习，使用神经网络增强了简单的物理模型。训练方法是使用高分辨率模拟作为“ground truth”，并专注于正确预测随时间推移的结果，而不是匹配瞬时变量。参数化基于 Richardson 数，并使用神经网络捕捉底层边界层的裹挟。

Result: NORi在不同对流强度、海洋背景分层、旋转强度和表面风力作用下表现出优异的预测和泛化能力。在大型模拟中，NORi 至少可以稳定运行 100 年，并且可以使用长达一小时的时间步长。

Conclusion: NORi代表了一种新的范例，通过将物理学和机器学习相结合，开发准确、计算效率高且需要更少训练数据的模型，有望减少气候模型的数据需求，并提高推理性能。
...
### [2] [Fermionic neural Gibbs states](https://arxiv.org/abs/2512.04663)
*Jannes Nys, Juan Carrasquilla*

Main category: quant-ph

TL;DR: 本文提出了一种名为费米神经网络吉布斯态（fNGS）的变分框架，用于模拟强相互作用费米子体系的有限温度性质，并成功地在掺杂费米-哈伯德模型中准确重现了热能。


<details>
  <summary>Details</summary>
Motivation: 准确模拟量子多体系统（尤其是具有费米子自由度）的热（吉布斯）态非常困难，但对于理解物质相、热力学响应、输运和非平衡弛豫至关重要。现有方法在处理强关联费米子系统时，往往受到系统尺寸、温度、掺杂量等因素的限制。

Method: fNGS从参考平均场热场双态开始，利用神经网络变换和虚时间演化来系统地构建强关联。具体来说，该方法基于热场双重形式主义，通过神经网络变换来优化变分态，从而逼近吉布斯态。

Result: fNGS在掺杂费米-哈伯德模型中表现出优异的性能，能够准确地重现各种温度、相互作用强度和掺杂水平下的热能，并且可以处理超出精确方法可达范围的系统尺寸。

Conclusion: fNGS提供了一种可扩展的途径来研究强关联费米子系统的有限温度性质，具有在超过一维的系统中应用前景，并为神经网络表示量子态在模拟费米子系统方面开辟了新的可能性。
...
### [3] [Amortized Inference of Multi-Modal Posteriors using Likelihood-Weighted Normalizing Flows](https://arxiv.org/abs/2512.04954)
*Rajneil Baruah*

Main category: cs.LG

TL;DR: 本文提出了一种利用似然加权重要性采样训练正态流的归纳后验估计新方法，无需后验训练样本，并发现基分布的拓扑结构对建模后验分布至关重要。


<details>
  <summary>Details</summary>
Motivation: 传统后验估计方法在高维逆问题中面临维度灾难，而现有的正态流训练方法通常需要后验样本，这在许多科学逆问题中难以获得。

Method: 该方法使用似然加权重要性采样训练正态流，将简单基分布（如高斯分布）映射到复杂的后验分布，从而实现无需后验样本的归纳后验推断。研究人员还探索了不同基分布的拓扑结构对后验分布的影响，并建议使用匹配目标模式数量的高斯混合模型作为基分布。

Result: 实验结果表明，标准单峰基分布无法捕捉断开的支持，导致模式之间的虚假概率桥。使用高斯混合模型初始化流可以显著提高重建精度。

Conclusion: 该研究提供了一种高效且能够捕捉后验分布正确拓扑结构的归纳推断框架，为解决高维逆问题提供了新的思路，尤其是在缺乏后验样本的情况下。
