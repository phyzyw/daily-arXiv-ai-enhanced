...
### [1] [Capturing non-Markovian dynamics in non-equilibrium stochastic systems using flow matching](https://arxiv.org/abs/2606.06658)
*Bhargav Sriram Siddani, John B. Bell, Alejandro L. Garcia, Ishan Srivastava*

Main category: cs.LG

TL;DR: 本文提出了一种基于流匹配的生成模型，用于模拟非马尔可夫动态的非平衡随机系统，该模型能够更准确地捕捉短时动态和非高斯分布，并优于传统的正则化Dean-Kawasaki (DK) 方程。


<details>
  <summary>Details</summary>
Motivation: 传统的正则化DK方程在描述小粒子数密度和短时动态时存在局限性，无法准确捕捉非马尔可夫效应和非高斯分布。此外，经典理论表明扩散系统存在记忆效应，而正则化DK方程忽略了这些效应。

Method: 研究人员开发了一种生成流匹配方法，直接对粒子模拟产生的通量概率分布进行建模，从而显式地包含非马尔可夫和非高斯效应。他们使用该方法模拟了Kramers第一通过时间问题，作为验证。

Result: 实验结果表明，该模型能够准确捕捉短时行为，并且在预测粒子数密度统计矩方面优于正则化DK方程。

Conclusion: 该研究提供了一种数据驱动的方法，可以更准确地模拟非平衡随机系统的动态，为流体动力学、化学、生物学和社会科学等领域的研究提供新的工具。
...
### [2] [ShallowBench: Benchmarking Generative Drug Design Models on Shallow-Pocket Targets](https://arxiv.org/abs/2606.06717)
*Saket Reddy, Shiwei Liu*

Main category: cs.LG

TL;DR: 本文提出了ShallowBench，一个专门用于评估生成式药物设计模型在浅口袋靶标上的性能的基准数据集。实验结果表明，现有模型在浅口袋靶标上的预测结合亲和力明显下降。


<details>
  <summary>Details</summary>
Motivation: 现有生成式药物设计模型主要依赖于深结合口袋，在处理浅口袋靶标（如KRAS和MYC等“难药化”肿瘤靶标）时表现不佳。缺乏专门针对浅口袋靶标的基准数据集阻碍了相关模型的开发。

Method: 通过计算Alpha Shape“盖子”体积与蛋白质原子体素体积的差异，从CrossDocked2020数据集中筛选出5780个浅口袋靶标。构建了包含训练集（4995个靶标）和测试集（785个靶标）的ShallowBench数据集，并利用30%序列同一性聚类防止数据泄露。

Result: 对多种先进的生成式药物设计模型在ShallowBench上的评估结果显示，这些模型在浅口袋靶标上的预测结合亲和力普遍下降，且在化学有效性等指标上也存在缺陷。

Conclusion: ShallowBench为生成式生物学模型提供了一个严格的基准，并强调了开发能够有效处理浅口袋靶标的新型架构或损失函数的重要性，从而推动药物设计领域的发展。
