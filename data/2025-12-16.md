...
TL;DR: 本研究开发了一种基于图卷积网络（GCN）和注意力机制的多组学融合模型，用于预测 Dabrafenib 对癌症细胞的敏感性，结果表明，选择性地整合蛋白组学和转录组学数据能够显著提高预测准确性。


<details>
  <summary>Details</summary>
Motivation: 由于癌症细胞对靶向治疗的反应源于复杂的分子相互作用，单一组学数据不足以准确预测药物敏感性。因此，需要整合多组学数据以理解生物复杂性并提高药物敏感性预测的准确性。

Method: 该模型将基因组学、转录组学、蛋白组学、表观基因组学和代谢组学等多组学数据与基于图卷积网络（GCN）生成的蛋白质网络嵌入相结合。利用神经网络对每种模态进行编码，并使用 STRING 数据库的蛋白质相互作用信息构建 GCN，同时采用注意力机制对不同模态赋予自适应权重。

Result: 使用 GDSC 癌症细胞系数据，研究发现选择性地整合蛋白组学和转录组学数据，能够实现最佳的测试性能（R2 约为 0.96），优于所有单一组学和全多模态设置。基因组学和表观基因组学数据信息量较少，而蛋白组学和转录组学层提供了与 MAPK 抑制剂活性相关的更强的表型信号。

Conclusion: 该研究表明，注意力引导的多组学融合与 GCN 结合可以提高药物反应预测的准确性，并揭示 Dabrafenib 敏感性的互补分子决定因素，为精准肿瘤学和靶向治疗的预测建模提供了一种有前景的计算框架。
...
### [2] [Noise-robust Contrastive Learning for Critical Transition Detection in Dynamical Systems](https://arxiv.org/abs/2512.12523)
*Wenqi Fang, Ye Li*

Main category: cs.LG

TL;DR: 本文提出了一种基于奇异值分解（SVD）和严格半正交约束的神经网络架构，用于在动态系统中鲁棒地检测临界转变，该方法在噪声环境下表现出更强的抗干扰能力。


<details>
  <summary>Details</summary>
Motivation: 现有对比学习方法在检测临界转变时，往往参数过多且对噪声敏感，导致识别不准确。此外，对噪声鲁棒性的关注不足。因此，需要一种更轻量级且更鲁棒的神经网络架构。

Method: 本文构建了一种基于奇异值分解（SVD）的神经网络架构，并结合严格半正交约束的训练算法。该方法通过矩阵分解减少了可训练参数的数量，并强制对权重矩阵施加半正交约束。

Result: 实验结果表明，所提出的方法在识别临界转变方面与传统的对比学习技术相当，但更加轻量级，并且对噪声的抵抗力明显更强。

Conclusion: 该研究提供了一种更有效、更鲁棒的临界转变检测方法，尤其适用于噪声较大的动态系统，为风险缓解和损失预防提供了新的思路。
