...
### [1] [Hybrid Quantum-Classical Encoding for Accurate Residue-Level pKa Prediction](https://arxiv.org/abs/2603.11061)
*Van Le, Tan Le*

Main category: physics.chem-ph

TL;DR: 本文提出了一种混合量子-经典编码框架，利用量子启发式特征映射增强残基级pKa预测的准确性，并在多个基准测试中表现出优越的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有残基级pKa预测方法依赖于经典描述符，难以泛化到不同的生物化学环境中。为了克服这些限制，需要一种能够捕捉非线性关系的更强大的模型。

Method: 该框架结合了高斯核量子启发式特征映射和归一化结构特征，形成统一的混合编码，并通过深度量子神经网络（DQNN）进行处理。该方法还解决了量子描述符的残基级对齐、混合模型的解释性以及跨数据集泛化等问题。

Result: 实验结果表明，DQNN在多个精心策划的描述符集上实现了优于经典基线的交叉上下文泛化能力，并在PKAD-R实验基准和Aβ40案例研究中表现出鲁棒性和可传递性。

Conclusion: 该研究建立了一种可扩展且实验上可传递的残基级pKa预测方法，通过整合量子启发式特征变换和经典生物化学描述符，为蛋白质静电学等更广泛的应用提供了新的途径。
...
### [2] [Proof-Carrying Materials: Falsifiable Safety Certificates for Machine-Learned Interatomic Potentials](https://arxiv.org/abs/2603.12183)
*Abhinaba Basu, Pavan Chakraborty*

Main category: cond-mat.mtrl-sci

TL;DR: 本文提出了Proof-Carrying Materials (PCM)框架，通过对抗性测试、置信区间细化和Lean 4形式化验证，对机器学习原子势（MLIP）进行可靠性评估，显著提高了材料筛选的准确性和安全性。


<details>
  <summary>Details</summary>
Motivation: 现有的MLIP在材料筛选中缺乏可靠性保证，单一MLIP模型会遗漏大量DFT稳定的材料，且存在架构特定的盲点，导致高通量筛选流程可能错过关键材料。

Method: PCM框架包含三个阶段：(1) 对抗性虚化，使用多种策略探索MLIP的失效区域；(2) 包络细化，通过引导抽样计算95%置信区间；(3) Lean 4形式化验证，将细化的包络编译成带有物理公理的证明。

Result: 实验结果表明：(1) 不同MLIP架构存在近零的力相关性，表明盲点是架构特定的；(2) 通过PCM发现的成分特征能够以高准确率（AUC-ROC=0.938±0.004）预测未见材料的失效；(3) 风险模型可以跨架构迁移，预测MACE失效（AUC-ROC =0.697）。在热电材料筛选案例中，PCM审计的协议发现了单一MLIP筛选遗漏的62种稳定材料，提高了25%的发现率。

Conclusion: PCM框架提供了一种新的MLIP验证范式，将回顾性审计转变为预测性干预，强调了多MLIP审计的重要性，并为机器学习原子势在材料发现中的安全可靠应用提供了保障。
