...
TL;DR: 本文提出了一种基于深度算子网络（DeepONets）的替代模型，用于模拟近岸波浪的辐射应力梯度和显著波高，以降低传统数值波浪模型（如SWAN）的计算成本。


<details>
  <summary>Details</summary>
Motivation: 传统数值波浪模型计算成本高昂，限制了其在波浪-海流耦合模拟中的应用，尤其是在需要高时间分辨率的场景下。为了提高风暴潮预测的准确性，需要更高效的波浪模型。

Method: 利用深度算子网络（DeepONets）作为替代模型，替代了Simulating WAves Nearshore (SWAN)数值波浪模型。在三个不同的1-D和2-D稳态数值例子中进行了测试，并应用于北卡罗来纳州Duck的实际稳态波浪模拟。

Result: 在Duck, NC的实际稳态波浪模拟中，DeepONet模型在预测辐射应力梯度和显著波高方面取得了持续的高精度。

Conclusion: DeepONets作为波浪模型替代模型具有潜力，可以降低计算成本，并有望提高波浪-海流耦合模拟的效率，从而改善风暴潮预测的准确性。
...
### [2] [Calibration of a neural network ocean closure for improved mean state and variability](https://arxiv.org/abs/2604.06398)
*Pavel Perezhogin, Alistair Adcroft, Laure Zanna*

Main category: physics.ao-ph

TL;DR: 本文提出了一种使用集合卡尔曼逆（EKI）方法校准神经网络参数化海洋模型的方法，显著减少了粗分辨率海洋模型在均态和变异性方面的误差。


<details>
  <summary>Details</summary>
Motivation: 全球海洋模型在粗分辨率下存在均态和变异性偏差，传统参数化系数调整依赖于人工经验，本文旨在通过数据驱动的方法系统地校准参数化，减少这些偏差。

Method: 研究人员将参数调整问题形式化为校准问题，利用集合卡尔曼逆（EKI）优化了两个理想化海洋模型中神经网络参数化的参数。他们还提出了一种高效的校准协议，避免了模型达到统计平衡。

Result: 校准后的参数化将时间平均流体界面及其变异性的误差减少了约一半，与未参数化模型或离线训练的参数化模型相比。EKI方法对海洋动力学混沌产生的统计噪声具有鲁棒性。

Conclusion: 本文展示了系统校准可以显著改善粗分辨率海洋模拟，并为减少全球海洋模型中的偏差提供了一条实用途径，为气候预测模型改进提供了可能。
...
### [3] [SMT-AD: a scalable quantum-inspired anomaly detection approach](https://arxiv.org/abs/2604.06265)
*Apimuk Sornsaeng, Si Min Chan, Wenxuan Zhang, Swee Liang Wong, Joshua Lim, Dario Poletti*

Main category: cs.LG

TL;DR: 本文提出了一种名为SMT-AD的量子启发式异常检测方法，该方法基于叠加的多分辨率张量，具有高并行性和线性可扩展性，并在标准数据集上表现出与现有基线相当的性能。


<details>
  <summary>Details</summary>
Motivation: 现有异常检测方法在处理稀缺、异构且训练时不可用的异常样本时面临挑战。传统的张量网络方法通常依赖于顺序优化，限制了可扩展性和并行化。

Method: SMT-AD结合了基于秩的预处理、傅里叶辅助的多分辨率嵌入和叠加的键尺寸为一的矩阵乘积算子(MPO)模型。该模型仅使用正常数据进行训练，并通过输出状态与固定参考产品状态的重叠来评估样本的正常性。

Result: 实验结果表明，即使在最小配置下，SMT-AD也能在信用卡交易等标准数据集上实现与现有异常检测基线相当的性能，并且可以通过突出最重要的输入特征来降低模型权重并提高性能。

Conclusion: SMT-AD提供了一种高效、可扩展且可解释的异常检测方法，它利用了量子启发式张量网络的优势，为异常检测领域提供了一种有前景的解决方案。
...
### [4] [Asymptotic-Preserving Neural Networks for Viscoelastic Parameter Identification in Multiscale Blood Flow Modeling](https://arxiv.org/abs/2604.06287)
*Giulia Bertaglia, Raffaella Fiamma Cabini*

Main category: cs.LG

TL;DR: 本文提出了一种基于渐近保持神经网络（APNN）的多尺度血管血流模型参数识别方法，该方法能够从超声多普勒的横截面积和速度测量中估计血管参数，无需直接测量压力。


<details>
  <summary>Details</summary>
Motivation: 传统数学模型和数值模拟在探索心血管现象方面具有非侵入性优势，但准确确定控制血管变形的粘弹性参数仍然是一个挑战，尤其是在无法直接测量压力的血管段。

Method: 研究人员利用渐近保持神经网络（APNN）将多尺度粘弹性血流模型的物理原理嵌入到学习过程中，从而推断粘弹性参数并同时重建血管状态变量的时间演化。该框架利用超声多普勒的横截面积和速度测量来估计压力波形。

Result: 在合成和患者特定场景下进行的数值模拟表明，所提出的方法有效，能够从间接数据中准确估计粘弹性参数。

Conclusion: 该研究提供了一种非侵入性的方法来识别血管粘弹性参数，并能够从易于获得的患者数据中重建压力波形，为心血管生理学研究和诊断工具开发提供了新的途径。
...
### [5] [VertAX: a differentiable vertex model for learning epithelial tissue mechanics](https://arxiv.org/abs/2604.06896)
...
TL;DR: 本文提出了VertAX，一个基于JAX的可微分顶点模型框架，用于学习上皮组织力学，支持前向模拟、参数推断和逆向设计。


<details>
  <summary>Details</summary>
Motivation: 现有的顶点模型参数众多且难以优化，缺乏灵活的计算框架来建模和学习组织力学。为了解决这个问题，需要一个易于集成机器学习流程的框架。

Method: VertAX框架使用JAX实现，提供自动微分、GPU加速和端到端双层优化。用户可以使用纯Python定义任意能量和成本函数，并支持自动微分、隐式微分和平衡传播三种微分策略。

Result: 在组织形态发生前向建模、机械参数推断和组织行为逆向设计三个任务中验证了VertAX，平衡传播策略仅通过重复的前向和伴随自由模拟即可近似梯度。

Conclusion: VertAX提供了一个公共、通用的顶点模型实现，促进了组织力学研究的合作和进步。平衡传播策略为将逆生物物理问题扩展到不可微分模拟器提供了一种简单途径。
