...
### [1] [Meshless solutions of PDE inverse problems on irregular geometries](https://arxiv.org/abs/2510.25752)
*James V. Roggeveen, Michael P. Brenner*

Main category: math.NA

TL;DR: 本文提出了一种基于谱基函数在任意时空域上参数化解的网格自由方法，用于求解非线性偏微分方程（PDE）上的逆问题和优化问题，该方法利用优化问题来寻找谱基函数的系数，并能有效融合数据同化。


<details>
  <summary>Details</summary>
Motivation: 求解复杂几何域上非线性偏微分方程的逆问题和优化问题是一项长期挑战，传统方法如伴随法和可微 PDE 求解器需要多次迭代，且在复杂域中推导伴随方程或重新实现传统方法具有挑战性。PINNs虽然灵活，但存在谱偏差和缺乏收敛保证。

Method: 该方法使用谱基函数在包含真实域的超矩形上参数化解，通过求解优化问题来寻找谱基函数的系数，优化目标是最小化 PDE 残差、边界条件和优化目标的损失函数。该方法借鉴了 Physics-Informed Neural Networks (PINNs) 的关键思想，并自然地允许数据同化。

Result: 实验结果表明，针对各种方程，优化协议能够找到具有指数收敛的解，并且该方法能够高效地解决 PDE 上的优化问题。

Conclusion: 该方法提供了一种网格自由、具有指数收敛性和数据同化能力的求解 PDE 逆问题和优化问题的有效途径，避免了传统方法的一些局限性，并有望在科学、数学和工程领域得到广泛应用。
...
TL;DR: 本文提出了EnzyControl方法，通过集成EnzyAdapter模块到预训练的motif-scaffolding模型中，实现了对酶骨架生成的官能和底物特异性控制，显著提升了酶的设计性能。


<details>
  <summary>Details</summary>
Motivation: 现有的蛋白质生成模型在酶设计方面存在局限性，未能充分考虑底物特异性、功能位点保护和催化构象等关键因素。因此，需要一种能够实现官能和底物特异性控制的酶骨架生成方法。

Method: 研究人员构建了包含11,100个实验验证的酶-底物对的EnzyBind数据集。基于此，提出了EnzyControl方法，该方法利用MSA注释的催化位点和对应的底物，通过EnzyAdapter模块将底物信息融入到预训练的motif-scaffolding模型中，并采用两阶段训练范式进一步优化模型性能。

Result: 实验结果表明，EnzyControl在EnzyBind和EnzyBench基准测试中表现优异，在设计性和催化效率方面分别提高了13%，显著优于基线模型。

Conclusion: EnzyControl为酶骨架生成提供了一种新的有效方法，能够更好地实现官能和底物特异性控制，为蛋白质工程和酶设计领域的发展提供了新的思路和工具。
...
### [3] [LieSolver: A PDE-constrained solver for IBVPs using Lie symmetries](https://arxiv.org/abs/2510.25731)
*René P. Klausen, Ivan Timofeev, Johannes Frank, Jonas Naujoks, Thomas Wiegand, Sebastian Lapuschkin, Wojciech Samek*

Main category: cs.LG

TL;DR: LieSolver是一种利用李群对称性来高效求解初始边界值问题(IBVP)的新方法，它通过构造性地强制执行偏微分方程(PDE)来提高计算效率和预测可靠性。


<details>
  <summary>Details</summary>
Motivation: 传统的数值方法在处理高维PDE时计算成本高昂，而现有的基于物理信息的神经网络(PINNs)虽然有潜力，但常常存在训练不稳定和精度不可靠的问题。

Method: LieSolver利用李群对称性，将物理定律融入模型中，通过对称变换学习初始和边界数据，从而直接衡量模型的准确性。该方法构建紧凑模型，便于优化，并能进行严格的误差估计。

Result: LieSolver在解决线性齐次PDE问题时，比PINNs更快、更准确，并且能够进行误差估计。

Conclusion: LieSolver提高了PDE约束问题的计算效率和预测可靠性，为科学机器学习领域提供了一种新的、更可靠的解决方案。
...
### [4] [Graph Network-based Structural Simulator: Graph Neural Networks for Structural Dynamics](https://arxiv.org/abs/2510.25683)
*Alessandro Lucchetti, Francesco Cadini, Marco Giglio, Luca Lomazzi*

Main category: cs.LG

TL;DR: 本文提出了一种基于图神经网络的结构模拟器 (GNSS)，用于动态结构问题的替代模型。GNSS在动态模拟中表现出卓越的性能，能够准确预测结构动力学行为并实现显著的推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有数值模拟在处理长时间、高频或重复求解参数集时成本高昂。因此，需要开发更快速、准确的替代模型，即代理模型，以加速推断。

Method: GNSS 采用典型的 GNN 编码-处理-解码范式，并具有三个关键特性：(i) 使用节点固定局部坐标系表达节点运动学，避免有限差分速度中的灾难性抵消；(ii) 采用感知符号的回归损失，减少长时间演化中的相位误差；(iii) 使用波长感知的连接半径优化图的构建。

Result: 在受 50kHz Hanning 调制脉冲激励的梁的案例研究中，GNSS 准确地重现了数百个时间步长的物理现象，并能泛化到未见过的加载条件，而现有的 GNN 模型无法收敛或提供有意义的预测。与显式有限元基线相比，GNSS 实现了显著的推理加速，同时保持了空间和时间保真度。

Conclusion: GNSS 证明了局部性保留的 GNN 具有物理一致的更新规则，是动态、波动主导的结构模拟的一种有竞争力的替代方案。
...
### [5] [Scaling flow-based approaches for topology sampling in $\mathrm{SU}(3)$ gauge theory](https://arxiv.org/abs/2510.25704)
*Claudio Bonanno, Andrea Bulgarelli, Elia Cellini, Alessandro Nada, Dario Panfalone, Davide Vadacchino, Lorenzo Verzichelli*

Main category: hep-lat

TL;DR: 本文提出了一种基于非平衡态模拟的策略，用于缓解格点量子色动力学（QCD）中拓扑冻结问题，并利用随机归一化流加速拓扑采样。


<details>
  <summary>Details</summary>
Motivation: 在接近连续极限时，标准蒙特卡洛模拟在格点QCD中会遇到严重的拓扑冻结问题，导致计算成本急剧增加，影响了算法的有效性。

Method: 该研究采用非平衡态蒙特卡洛方法，通过开放边界条件降低拓扑荷的自相关性，并逐步切换到周期性边界条件以消除开放边界条件的不物理效应。此外，还设计了定制的随机归一化流，用于边界条件演化，以进一步提高效率。

Result: 研究人员在四维SU(3)杨-米尔斯理论中详细分析了该策略的计算成本，并在格点间距小至0.045 fm时验证了其有效性。随机归一化流方法表现出优于纯粹随机非平衡态方法的性能。

Conclusion: 该研究为在连续极限中高效采样拓扑提供了一种明确的策略，并为未来基于流的解决方案开辟了道路，有望加速格点QCD计算。
