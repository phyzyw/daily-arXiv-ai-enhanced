...
TL;DR: 本文提出了一种名为“My Chemical Harness”的框架，利用大型语言模型（LLM）作为策略控制器，在可执行的合成路径上进行进化式分子设计，从而克服了传统方法中合成不可行的问题。


<details>
  <summary>Details</summary>
Motivation: 传统分子设计方法通常生成合成不可行的分子，缺乏可行的合成路线。因此，需要一种能够同时优化分子性质和合成可行性的方法。

Method: 该框架使用可购买的构建块和反应模板构建可执行的合成路径，并由确定性化学工具执行。LLM仅作为策略控制器，选择关于路径长度、移动类型、反应家族等的高层偏好，而本地代码负责路径构建、验证、评分、选择和记忆更新，避免了LLM产生幻觉或不支持的反应步骤。

Result: 在可溶性环氧水解酶（sEH）代理任务中，LLM代理的表现优于单次LLM和确定性控制器，在sEH分数、合成可及性分数和AiZynthFinder成功率指标上达到了最先进的性能。

Conclusion: 受限的LLM代理可以在没有训练、微调或专用生成模型的情况下，在分子发现中发挥重要作用，为药物和材料设计提供了一种新的途径。
...
### [2] [Neural-Parameterized Cellular Automata for Wildfire Spread](https://arxiv.org/abs/2606.11676)
*Maksym Zhenirovskyy, Ion Matei, Rohit Vuppala, Takuya Kurihana, Hon Yung Wonga*

Main category: cs.CE

TL;DR: 本文提出了一种基于JAX的神经参数化概率元胞自动机(CA)框架，用于模拟野火蔓延，通过深度学习动态生成空间变化的参数，提高了预测精度。


<details>
  <summary>Details</summary>
Motivation: 传统野火模型依赖于刚性参数和静态燃料图，难以准确预测火灾蔓延。为了解决这一问题，需要开发高保真度、适用于实时使用的模型，优化疏散方案和消防资源部署。

Method: 该方法采用多尺度卷积神经网络动态生成空间变化的参数，这些参数控制火灾蔓延概率、风向和坡度影响。该框架将深度学习与三态CA相结合，利用JAX实现硬件加速和基于梯度的参数校准。

Result: 在六个美国西部大规模野火的评估中，模型在72小时的预测范围内保持了>0.6的IoU，在10天的资料同化窗口内进行增量拟合后，实现了对火灾增长的条件投影。

Conclusion: 该神经参数化CA框架结合了物理可解释性和机器学习的灵活性，为野火预测提供了更准确、更高效的解决方案，并为实时火灾管理提供了支持。
...
### [3] [HAMNO: A Hierarchical Adaptive Multi-scale Neural Operator with Physics-Informed Learning for Dynamical Systems](https://arxiv.org/abs/2606.11963)
*Mostafa Bamdad, Mohammad Sadegh Eshaghi, Timon Rabczuk*

Main category: cs.LG

TL;DR: 本文提出了一种名为HAMNO的层次自适应多尺度神经网络算子，结合了局部卷积、全局谱算子和层次编码器-解码器，用于学习非线性时变系统，并提出了基于多目标损失的物理信息学习扩展PI-HAMNO。


<details>
  <summary>Details</summary>
Motivation: 现有神经网络算子在处理涉及多尺度结构、长程相互作用和稳定长时演化的非线性时变系统时仍存在困难，尤其是在长时预测和数据有限的情况下。

Method: HAMNO通过数据依赖的门控机制自适应地平衡局部和全局信息，同时PI-HAMNO通过结合数据拟合和强/弱形式物理约束的多目标损失策略，实现了物理信息学习。

Result: HAMNO在Allen-Cahn、Cahn-Hilliard和Swift-Hohenberg方程上表现出优于标准神经网络算子的预测精度，PI-HAMNO进一步增强了稳定性、物理一致性和数据效率。

Conclusion: HAMNO为解决计算力学、材料科学和模式形成系统中的非线性偏微分方程提供了更有效、更稳定的方法，尤其适用于需要长时预测和数据有限的场景。
...
### [4] [Least-Action-Guided Diffusion for Physical Extrapolation](https://arxiv.org/abs/2606.11277)
*Zhongxin Yang, Yuanwei Bin, Xiang I. A. Yang, Shiyi Chen*

Main category: cs.LG

TL;DR: 本文提出了一种名为LAPG的基于最小作用量原理引导的扩散模型，用于物理外推，它在推理时通过动作导向的物理指导分数来提高物理一致性，从而解决传统扩散模型在超出训练分布时的物理不一致问题。


<details>
  <summary>Details</summary>
Motivation: 传统生成模型在物理外推时，由于依赖于训练期间施加的约束，容易产生物理不一致的预测。为了解决这个问题，需要一种在推理时能够促进物理一致性的方法。

Method: LAPG框架结合了条件得分引导的扩散模型和基于动作的物理指导分数。首先，学习到的得分模型生成一个在分布内的建议；其次，基于动作的变分先验将该建议细化到目标分布外的条件。

Result: 在自由落体、弹簧-质量动力学、相互作用的点涡流和参数化翼型气流等系统上进行了评估，LAPG在时间、参数和几何外推测试中减少了相位漂移，保留了耗散衰减，捕获了涡流运动，并改善了翼型气流的升力响应。

Conclusion: LAPG提供了一种替代点残差惩罚的机制，将最小作用量原理转化为可微分的推理时间校正机制，为物理外推提供了一种新的有效方法。
...
### [5] [Structure-Preserving Neural Surrogates with Tractable Uncertainty Quantification](https://arxiv.org/abs/2606.11650)
*Handi Zhang, Adrienne M. Propp, Brooks Kinch, Houman Owhadi, Nathaniel Trask*

Main category: cs.LG

TL;DR: 本文提出了一种结构保持的神经网络代理模型，利用高斯过程(GP)对状态-通量关系中的不确定性进行量化，并提供闭式形式的后验不确定性估计，从而支持科学计算中的验证和校准。


<details>
  <summary>Details</summary>
Motivation: 现有科学机器学习方法虽然能实现快速求解偏微分方程(PDEs)，但缺乏传统模拟器所具备的理论基础，难以进行验证和校准。此外，现有方法通常是黑盒模型，难以进行不确定性量化。

Method: 该方法构建了基于数据驱动的降阶模型，利用有限元外微分形式(exterior calculus)暴露拓扑结构，并利用高斯过程(GP)表示状态-通量关系中的不确定性。具体地，提出了结构保持的H(div)–L2子空间，并通过轻量级Transformer进行约束。将学习问题表述为最优恢复问题(ORP)，利用Schur补训练策略加速GP回归训练。

Result: 训练后的模型可以在实时内求解，并提供边界通量的闭式形式估计器。研究还推导了线性泛函的RKHS后验误差界限，并通过数值实验验证了后验分布作为误差估计的有效性。

Conclusion: 该研究提供了一种新的科学机器学习框架，结合了有限元方法和高斯过程回归，实现了结构保持的降阶模型，并支持可跟踪的不确定性量化，为科学计算和工程应用提供了新的解决方案。
...
### [6] [Loss Landscape Diagnosis for Gradient-Based Gray-Scott System Inversion: Disentangling the Roles of PINN Components](https://arxiv.org/abs/2606.11258)
*Yan Yang*

Main category: cs.LG

TL;DR: 本文通过直接反向传播Gray-Scott系统的损失函数来诊断梯度优化问题，揭示了损失景观的几何结构，并分析了PINN各组件的作用。


<details>
  <summary>Details</summary>
Motivation: 现有物理系统反演方法多采用代理模型或PINN，本文旨在探索直接通过PDE结构反向传播的可能性，以诊断梯度优化方法在反应-扩散系统中的挑战，并理解现有方法的局限性。

Method: 研究人员选择Gray-Scott系统作为测试平台，直接反向传播损失函数，无需代理模型或神经网络增强，分析了优化过程中的损失景观，并将其视为PINN的消融实验。

Result: 实验结果显示损失景观存在平坦高原和陡峭悬崖，导致梯度优化无法收敛。残差损失本身就具有良好的景观，而神经网络无法改善参数空间，仅能补充缺失的观测数据。

Conclusion: 研究结果揭示了PINN各组件的作用分工，为PINN类方法的具体设计提供了指导，并提出了关于增加维度是否真正有帮助的更广泛的启发式规则。
...
### [7] [Range-Aware Bayesian Optimization for Discovering Diverse Designs within Target Property Windows](https://arxiv.org/abs/2606.11574)
*Shengli Jiang, Jason Wu, Charles M. Schroeder, Michael A. Webb*

Main category: cs.LG

TL;DR: 本文提出了一种范围感知的贝叶斯优化 (Range-Aware Bayesian Optimization, RA-BO) 框架，用于在目标属性范围内发现多样化的设计方案，特别适用于材料和产品设计中需要满足特定范围要求的场景。


<details>
  <summary>Details</summary>
Motivation: 传统贝叶斯优化方法主要关注优化单个属性或寻找帕累托前沿，难以有效发现满足特定范围要求的多个、不同的设计方案。在实际应用中，多个满足范围要求的候选方案可能因成本、可加工性或鲁棒性等因素而各有优劣，因此需要寻找多样化的解决方案。

Method: RA-BO框架通过直接评估候选方案满足目标范围的后验概率来构建采集函数。该框架可以并行地探索多个不同的规格，并共享候选空间。与标准BO和目标搜索方法相比，RA-BO更注重发现位于目标范围内的设计。

Result: 在基准任务和两个实际案例研究（聚合物合成反应条件优化和具有特定吸收带的序列定义寡聚物发现）中，RA-BO能够比标准BO基线和目标搜索方法恢复更大、更多样化的有效设计方案。

Conclusion: RA-BO提供了一个实用且样本效率高的、基于规格的设计基础，尤其适用于设计灵活性和解决方案多样性是重要考虑因素的场景，为材料和产品设计提供了一种新的思路。
...
### [8] [GLACIER: A Multimodal Student-Teacher Foundation Model for Molecular Property Prediction](https://arxiv.org/abs/2606.11382)
*Emily Nguyen, Yongchan Hong, Harsh Toshniwal, Yan Liu, Andreas Luttens*

Main category: cs.LG

TL;DR: 本文提出了GLACIER模型，一个多模态学生-教师框架，通过整合分子图、SMILES字符串和物理化学描述符来学习丰富的分子嵌入，从而提高分子性质预测的效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的大规模分子性质预测模型计算负担高，且大多为单模态，未能充分利用不同分子数据的互补信息。因此，需要一种更高效、更全面的模型。

Method: GLACIER模型包含三个阶段：(1) 预训练三个学生编码器（分子图、SMILES字符串、物理化学描述符）；(2) 使用一种新的Finsler几何感知模块融合这些学生模态；(3) 通过对比学习将大型教师模型（MiniMol和MolFormer）的互补知识蒸馏到单个轻量级模型中。

Result: 实验表明，GLACIER模型在复杂的分子性质预测任务中表现出高预测性能和计算效率，并且在模型参数量和推理时间方面优于现有模型。

Conclusion: GLACIER模型提供了一个强大的框架，可以有效地进行分子性质预测，加速药物发现过程，其代码已公开。
...
### [9] [Learning from almost nothing: How neural networks survive heavy input corruption](https://arxiv.org/abs/2606.11319)
*Justin Tahmassebpur, Asadullah Bhuiyan, Hyejin Kim, Omri Lesser*

Main category: cs.LG

TL;DR: 本文研究了神经网络在输入数据严重污染情况下的鲁棒性，发现即使输入数据被污染超过90%，神经网络仍能保持较高的准确率，其原因在于网络实际上执行了一种基于类平均值的原型规则。


<details>
  <summary>Details</summary>
Motivation: 机器学习中，处理不完美数据是一个核心问题。虽然标签噪声研究较多，但输入属性噪声（特征污染）却相对较少被关注，而实际应用中特征污染很常见。

Method: 研究人员通过多层感知机（MLP）在被污染的分类数据集上进行实验，并使用基于均场的方法分析了无限宽度网络在严重污染情况下的行为，推导出了分类结果的领先规则。

Result: 实验结果表明，神经网络在输入数据被污染超过90%时仍能保持高于随机猜测的准确率。理论分析揭示，网络实际上执行了一种原型规则，即“最近类平均值”，将每个测试点分配到其最接近的训练集平均值所属的类别。

Conclusion: 该研究揭示了神经网络在极端数据污染情况下仍能有效学习的机制，并提供了一种可解释且分析上易于处理的解释，表明即使训练样本携带的信息量很小，学习仍然可以成功。
...
### [10] [Perturbative Contrastive Physical Learning](https://arxiv.org/abs/2606.09756)
*Kyungeun Kim, Amanuel Anteneh, Israel Klich, Olivier Pfister, J. M. Schwarz*

Main category: cs.LG

TL;DR: 本文提出了一种通用的物理学习框架Perturbative Contrastive Physical Learning (PCPL)，该框架通过测量物理状态之间的对比来驱动学习，无需中心梯度计算。


<details>
  <summary>Details</summary>
Motivation: 学习通常与大脑和神经处理相关联，但生物和非生物系统也表现出学习能力。本文旨在探索直接从物理过程涌现的学习，将硬件作为软件。

Method: PCPL框架通过对输入、边界条件、内部参数或读取映射进行受控扰动，测量物理状态之间的对比，并利用这些对比来更新可修改的参数。该框架统一并扩展了现有方法，如平衡传播和频率传播，并在弹簧网络和连续变量光子电路中进行了演示。

Result: 在弹簧网络和连续变量光子电路中成功实现了分类任务，光子电路还被训练用于模拟乘法运算，表明了物理学习系统自主性的潜力。

Conclusion: PCPL提供了一种新的物理学习范式，它依赖于对比而不是梯度，并允许学习几何从系统的物理响应中隐式地涌现，为构建更自主的物理学习系统开辟了道路。
...
### [11] [Learning Dynamics Reveal a Hierarchy of Weight-Induced Layerwise Gram Metrics](https://arxiv.org/abs/2606.09744)
*Claudio Nordio*

Main category: cs.LG

TL;DR: 本文研究了前馈ReLU网络的梯度下降过程，提出了将学习动态描述为激活场和辅助场的封闭表示，并揭示了深度网络中权重诱导的层间Gram度量的新层次结构。


<details>
  <summary>Details</summary>
Motivation: 传统的梯度下降方法关注权重空间动态，本文旨在探索一种替代方法，将梯度下降重新表述为基于激活场和有限层次的辅助场的动态过程，从而更直接地描述学习过程。

Method: 研究对象为固定读出和二次损失的前馈ReLU网络。通过类似于NTK的方法，消除了单隐层网络的权重变量，得到关于激活的封闭动态描述。对于更深的网络，推导了残差动态，并发现了层间Gram算子的层次结构，这些算子介导了层间信息传递。

Result: 对于单隐层网络，动态封闭在激活场上。对于双隐层网络，需要引入共轭场。从三隐层开始，出现权重诱导的Gram算子层次结构，这些算子满足向后拉回递归，并且残差核可以用二阶激活重叠和激活与共轭场的四阶相关器来表示。

Conclusion: 研究表明，深度网络中的学习动态可以通过激活场和辅助场的封闭表示来描述，并揭示了权重诱导的层间Gram度量的新层次结构，为理解深度网络学习过程提供了新的视角，并暗示了更深层次的递归规则控制着更深场变化的向后传递。
...
### [12] [Phase Transitions in Attention: A Bayesian Theory of Copy Head Emergence](https://arxiv.org/abs/2606.12058)
*Itay Lavie, Kirsten Fischer, Andrey Lekov, Frederic Van Maele, Zohar Ringel, Moritz Helias*

Main category: stat.ML

TL;DR: 本文提出了一个基于贝叶斯理论的注意力机制特征学习模型，并分析了在复制任务中，单层注意力网络中“复制子电路”的突然出现，揭示了训练数据量与注意力模式转变之间的相变现象。


<details>
  <summary>Details</summary>
Motivation: 为了解释大型语言模型中上下文学习能力（如复制任务）的突然涌现，以及能力发展是逐渐的还是突变的争论，本文旨在提供一个从第一原理出发的理论解释。

Method: 研究人员构建了一个单层softmax注意力网络，并将其应用于复制任务。他们推导了注意力矩阵的闭式后验分布，将其简化为低维参数空间，并通过贝叶斯采样和标准训练（Adam）验证了相变的存在。

Result: 研究发现，softmax注意力机制在训练数据量达到一定阈值时，会经历一个一阶相变，导致“复制子电路”的突然出现。与此不同，线性注意力机制则经历了一个二阶相变，随后是向结构化注意力模式的平滑过渡。

Conclusion: 本文提供了一个理论框架，解释了大型语言模型训练中观察到的“复制子电路”的突然涌现现象，并对比了softmax和线性注意力机制在相变行为上的差异，为理解上下文学习能力的发展提供了新的视角。
...
### [13] [Magnetic HIP-NN for spin dynamics in disordered itinerant magnets](https://arxiv.org/abs/2606.10349)
*Supriyo Ghosh, Yunhao Fan, Sheng Zhang, Kipton Barros, Gia-Wei Chern*

Main category: cond-mat.dis-nn

TL;DR: 本文提出了一种磁性分层相互作用粒子神经网络 (mHIP-NN)，用于模拟无序易行磁体中的电子介导自旋动力学，并成功捕捉了非平衡态自旋相关性。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习方法主要关注静态磁性性质，缺乏对动态自旋行为的大规模模拟能力。本文旨在开发一种可扩展的框架，用于模拟易行磁体中的非平衡态自旋动力学。

Method: 研究人员扩展了分层相互作用粒子神经网络 (HIP-NN)，引入了旋转不变的自旋相关性，构建了 mHIP-NN。该网络通过分层消息传递学习磁性能量景观和有效局部磁场，并应用于结构无序易行d交换模型。

Result: mHIP-NN 准确地再现了控制兰道-利夫希茨-吉尔伯特动力学的局部扭矩，并忠实地捕捉了热猝火后空间自旋相关性的非平衡演化。

Conclusion: 研究结果表明，具有感知对称性的分层消息传递网络是模拟受挫易行自旋系统和非平衡态磁动力学的有效且可扩展的框架。该框架还为自旋相关的原子间势能和耦合原子-自旋动力学提供了基础。
