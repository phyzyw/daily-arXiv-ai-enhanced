...
### [1] [Physics-Informed Machine Learning for Two-Phase Moving-Interface and Stefan Problems](https://arxiv.org/abs/2512.14010)
*Che-Chia Chang, Te-Sheng Lin, Ming-Chih Lai*

Main category: physics.comp-ph

TL;DR: 本文提出了一种基于物理信息的神经网络框架，用于求解两相Stefan问题，该框架能够精确追踪界面运动并处理界面处的温度梯度不连续性。


<details>
  <summary>Details</summary>
Motivation: Stefan问题是经典的热传导与相变模型，但其移动界面和非线性耦合带来了计算挑战。现有神经网络方法在处理界面不连续性方面存在困难，因此需要更有效的方法。

Method: 该方法采用两个神经网络：一个用于表示移动界面，另一个用于表示温度场。界面网络用于快速分类空间域内的热扩散率，为温度网络选择训练点提供依据。温度网络的输入增强了修改后的零水平集函数，以准确捕捉界面处的法向导数跳变。

Result: 数值实验表明，该方法在求解两相动态Stefan问题时，比其他神经网络方法具有更高的精度和有效性，并且能够捕捉Mullins-Sekerka不稳定性相关的界面演化。

Conclusion: 该框架为求解由移动边界控制的相变问题提供了一种稳健且灵活的替代方案，具有广泛的应用前景。
...
### [2] [Capturing reduced-order quantum many-body dynamics out of equilibrium via neural ordinary differential equations](https://arxiv.org/abs/2512.13913)
*Patrick Egenlauf, Iva Březinová, Sabine Andergassen, Miriam Klopotek*

Main category: cs.LG

TL;DR: 本文研究了神经网络常微分方程（neural ODE）在模拟非平衡量子多体系统动力学方面的能力，并揭示了其适用性的限制，为改进量子多体理论方法提供了指导。


<details>
  <summary>Details</summary>
Motivation: 现有方法在描述非平衡量子多体系统动力学时面临挑战：精确波函数方法计算量大，简化方法忽略了重要相关性。本文旨在探索一种新的方法，并评估其适用性。

Method: 研究人员使用神经网络常微分方程（neural ODE）模型，基于精确的二体密度矩阵数据进行训练，模拟量子多体系统的动力学，无需显式的三体信息。他们分析了二体和三体相关性之间的Pearson相关性对模型性能的影响。

Result: 结果表明，neural ODE模型仅在二体和三体相关性强烈的参数区域内能够准确重现动力学。当相关性较弱或反相关时，模型失效。三体相关性积累的程度是模型成功的关键预测指标。

Conclusion: 该研究将neural ODE定位为一种模型无关的诊断工具，用于评估微扰展开方法的适用范围，并指导非局部闭合方案的开发。更广泛地讲，从有限数据学习高维密度矩阵动力学为快速、数据驱动的量子相关物质模拟开辟了途径。
...
### [3] [Dropout Neural Network Training Viewed from a Percolation Perspective](https://arxiv.org/abs/2512.13853)
*Finley Devlin, Jaron Sanders*

Main category: cs.LG

TL;DR: 本文将Dropout神经网络训练视为一种渗透现象，并利用渗透理论分析了Dropout的机制和潜在问题。研究表明Dropout存在渗透效应，且在特定条件下可能导致训练崩溃。


<details>
  <summary>Details</summary>
Motivation: 尽管Dropout在实践中广泛应用，但其作为随机近似方法的工作原理尚未完全明确。本文旨在通过渗透理论，深入理解Dropout的机制，填补理论上的空白，并为人工智能和机器学习领域的发展提供更坚实的理论基础。

Method: 本文构建了新的渗透模型来模拟神经网络中的Dropout，并研究了网络拓扑结构与信息传递路径之间的关系。通过对这些模型的分析，揭示了Dropout中的渗透行为。

Result: 研究结果表明，Dropout训练中存在渗透效应，并且在训练无偏置神经网络时，该渗透效应可能导致训练过程崩溃。同时，论文还建立了渗透概率的临界行为。

Conclusion: 本文将Dropout与渗透理论联系起来，为理解Dropout的运作机制提供了一种新的视角。研究结果有助于更好地设计和优化神经网络训练方法，提高模型的泛化能力，并为人工智能领域的理论研究提供新的方向。
...
### [4] [Error Bound Analysis of Physics-Informed Neural Networks-Driven T2 Quantification in Cardiac Magnetic Resonance Imaging](https://arxiv.org/abs/2512.14211)
...
TL;DR: 本文提出了一种基于物理信息神经网络 (PINN) 的心脏磁共振成像 T2 定量方法，无需预定义训练数据库，并推导了 T2 估计误差和泛化误差的理论上界。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法虽然能准确估计 T2 参数，但需要大量训练数据且缺乏理论支持。PINN 在 T2 估计中的应用空白，以及对定量准确性的理论评估需求，是本文研究的动机。

Method: 将 Bloch 方程嵌入到 PINN 的损失函数中，利用 PINN 仅基于目标扫描数据进行 T2 估计。同时，推导了 T2 估计误差和泛化误差的严格上界，为 PINN 的定量准确性提供理论基础。

Result: 在数值心脏模型和水幻象上验证了该方法，结果显示在心肌 T2 范围内具有出色的定量精度。在 94 名急性心肌梗死患者的临床试验中，该方法实现了低误差的 T2 估计，且误差在理论上界内。

Conclusion: 本文提出的 PINN 方法在 T2 定量中表现出鲁棒性和潜力，为无需地面真实数据或金标准即可评估 T2 估计误差提供了理论依据，有望促进 MRI 定量分析的应用。
...
### [5] [Unreasonable effectiveness of unsupervised learning in identifying Majorana topology](https://arxiv.org/abs/2512.13825)
*Jacob Taylor, Haining Pan, Sankar Das Sarma*

Main category: cond-mat.dis-nn

TL;DR: 本文结合无监督学习和有监督学习，利用自编码器分析短无序纳米线中的马约拉纳分裂数据，成功区分了拓扑和非拓扑状态，并确定了两者之间的交叉点。


<details>
  <summary>Details</summary>
Motivation: 在实际系统中识别马约拉纳零模式的拓扑性质面临挑战，传统的有监督机器学习需要大量的带标签数据，而实验数据无法直接提供标签。因此，需要一种无需标签就能识别拓扑状态的无监督学习方法。

Method: 本文提出了一种结合无监督学习（UML）和有监督学习（SML）的方法。首先使用UML识别纳米线数据中的潜在模式，然后利用SML对这些模式进行标记，从而确定拓扑状态。具体采用自编码器进行UML，并将其结果作为SML的输入。

Result: 实验结果表明，该结合UML/SML的方法在测试数据上表现出极高的保真度，能够准确区分拓扑和非拓扑状态，并预测拓扑散射不变量，且该方法更稳健、参数依赖性更低。

Conclusion: 该研究提供了一种有前景的新技术，有望在马约拉纳拓扑量子计算中解决识别马约拉纳零模式拓扑的难题，为实验验证和发展拓扑量子计算提供新的思路。
...
### [6] [From STLS to Projection-based Dictionary Selection in Sparse Regression for System Identification](https://arxiv.org/abs/2512.14404)
*Hangjun Cho, Fabio V. G. Amaral, Andrei A. Klishin, Cassio M. Oishi, Steven L. Brunton*

Main category: stat.ML

TL;DR: 本文重新审视了基于字典的稀疏回归方法STLS，并提出了一种基于得分的库选择策略，旨在为数据驱动建模提供实用指导，特别适用于SINDy算法。


<details>
  <summary>Details</summary>
Motivation: 系统识别从时序数据中仍然是一个重要的挑战，而数据驱动方法，特别是基于符号回归的方法，越来越受到关注。本文旨在改进SINDy算法的鲁棒性和方程发现能力。

Method: 本文提出了基于得分的库选择策略，用于优化STLS算法。该策略通过计算得分来评估字典项，并选择得分最小的子字典，从而实现更准确的字典筛选。

Result: 数值实验表明，基于得分的筛选方法提高了动态系统识别的准确性和可解释性，表明该方法可以增强SINDy算法的鲁棒性。

Conclusion: 将基于得分的方法集成到字典优化中，可以帮助SINDy用户更准确地发现控制方程，从而提高数据驱动发现的效率和可靠性。
...
### [7] [Intelligent matter consisting of active particles](https://arxiv.org/abs/2512.13912)
*Julian Jeggle, Raphael Wittkowski*

Main category: cond-mat.soft

TL;DR: 本文综述了利用简单运动剂系统构建智能系统的途径，探讨了主动物质在涌现计算和物理储层计算方面的应用，旨在实现具有复杂集体行为的“智能物质”。


<details>
  <summary>Details</summary>
Motivation: 自然界中动物群体等系统展现出涌现的复杂行为，激发了研究者希望通过合成物质模拟这种行为，并将其发展到智能系统的复杂程度。

Method: 本文对比了两种主要方法：一是设计专门的主动物质系统直接通过涌现行为解决特定任务的涌现计算；二是利用主动粒子系统的动力学进行物理储层计算。此外，还描述了一种新型的主动粒子计算方案，通过超声或光折射驱动。

Result: 概述了微观尺度主动物质系统在涌现计算和物理储层计算方面的最新进展，强调了集体现象在产生智能行为中的作用。

Conclusion: 通过研究主动物质系统，有望开发出具有适应性和学习能力的智能材料，为人工智能提供一种新的途径，特别是在微纳米尺度上，可以克服传统人工智能的局限性。
