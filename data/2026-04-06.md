...
### [1] [Topological Effects in Neural Network Field Theory](https://arxiv.org/abs/2604.02313)
*Christian Ferko, James Halverson, Vishnu Jejjala, Brandon Robinson*

Main category: hep-th

TL;DR: 本文将拓扑效应引入到神经网络场论(NN-FT)中，通过离散参数模拟拓扑量子数，并成功恢复了Berezinskii–Kosterlitz–Thouless (BKT) 转变和验证了弦理论的T-对偶性。


<details>
  <summary>Details</summary>
Motivation: NN-FT旨在从神经网络架构和参数密度定义场论，而非仅仅通过时空场的路径积分。本文旨在扩展NN-FT框架，以包含拓扑效应，并探索其在物理系统中的应用。

Method: 研究者通过引入离散网络参数来模拟拓扑量子数，从而将拓扑效应纳入NN-FT框架。他们分析了NN-FT架构，进行了解析匹配和数值模拟，并验证了NN-FT对世界面西格玛模型的实现以及T-对偶性。

Result: 研究成功恢复了BKT转变，包括自旋波临界线和高温下的涡旋增殖。此外，还验证了玻色弦的T-对偶性，包括动量和S1上的缠绕的交换不变性、Buscher规则在常数环面背景下的耦合变换、自对偶半径处的电流代数增强以及非几何T-折叠过渡函数。

Conclusion: 本文的结论表明，NN-FT框架可以成功地模拟拓扑效应，并为理解物理系统中的拓扑现象提供了一种新的视角。该研究为机器学习和场论之间的桥梁提供了进一步的理论基础，并为构建更丰富的物理系统提供了框架。
...
### [2] [Real-Time Surrogate Modeling for Personalized Blood Flow Prediction and Hemodynamic Analysis](https://arxiv.org/abs/2604.03197)
...
TL;DR: 本文提出了一种基于机器学习的代理模型框架，用于实时预测个性化血压和心脏输出，从而加速心血管模型的应用并减少非生理参数组合。


<details>
  <summary>Details</summary>
Motivation: 传统的心血管模型在处理大规模人群或生成大型虚拟队列时面临挑战，且某些关键参数难以临床估计，导致大量模拟数据被废弃。为了解决这些问题，需要一种高效的个性化血压和心脏输出预测方法。

Method: 研究人员首先基于Asklepios临床数据集构建了一个参数化虚拟队列，然后训练了一个深度神经网络代理模型，该模型能够预测患者特定的动脉血压和心脏输出。该模型用于快速筛选输入参数，排除非生理组合，并优化目标合成数据集的生成。

Result: 代理模型能够实现即时血压和心脏输出预测，显著降低了目标合成数据集的生成成本。研究还确定了解决心脏输出反问题所需的理论信息量，并在临床数据集上成功估计了中心主动脉的血压和心脏输出。

Conclusion: 该研究提供了一种高效且实用的方法，用于个性化心血管建模和参数估计，有望加速心血管疾病的早期检测和健康追踪，并为临床应用提供支持。
