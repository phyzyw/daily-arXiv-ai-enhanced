...
### [1] [$Δ$-ML Ensembles for Selecting Quantum Chemistry Methods to Compute Intermolecular Interactions](https://arxiv.org/abs/2511.17753)
*Austin M. Wallace, C. David Sherrill, Giri P. Krishnan*

Main category: physics.chem-ph

TL;DR: 本文提出了一种基于∆-ML模型集成的方法，利用预训练的原子对神经网络提取特征，预测不同量子化学方法计算分子间相互作用的误差，从而选择计算效率高且误差可控的方法。


<details>
  <summary>Details</summary>
Motivation: 量子化学计算分子间相互作用的准确性与计算成本之间存在权衡，选择合适的方法是一个挑战。高精度方法（如CCSD(T)/CBS）计算成本高昂，而低精度方法可能存在较大误差，因此需要一种能够有效评估和选择方法的框架。

Method: 该框架基于∆-ML模型集成，这些模型在从预训练的原子对神经网络提取的特征上进行训练，用于预测每种方法相对于其他方法（包括CCSD(T)/CBS）的误差。通过分析所有方法之间的∆-ML模型，可以识别理论假设相符的方法分组。

Result: 在扩展的BioFragment数据集上，该框架实现了小于0.1 kcal/mol的平均绝对误差，表明其具有很高的准确性。同时，研究人员还发现了不同理论方法之间的关联性。

Conclusion: 该研究表明，∆-ML模型能够有效地学习不同理论方法之间的误差校正，为选择合适的量子化学方法提供了新的途径，在分子间相互作用计算中具有广泛的应用前景。
...
### [2] [Analog Physical Systems Can Exhibit Double Descent](https://arxiv.org/abs/2511.17825)
*Sam Dillavou, Jason W Rocks, Jacob F Wycoff, Andrea J Liu, Douglas J Durian*

Main category: cond-mat.dis-nn

TL;DR: 该研究在自调节电阻元件组成的去中心化模拟网络中观察到双下降现象，表明模拟物理系统在适当训练下可以表现出与数字人工智能相似的行为。


<details>
  <summary>Details</summary>
Motivation: 数字神经网络的训练能耗高，而生物系统采用分布式计算。研究旨在探索模拟物理系统能否像数字神经网络一样学习，并验证其在能量效率、速度和容错性方面的潜力，同时解决物理缺陷带来的挑战。

Method: 研究人员使用一种非线性对比局部学习网络（CLLN），该网络由自调节非线性电阻器组成。他们开发了一种修改后的训练协议，以适应模拟系统固有的不完美，并观察其训练过程中的泛化误差。

Result: 标准训练方法未能产生双下降现象，但修改后的训练协议成功地观察到双下降，即随着可调参数数量超过训练数据点数量，泛化误差反而下降。

Conclusion: 该研究表明，经过适当训练的模拟物理系统可以表现出数字人工智能成功的关键特征，并暗示生物系统可能也受益于过参数化。这为模拟物理学习网络的可行性和可扩展性提供了重要证据，并对我们理解学习、适应性和复杂生物功能的稳健性提供了新的视角。
...
### [3] [Artificial Intelligence Driven Workflow for Accelerating Design of Novel Photosensitizers](https://arxiv.org/abs/2511.19347)
*Hongyi Wang, Xiuli Zheng, Weimin Liu, Zitian Tang, Sheng Gong*

Main category: cond-mat.mtrl-sci

TL;DR: 本文提出了一种名为AAPSI的AI驱动的工作流程，通过整合专家知识、基于支架的分子生成和贝叶斯优化，加速了新型光敏剂的设计，并成功发现了一种具有优异性能的光敏剂HB4Ph。


<details>
  <summary>Details</summary>
Motivation: 传统光敏剂设计方法耗时且资源密集，且难以平衡多种物理化学性质。因此，需要更高效的策略来加速光敏剂的发现和优化。

Method: AAPSI工作流程整合了支架驱动的分子生成、图神经网络预测单线态氧量子产率和吸收最大值，以及多目标贝叶斯优化。利用102,534个光敏剂-溶剂对的数据库，生成6,148个可合成的候选分子，并通过实验验证。

Result: AAPSI成功生成了多个新型光敏剂候选物，其中HB4Ph表现出优异的性能，单线态氧量子产率高达0.85，吸收最大波长为650nm，处于当前光敏剂的帕累托前沿。

Conclusion: AAPSI工作流程为光敏剂的快速设计和优化提供了一种有效途径，有望加速光动力疗法（PDT）的发展，并为相关数据库的构建奠定基础。
...
### [4] [Torsion-Space Diffusion for Protein Backbone Generation with Geometric Refinement](https://arxiv.org/abs/2511.19184)
*Lakshaditya Singh, Adwait Shelke, Divyansh Agrawal*

Main category: q-bio.BM

TL;DR: 本文提出了一种基于扭转空间扩散模型的蛋白质骨架生成方法，通过预测扭转角来保证局部几何结构的完美性，并采用约束优化步骤来提升全局紧凑性。


<details>
  <summary>Details</summary>
Motivation: 传统的基于笛卡尔坐标的扩散模型在蛋白质骨架生成中难以保证化学键长和角度的正确性，导致生成的结构不合理。因此，需要一种能够更好地遵循蛋白质几何规则的生成方法。

Method: 该方法采用了一种基于Transformer的扭转空间扩散模型，首先预测蛋白质的扭转角(ϕ, ψ, ω)，然后通过可微分的正向运动学层重建3D坐标，保证键长为3.8Å。此外，还引入了一种迭代约束优化算法，优化半径回旋率(Rg)以提升全局紧凑性，同时保持局部键长不变。

Result: 实验结果表明，该方法实现了100%的键长准确率（平均3.800 Å，σ≈0.0002 Å），并将半径回旋率误差降低至18.6%，显著优于基于笛卡尔坐标的基线模型。

Conclusion: 该研究提供了一个有效的蛋白质骨架生成框架，通过结合Transformer架构和几何转换，实现了物理上合理的蛋白质结构生成，为蛋白质设计和合成生物学领域提供了新的工具。
...
### [5] [Interpreting GFlowNets for Drug Discovery: Extracting Actionable Insights for Medicinal Chemistry](https://arxiv.org/abs/2511.19264)
*Amirtha Varshini A S, Duminda S. Ranasinghe, Hok Hei Tam*

Main category: cs.LG

TL;DR: 本文提出了一种用于解释生成流网络(GFlowNets)的框架，旨在揭示其在药物发现中的决策过程，并为药物化学家提供可操作的见解。


<details>
  <summary>Details</summary>
Motivation: 现有的深度生成模型虽然能提出新分子，但往往缺乏合成可行性、多样性和可解释性，阻碍了其在药物发现中的应用。药物化学家需要了解模型选择分子的原因，例如哪些官能团起作用，哪些物理化学性质被优先考虑，以及结构修改如何影响预测结果。

Method: 该框架结合了三种方法：(i) 基于梯度的显著性分析与分子子结构的逆反扰动；(ii) 通过在内部嵌入上训练的稀疏自编码器(SAE)进行概念归因；(iii) 动机探针，评估功能基团是否可以从嵌入中线性解码。将这些方法应用于SynFlowNet，分析其内部表示。

Result: 研究表明，SynFlowNet的内部表示组织了药物相似性(QED)，并沿物理化学可解释的轴（如极性、亲脂性和大小）排列。卤素和芳环系统可靠地编码为基团水平特征。逆反显著性进一步识别了子结构，其有针对性的替换会产生预测奖励的系统性变化。

Conclusion: 该研究扩展了结构化生成策略的可解释性工具，为药物化学驱动的分子设计提供了实际的见解，有助于提高模型的可信度和在药物发现流程中的整合。
...
### [6] [Physical Reinforcement Learning](https://arxiv.org/abs/2511.17789)
*Sam Dillavou, Shruti Mishra*

Main category: cs.LG

TL;DR: 本文将强化学习(RL)应用于新兴的对比局部学习网络(CLLNs)，这是一种基于自调节非线性电阻器的模拟网络，旨在探索在能源受限和不确定环境中构建低功耗、容错自主代理的可能性。


<details>
  <summary>Details</summary>
Motivation: 数字计算机在能源消耗和容错性方面存在局限性，而生物系统在这些方面表现出色。本文旨在探索利用模拟硬件（CLLNs）进行RL的可能性，以克服数字计算机的这些缺点，并研究RL工具在非数字系统中的适用性。

Method: 研究人员将Q-learning算法应用于模拟的CLLNs，并在两个简单的RL问题上进行了实验。他们分析了哪些RL工具（如策略函数和价值函数）在CLLNs中更自然，哪些需要额外的预编程硬件。

Result: 实验表明，CLLNs可以在模拟环境中成功执行简单的RL任务，并揭示了在数字RL和基于CLLNs的RL之间存在的差异，例如对物理安全的需求。

Conclusion: 该研究为构建能源效率高、容错性强的自主代理开辟了道路，并强调了生物系统中重要的概念（如物理安全）在CLLNs中的应用潜力，为未来在非数字硬件上实现RL提供了新的思路。
