...
### [1] [Physics-informed neural operator for predictive parametric phase-field modelling](https://arxiv.org/abs/2603.09693)
*Nanxi Chen, Airong Chen, Rujin Ma*

Main category: cs.LG

TL;DR: 本文提出了一种物理信息神经网络算子框架（PF-PINO），用于加速参数化相场模型求解，通过将相场控制方程的残差嵌入到损失函数中，有效施加物理约束。


<details>
  <summary>Details</summary>
Motivation: 传统的相场模型计算量大，难以进行高通量参数化研究。虽然神经网络算子（如FNO）可以加速求解参数化偏微分方程，但缺乏显式的物理约束可能限制其泛化能力和长期精度。

Method: PF-PINO框架通过将相场控制方程的残差嵌入到数据保真度损失函数中，在训练过程中强制执行物理约束。该框架被应用于电化学腐蚀、树枝状晶体凝固和自发分解等基准相场问题。

Result: 实验结果表明，PF-PINO在精度、泛化能力和长期稳定性方面显著优于传统的FNO。

Conclusion: 该研究提供了一种强大而高效的相场模型计算工具，并突出了物理信息神经网络算子在推进复杂界面演化问题的科学机器学习方面的潜力。
...
### [2] [First Estimation of Model Parameters for Neutrino-Induced Nucleon Knockout Using Simulation-Based Inference](https://arxiv.org/abs/2603.09778)
*Karla Tame-Narvaez, Steven Gardiner, Aleksandra Ćiprijanović, Giuseppe Cerati*

Main category: hep-ph

TL;DR: 本文研究了基于模拟推断（SBI）方法在中微子核反应模型参数调整中的应用，并发现SBI算法在拟合实验数据时，能够给出与传统方法略有不同的参数值，并取得更好的拟合效果。


<details>
  <summary>Details</summary>
Motivation: 为了提高下一代中微子实验的精度，需要对中微子-原子核相互作用的模拟进行精确调整。随着实验精度要求的提高，机器学习技术有望处理未来中微子相互作用模型调整中日益增长的复杂性。

Method: 研究人员重新审视了MicroBooNE合作开发的GENIE中微子事件生成器的调整配置，并使用基于模拟推断（SBI）算法进行参数调整。SBI算法通过训练来学习模拟参数，使其更好地拟合实验数据。

Result: 训练后的SBI算法在拟合实验数据时，得到的参数值与MicroBooNE合作最初设定的值略有不同（在MicroBooNE指定的误差范围内），并且在拟合优度方面略有提升。此外，该算法还能较好地模拟另一种中微子散射模拟NuWro。

Conclusion: 本文表明，基于模拟推断（SBI）方法是一种有潜力的中微子核反应模型参数调整技术，可以提高模拟精度，并为未来更复杂的模型调整提供解决方案。
...
### [3] [Verifying Good Regulator Conditions for Hypergraph Observers: Natural Gradient Learning from Causal Invariance via Established Theorems](https://arxiv.org/abs/2603.09067)
*Max Zhuravlev*

Main category: stat.ML

TL;DR: 本文验证了在因果不变的超图基底中，持久的观察者满足Conant-Ashby良性调节器定理的条件，从而证明了它们必须使用自然梯度下降学习，与Vanchurin的神经网络宇宙学一致。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于探索因果不变性是否能够约束物理定律，并连接Wolfram物理和Vanchurin神经网络宇宙学这两个独立的宇宙学研究项目。

Method: 本文将持久观察者形式化为在环境边界处最小化预测误差的实体，并应用Conant-Ashby定理和Amari的唯一性定理。通过分析损失函数、Fisher信息度量和参数空间，推导了自然梯度下降的学习规则，并对Vanchurin框架中的参数进行了计算预测。

Result: 研究结果表明，在特定条件下（M=F2，特定收敛时间函数），可以推导出Vanchurin Type II框架中参数α的闭式公式，并在量子-经典阈值处得到κ(F) = 2。然而，其他收敛模型并未复现该结果。此外，研究还引入了方向参数α vk和迹自由偏差张量∆ µν，表明单个观察者可以同时处于不同的Vanchurin区域。

Conclusion: 本文通过验证Amari链，将因果不变性与自然梯度学习联系起来，为超图宇宙学提供了一个应用框架，并贡献了约25-30%的新颖性，为宇宙学统一程序提供了新的视角。
...
### [4] [A Variational Latent Equilibrium for Learning in Cortex](https://arxiv.org/abs/2603.09600)
*Simon Brandt, Paul Haider, Walter Senn, Federico Benitez, Mihai A. Petrovici*

Main category: q-bio.NC

TL;DR: 本文提出了一种基于变分潜势平衡的通用形式化方法，用于在生物学上合理地逼近大脑中的时间回溯传播 (BPTT) 算法，并为实现这种计算的物理电路提供了蓝图。


<details>
  <summary>Details</summary>
Motivation: 现有的人工智能算法，特别是回溯传播算法，与我们对大脑电路和动态的理解存在偏差。本文旨在提出一种更符合生物学原理的学习方法，以更好地模拟大脑的学习机制。

Method: 该方法基于能量守恒和极值作用的原则，构建了一个神经元状态的潜在能量函数，并从中推导出实时误差动态。通过变分微积分，该方法可以推导出神经元网络的伴随方法结果，并进一步简化为局部（空间和时间）的神经元和突触动力学方程，从而改进了最近提出的广义潜势平衡 (GLE) 模型。

Result: 该理论提供了一个用于大脑中时空深度学习的严格框架，并能够逼近伴随方法 (AM)，同时为能够执行这些计算的物理电路提供了蓝图。

Conclusion: 本文提出的方法为理解大脑学习机制提供了新的视角，并为构建生物学上合理的神经计算系统提供了设计灵感，有望促进人工智能与神经科学的交叉融合。
