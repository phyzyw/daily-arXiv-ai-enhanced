...
### [1] [Pharmacology Knowledge Graphs: Do We Need Chemical Structure for Drug Repurposing?](https://arxiv.org/abs/2603.01537)
*Youssef Abo-Dahab, Ruby Hernandez, Ismael Caleb Arechiga Duran*

Main category: cs.AI

TL;DR: 本研究表明，在药物重定位中，仅使用靶点信息和药物网络拓扑结构，而无需显式化学结构表示，即可准确预测药物的药理行为。


<details>
  <summary>Details</summary>
Motivation: 药物发现成本高昂且耗时，药物重定位是一种更具成本效益的替代方案。本研究旨在量化知识图谱中模型复杂性、数据量和特征模态对药物重定位的相对贡献。

Method: 构建了一个包含5348个实体和20015条边的药理知识图谱，并使用严格的时间分割（训练：≤2022；测试：2023–2025）进行验证。比较了五种知识图谱嵌入模型和一种结合药物化学结构的图神经网络（GNN），并通过PR-AUC和Hits@kon指标评估了药物-蛋白质和药物-适应症的链接预测性能。进行了特征消除和模型缩放实验。

Result: 研究发现，移除GNN中的基于GAT的药物结构编码器，仅保留拓扑嵌入和蛋白质特征，反而提高了药物-蛋白质的PR-AUC。显式化学结构表示不仅是冗余的，而且可能对预测药理网络交互产生不利影响。数据量对模型性能影响大于模型复杂性。

Conclusion: 药物的药理行为可以通过靶点信息和药物网络拓扑结构进行准确预测，无需显式化学结构表示。强调了数据量和节点密度对模型性能的重要性，表明在预算有限的硬件上即可实现先进的性能。
...
### [2] [Graph neural network force fields for adiabatic dynamics of lattice Hamiltonians](https://arxiv.org/abs/2603.02039)
*Yunhao Fan, Gia-Wei Chern*

Main category: cond-mat.str-el

TL;DR: 本文提出了一种基于图神经网络 (GNN) 的力场框架，用于模拟晶格哈密顿量的绝热动力学，并展示了其在 Holstein 模型中的应用，揭示了电荷密度波序化的动态行为。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习力场模型在扩展到大时空尺度方面存在局限性，尤其是在处理晶格对称性时。需要更简洁和统一的替代方案，以实现对称性感知的大规模动力学模拟。

Method: 研究人员开发了一种基于 GNN 的力场框架，通过局部消息传递和权重共享直接强制执行离散晶格平移和点群对称性。该框架在精确对角化数据上进行训练，并应用于 Holstein 模型。

Result: GNN 力场实现了高精度的力，严格的系统尺寸线性扩展，以及直接的大规模晶格转移能力。通过 Langevin 模拟，观察到电荷密度波序化的动态行为，揭示了动态缩放和异常缓慢的亚 Allen–Cahn 粗化。

Conclusion: 这项研究表明，GNN 是一种优雅高效的架构，可用于对称性感知、大规模的关联晶格系统的动力学模拟，为材料科学研究提供了新的工具。
