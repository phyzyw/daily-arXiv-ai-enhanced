...
### [1] [Thermodynamic properties of chemically disordered compounds via AI-driven estimation of partition function with the PULSE method](https://arxiv.org/abs/2605.28594)
*Baptiste Bernard, Luca Messina, Eiji Kawasaki, Emeric Bourasseau*

Main category: cond-mat.stat-mech

TL;DR: 本文提出了一种改进的PULSE方法，利用人工智能驱动的采样和评估来估计化学无序化合物的划分函数，从而高效地计算其热力学性质。


<details>
  <summary>Details</summary>
Motivation: 核安全和反应堆效率需要准确表征材料性质，但实验条件苛刻。对于新型多组分材料（如高熵合金和MOX燃料）的原子级建模，尤其是在化学无序带来的高维度配置空间下，传统方法计算成本过高。

Method: 改进了PULSE（划分函数无监督学习采样和评估）方法，该方法基于逆变分自编码器架构，通过生成式采样找到系统的重要配置，从而计算其平均性质。使用2D Ising模型作为基准进行验证。

Result: 实验结果表明，改进后的PULSE方法能够以高精度和效率重现平均性质，优于传统的蒙特卡洛采样方法。

Conclusion: PULSE方法是一种有价值的工具，可用于研究传统方法效率较低，且受化学无序影响的材料的热力学性质，为核工业中的材料建模提供了新的途径。
...
### [2] [DGLD: Domain-Gated Latent Diffusion for the Discovery of Novel Energetic Materials](https://arxiv.org/abs/2605.26540)
*Yehudit Aperstein, Alexander Apartsin*

Main category: physics.chem-ph

TL;DR: 本文提出了一种名为DGLD的新方法，用于发现新型高能材料。该方法结合了领域门控、潜在扩散模型和多阶段化学验证，成功发现了12种DFT确认的新型先导化合物，其中一种化合物的性能指标达到HMX/CL-20的水平。


<details>
  <summary>Details</summary>
Motivation: 由于过去15年内没有新的HMX类化合物被公开，而高能材料的性能提升可以带来火箭推进剂质量降低、弹头尺寸减小和民用燃气发生器效率提高等好处，因此迫切需要开发新的高能材料。

Method: DGLD方法包括：训练时使用四级标签信任等级进行门控，区分实验和DFT标签与低可靠性标签；采样时使用多任务评分模型进行每一步引导；以及一个包含SMARTS化学门控、帕累托重新排序、半经验GFN2-xTB分诊和全DFT审计的四阶段验证漏斗。

Result: DGLD成功发现了12种DFT确认的新型先导化合物，其中一种名为3,4,5-三硝基-1,2-异恶唑（L1）的化合物，其密度和爆速分别达到2.09 g/cm³和8.25 km/s，且与训练集中的所有分子结构差异显著。此外，还发现了一种名为E1的化合物，其爆速超过L1。

Conclusion: DGLD是唯一能够在DFT级别同时实现新颖性和目标性（即同时具有新颖性和性能指标）的方法，证明了其在新型高能材料发现方面的有效性，并为未来发现和合成HMX类化合物提供了可行途径。
...
### [3] [Hybrid Neural World Models](https://arxiv.org/abs/2605.28317)
*Pranav Lakshmanan, Paras Chopra*

Main category: cs.LG

TL;DR: 本文提出了一种混合神经网络世界模型，通过训练一个多时间跨度的神经网络代理来预测物理动力系统的未来状态，并利用其前向传播结果生成误差图，从而识别不确定轨迹，无需额外训练或领域知识。


<details>
  <summary>Details</summary>
Motivation: 现有神经网络代理在预测物理动力系统时，在尖锐动态事件（如冲击波、前沿和接触）时会失效，且通常无法检测到这些失效，这限制了其在实际应用中的安全性。

Method: 该方法训练一个具有连续时间跨度调度的多时间跨度代理，直接使用参考求解器进行监督回归，预测未来状态。通过比较代理预测和半时间跨度迭代预测之间的差异，生成误差图，用于识别不确定轨迹。然后，采用双模式部署策略：模式1仅使用代理，模式2使用误差图来选择性地回退到参考求解器。

Result: 实验结果表明，该方法在反应-扩散、可压缩欧拉和刚体碰撞动力学等不同物理系统中均有效。误差图的性能优于或与标准无标签基线相当，同时实现了高达72倍的CPU加速，并且可以显著降低残差误差。

Conclusion: 该研究提供了一种无需额外训练、校准集或领域知识即可识别不确定轨迹的有效方法，为大规模部署神经网络代理解决物理动力系统问题提供了新的途径。
...
### [4] [High-Fidelity Industrial Crash Dynamics Prediction via Geometry-Aware Operator Learning with Memory-Efficient Low-Rank Attention](https://arxiv.org/abs/2605.27758)
*Deepak Akhare, Mohammad Amin Nabian, Corey Adams, Sudeep Chavare, Sanjay Choudhry*

Main category: cs.LG

TL;DR: 本文提出了一种基于几何感知的算子学习框架GeoTransolver，用于工业级汽车碰撞动态预测，并通过引入低秩注意力路由引擎FLARE进一步提升了效率和精度。


<details>
  <summary>Details</summary>
Motivation: 传统有限元分析在汽车碰撞安全性优化中计算成本高昂，限制了大规模设计探索。因此，需要一种快速且高保真度的替代模型，以加速碰撞安全性设计流程。

Method: 该研究基于GeoTransolver框架，并引入了Fast Low-rank Attention Routing Engine (FLARE) 改进注意力机制。同时，系统性地评估了多种时间预测策略，最终采用one-shot方法。

Result: GeoTransolver在复杂的保险杠梁和全车碰撞数据集上表现出色，准确捕捉了多尺度几何上下文和塑性变形模式，同时实现了state-of-the-art的预测精度。FLARE降低了内存开销约2倍，并提高了对长程高频瞬变的预测精度。

Conclusion: 该研究证明了基于几何感知的算子学习在工业级汽车碰撞动态高保真代理建模中的实用可行性，为汽车碰撞安全性设计提供了新的解决方案。
...
### [5] [Co-folding model guided by structural proteomics](https://arxiv.org/abs/2605.26192)
*Alon Shtrikman, Nitzan Simchi, Michal Ran Shchory, Sagie Brodsky, Eran Seger, Kirill Pevzner*

Main category: cs.LG

TL;DR: 本文提出了AIMS-Fold框架，它将结构蛋白质组学数据（XL-MS和HDX-MS）与预训练的扩散模型相结合，以更准确地预测蛋白质复合物的结构，尤其是在诱导邻近药物设计中。


<details>
  <summary>Details</summary>
Motivation: 现有的蛋白质结构生成模型通常难以准确预测蛋白质复合物的构象状态，而结构蛋白质组学技术可以提供有价值的空间和动态信息。因此，需要一种方法将这些实验数据整合到结构生成模型中。

Method: AIMS-Fold是一种基于扩散的生成模型，在推断时利用结构蛋白质组学数据引导结构生成。它将XL-MS空间约束和HDX-MS溶剂可及性曲线转化为可微分的物理势能，从而在逆扩散过程中主动改变概率景观，引导采样轨迹。

Result: 实验表明，AIMS-Fold能够利用XL-MS和HDX-MS数据，在预测具有挑战性的诱导邻近药物靶标时，比纯计算模型（如Boltz-2）具有更高的准确性，并且单独使用XL-MS和HDX-MS数据也能提高预测准确性。

Conclusion: AIMS-Fold为基于结构的药物设计，特别是诱导邻近药物的设计，提供了一种强大的整合计算方法，有望加速新药研发。
