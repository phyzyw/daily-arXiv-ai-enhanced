...
### [1] [Polarizable atomic multipoles for learning long-range electrostatics](https://arxiv.org/abs/2605.05746)
*Dongjin Kim, Daniel S. King, Yoonjae Park, Roya Savoj, Sebastien Hamel, Xiaoyu Wang, Bingqing Cheng*

Main category: cond-mat.mtrl-sci

TL;DR: 本文提出了一种半局域框架，利用可极化的原子多极子学习长程静电相互作用，该框架不仅提高了机器学习原子间势能表面的准确性，还能够恢复物理上合理的电学响应。


<details>
  <summary>Details</summary>
Motivation: 传统的机器学习原子间势能（MLIP）通常基于局域性假设，难以准确描述离子、极性和界面系统中长程静电相互作用和极化现象，因此需要一种能够有效建模长程效应的方法。

Method: 该框架使用局域等价描述符预测环境相关的潜在单极子、偶极子和四极子，并通过非自洽线性响应捕捉残余的非局域电荷转移和极化。它结合了局部描述符和非局域响应项，无需额外的电子结构标签。

Result: 在四个不同的基准测试和四个短程MLIP架构中，多极子层级和响应项系统地提高了势能表面的准确性。学习到的潜在变量能够恢复物理上合理的电学响应，包括准确的Born有效电荷张量、涌现的极化率、与实验一致的红外光谱以及对水和MAPbI3钙钛矿的半定量拉曼光谱。

Conclusion: 该框架提供了一种系统可改进且物理上透明的方法，使基于标准能量和力标签训练的MLIP能够预测对极化敏感的观测值，为机器学习原子间势能的进一步发展提供了新的思路。
...
### [2] [Concurrence of Symmetry Breaking and Nonlocality Phase Transitions in Diffusion Models](https://arxiv.org/abs/2605.04830)
*Yifan F. Zhang, Fangjun Hu, Guangkuo Liu, Mert Okyay, Xun Gao*

Main category: cs.LG

TL;DR: 本文研究了现代扩散模型中对称性破缺和非局域性两种相变概念是否同时发生，并观察到它们几乎同时出现。


<details>
  <summary>Details</summary>
Motivation: 虽然扩散模型已能生成高质量图像，但对其生成过程中不同能力何时启动的理解仍然有限。现有研究分别从对称性破缺和非局域性角度解释了扩散模型的相变，但缺乏统一的实践诊断。

Method: 通过评估生成轨迹的动态和结果，研究人员观察了扩散模型在生成过程中对称性破缺和非局域性相变发生的时机。

Result: 研究发现，在现代扩散转换器中，非局域性和对称性破缺的临界时间几乎同时发生。

Conclusion: 这项工作首次统一了对称性破缺和非局域性两种相变的概念，为理解扩散模型何时以及为何依赖条件和全局去噪提供了具体的诊断方法，并为模型效率评估和架构设计提供了指导。
...
### [3] [Agentic Discovery of Exchange-Correlation Density Functionals](https://arxiv.org/abs/2605.05460)
*Titouan Duston, Jiashu Liang, Yuanheng Wang, Weihao Gao, Xuelan Wen, Nan Sheng, Weiluo Ren, Yang Sun, Yixiao Chen*

Main category: cs.AI

TL;DR: 本文提出了一种基于大型语言模型（LLM）的智能搜索系统，用于自动发现新的交换-关联（XC）密度泛函，并成功地将性能提升了约9%，超越了目前业界标准ωB97M-V。


<details>
  <summary>Details</summary>
Motivation: 现有交换-关联泛函的开发主要依赖于人工设计，缺乏系统性的自动化方法。大型语言模型的发展为自动设计提供了新的可能性，但需要解决现有泛函已经高度优化以及过度拟合基准数据集的问题。

Method: 研究人员构建了一个智能搜索系统，该系统利用LLM提出结构化的泛函形式修改，并通过一个迭代的计划-执行-总结循环来优化泛函性能。该系统通过优化泛函参数来衡量改进，并使用保留的子集进行性能评估，同时强调探索新结构区域的重要性。

Result: 发现的最高性能泛函SAFS26-a在标准热化学数据集上性能提升了约9%，优于ωB97M-V基准。同时，研究也发现强大的AI模型可能利用非物理的捷径来“作弊”基准，强调领域专业知识和约束条件的重要性。

Conclusion: 该研究表明，基于LLM的智能搜索系统在自动发现高性能交换-关联泛函方面具有潜力，但也突出了AI辅助科学中保持结果科学性的重要性，即需要结合领域知识和明确的约束条件。
...
### [4] [FunctionalAgent: Towards end-to-end on-top functional design](https://arxiv.org/abs/2605.06215)
*Yuhao Chen, Donald G. Truhlar, Xiao He*

Main category: physics.chem-ph

TL;DR: 本文介绍了一种名为FunctionalAgent的智能系统，用于自动化多配置对密度泛函理论(MC-PDFT)中的on-top泛函开发，并成功开发出MC26和COF26两种新型泛函。


<details>
  <summary>Details</summary>
Motivation: 准确且计算上可行的电子结构预测对于强相关分子系统仍然是一个核心挑战。MC-PDFT在准确性和效率之间提供了平衡，但其预测能力的关键在于on-top泛函的质量，而传统的泛函开发过程复杂且各个环节相互影响。

Method: FunctionalAgent通过协调专门的子代理，将泛函开发过程分解为数据集构建、主动空间生成、MCSCF计算、描述符生成、损失函数构建、泛函拟合、优化和评估等多个阶段，形成一个闭环自动化工作流程。利用该系统，研究人员开发了MC26和COF26两种新型泛函。

Result: 使用FunctionalAgent开发的MC26泛函在训练集上表现出优于其他方法的整体准确性。COF26泛函则在训练集和测试集上都达到了最佳性能。

Conclusion: FunctionalAgent为自动化泛函开发提供了一种新途径，有望加速化学复杂电子结构问题的研究，并为开发更准确、更高效的量子化学计算方法奠定基础。
...
### [5] [A renormalization-group inspired lattice-based framework for piecewise generalized linear models](https://arxiv.org/abs/2605.05493)
*Joshua C. Chang*

Main category: stat.ME

TL;DR: 本文提出了一种受重整化群理论启发的新型模型框架，基于多维格点划分，构建分段广义线性模型（piecewise GLMs），兼顾了可解释性和模型复杂度的平衡。


<details>
  <summary>Details</summary>
Motivation: 现有模型在可解释性和非线性建模能力之间存在权衡。本文旨在设计一种具有内在可解释性的模型，同时允许模型参数随输入空间的变化而变化，以捕捉数据集中的非线性关系。

Method: 该框架通过定义输入空间的格点划分，并使用分层展开来表达回归参数，从而实现参数随不同尺度交互的变化。研究人员利用统计物理中的Replica分析方法，分析了Watanabe-Akaike信息准则（WAIC）作为泛化损失的代理，并据此指导格点设计和正则化参数选择。

Result: 研究结果表明，该方法能够提供格点设计和正则化参数选择的指导，并揭示了模型复杂度和泛化损失之间的scaling law。在公共数据集上的实验结果表明，该模型在性能上与黑盒方法和其他内在可解释方法具有竞争力。

Conclusion: 该研究提供了一种新的、内在可解释的模型设计范式，为构建可解释且具有强大非线性建模能力的统计模型提供了理论基础和实践指导，并为解决实际应用中的可解释性问题提供了新的思路。
