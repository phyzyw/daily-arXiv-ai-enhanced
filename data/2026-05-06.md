...
### [1] [Expanding functional protein sequence space using high entropy generative models](https://arxiv.org/abs/2605.03578)
*Roberto Netti, Emily Hinds, Francesco Calvanese, Rama Ranganathan, Martin Weigt, Francesco Zamponi*

Main category: q-bio.QM

TL;DR: 本研究调查了玻尔兹曼模型参数密度与蛋白质设计性能之间的关系，发现高熵模型能够更好地表示潜在的进化适应度景观，并提供更大的可采样序列空间。


<details>
  <summary>Details</summary>
Motivation: 现有数据驱动的蛋白质设计方法（如DCA）通常会过度参数化，导致模型过拟合，难以准确捕捉进化适应度景观。本研究旨在探索参数密度（即模型稀疏性）对模型性能的影响。

Method: 研究人员以酒石酸盐合酶家族为模型系统，比较了标准全连接玻尔兹曼机 (bmDCA)、通过逐步边激活生成的稀疏模型 (eaDCA) 和通过边删除生成的稀疏模型 (edDCA)。他们识别出在删除轨迹上具有最大熵的模型 (meDCA)，并通过体内互补试验测试了所有模型的合成序列。

Result: 所有架构的蛋白质都能成功合成功能性酶，即使与天然序列有显著差异。meDCA模型的可采样序列空间比低熵模型大15个数量级。高熵模型系统地减少了过拟合，并更好地捕捉了天然蛋白质周围的局部中性空间。

Conclusion: 虽然各种满足协同进化统计的模型都可以生成功能序列，但高熵玻尔兹曼机提供了对潜在进化适应度景观的更优越的表示，为蛋白质设计提供了更广阔的可能性。
...
### [2] [Analytic Bridge Diffusions for Controlled Path Generation](https://arxiv.org/abs/2605.02961)
*Michael Chertkov*

Main category: cs.LG

TL;DR: 本文提出了一种名为LQ-GM-PID的解析桥扩散模型，它通过将经典线性-二次-高斯（LQG）随机控制结构推广到有限时间传输问题，实现了路径形状控制，无需神经网络。


<details>
  <summary>Details</summary>
Motivation: 现有桥扩散方法通常依赖于神经网络来估计分数或漂移场，缺乏解析透明度。本文旨在寻找一个足够表达力且解析可解的子类，作为评估神经网络桥扩散方法的基准。

Method: 将经典LQG随机控制结构推广到有限时间传输问题，形成LQ-GM-PID模型。该模型保留了线性-二次随机控制的骨干，但用预先设定的终端概率密度替换了终端状态的调节，并允许初始和终端分布为高斯混合模型。协议参数（βt, νt, σt, κt）保持低维且可解释。

Result: 在2D走廊任务、2D多入口传输任务以及高维（d=32，M=16）分支任务中，LQ-GM-PID模型实现了路径形状控制，并在笔记本电脑上实现了小于50ms的解析预计算。

Conclusion: LQ-GM-PID提供了一个解析可解的参考模型，用于评估和测试最先进的神经网络桥扩散和生成传输方法，为路径形状控制和生成建模提供了一种新的思路。
...
### [3] [Pretrained Model Representations as Acquisition Signals for Active Learning of MLIPs](https://arxiv.org/abs/2605.03964)
*Eszter Varga-Umbrich, Shikha Surana, Paul Duckworth, Jules Tilly, Olivier Peltre, Zachary Weller-Davies*

Main category: cs.LG

TL;DR: 本文研究了如何利用预训练机器学习原子间势（MLIP）模型的潜在空间信息作为主动学习的获取信号，无需额外的不确定性头、贝叶斯训练或委员会集成，从而有效减少对昂贵量子化学标签的需求。


<details>
  <summary>Details</summary>
Motivation: 训练用于反应化学的MLIP需要大量的量子化学标签，且过渡态配置稀少。主动学习可以缓解这些问题，但其有效性取决于获取规则。现有主动学习方法通常依赖委员会不一致或外推标准，而本文旨在探索预训练MLIP模型本身是否已经包含有效获取所需的信息。

Method: 研究人员从预训练的MACE势中提取两种获取信号：有限宽度神经网络切向核（NTK）和基于隐藏潜在空间特征的激活核。在反应化学基准测试中，将这些核与固定描述符基线、委员会不一致和随机获取进行比较。

Result: 实验结果表明，这两种核都显著优于固定描述符基线、委员会不一致和随机获取，平均减少了38%的能量误差和28%的力误差。预训练模型诱导的相似空间保留了化学上具有意义的结构，并提供了比随机初始化或基于固定描述符的核更可靠的残余不确定性估计。

Conclusion: 研究结果表明，预训练将潜在空间几何与模型误差对齐，从而为反应MLIP的微调提供了一种实用且充分的获取信号。这表明预训练模型本身就蕴含了主动学习所需的信息，无需额外的复杂机制。
