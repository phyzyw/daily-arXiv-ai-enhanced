...
### [1] [Improving FMQA via Initial Training Data Design Considering Marginal Bit Coverage in One-Hot Encoding](https://arxiv.org/abs/2605.04825)
*Taiga Hayashi, Yuya Seki, Kotaro Terada, Yosuke Mukasa, Shuta Kikuchi, Shu Tanaka*

Main category: cs.LG

TL;DR: 本文提出了一种改进的FMQA方法，通过设计初始训练数据以实现完全边际比特覆盖，从而提升优化性能，尤其是在高维问题中。


<details>
  <summary>Details</summary>
Motivation: 在将FMQA应用于通过一热编码的整数或离散连续变量时，随机初始采样可能导致许多二进制变量在初始训练数据中从未激活，从而影响FM参数的梯度更新。为了解决这个问题，需要改进初始训练数据的设计。

Method: 本文提出了两种空间填充采样方法，即拉丁超立方体采样（LHS）和Sobol序列，分别构建了LHS-FMQA和Sobol’-FMQA。这些方法确保了通过一热编码获得的每个二进制变量至少出现一次值为一的情况，实现了完全边际比特覆盖。

Result: 在17和32个设计变量的人力驱动飞机翼型优化基准测试中，LHS-FMQA和Sobol’-FMQA均实现了比基线FMQA更高的平均最终巡航速度，尤其是在32变量问题中优势更明显。

Conclusion: 通过设计初始训练数据实现完全边际比特覆盖，可以有效提升FMQA在黑盒优化中的性能，特别是在处理高维离散优化问题时，为解决实际工程问题提供了新的思路。
...
### [2] [From Video-to-PDE: Data-Driven Discovery of Nonlinear Dye Plume Dynamics](https://arxiv.org/abs/2605.04535)
*Cesar Acosta-Minoli, Sayantan Sarkar*

Main category: cs.LG

TL;DR: 本文提出了一种视频到偏微分方程 (PDE) 的转换流程，能够从染料羽流的灰度视频中自动发现非线性输运模型，并评估模型系数的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以直接从视频中推断出连续模型，因为视频记录的是未经校准的图像强度，且直接数值微分对噪声帧不稳定。本文旨在解决这一问题。

Method: 该流程包括：将灰度视频转换为归一化标量场，分离出整体漂移和内在扩散，并通过弱形式稀疏回归识别有效输运律。采用条件化、阈值扫描和随机中心诊断来选择紧凑的基于梯度的库，并使用逆物理信息网络和前滚轮校准来优化系数，同时使用时间块自举量化不确定性。

Result: 选定的PDE模型（𝑢𝑡+𝐯(𝑡)⋅∇𝑢= 9.005|∇𝑢|2+0.666Δ𝑢）在保留测试帧上优于对流-扩散基线模型，具有正的拉普拉斯系数，并且可以简化为线性对流-扩散方程。

Conclusion: 该框架表明，即使使用未经校准的视觉数据，也可以获得紧凑、可预测且结构可解释的连续模型，证明了将发现、校准和不确定性视为独立阶段的重要性。
...
### [3] [Deep Wave Network for Modeling Multi-Scale Physical Dynamics](https://arxiv.org/abs/2605.04198)
*Alexander I. Khrabry, Edward A. Startsev, Andrew T. Powis, Igor D. Kaganovich*

Main category: cs.LG

TL;DR: 本文提出了一种名为Deep Wave Network (DW-Net) 的新型深度学习架构，通过堆叠多个U-Net模块并引入跨尺度特征精炼机制，在多尺度物理动力学建模中实现了更高的精度和更低的计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型在多尺度物理动力学建模中，通常只关注宽度调整，而忽略了深度对精度-成本权衡的影响。U-Net模型尤其如此，其深度通常固定，限制了系统性地探索深度对性能提升的作用。

Method: DW-Net架构通过在序列中堆叠多个encoder-decoder“波”（U-Net模块），并在每个波和连续波之间引入跳跃连接，从而增加了有效的深度。研究人员保持了训练数据和学习计划不变，并使用GPU时间来量化计算成本，比较不同宽度变体的精度-GPU时间Pareto前沿。

Result: 在多个2D和3D流动基准测试中，DW-Net模型在匹配GPU时间下实现了更高的精度，或在可比的GPU时间下实现了可观的精度提升，并且在相同的训练计划下，训练时间减少了最高3倍。

Conclusion: DW-Net架构证明了深度在多尺度物理动力学建模中的重要性，为构建更高效、更准确的机器学习模型提供了新的思路，有望加速复杂物理系统的预测和模拟。
...
### [4] [Enabling Real-Time Training of a Wildfire-to-Smoke Map with Multilinear Operators](https://arxiv.org/abs/2605.04164)
*Zachary Morrow, Joseph Crockett, John D. Jakeman, Dan J. Krofcheck*

Main category: cs.LG

TL;DR: 本文提出了一种基于多线性算子的数据驱动方法，用于实时预测野火产生的烟雾浓度场，从而避免了昂贵的耦合火-大气模型模拟。


<details>
  <summary>Details</summary>
Motivation: 传统的耦合火-大气模型计算成本高昂，难以进行大规模的蒙特卡洛采样以预测长期燃料管理和气候变化场景下的烟雾影响，尤其是在需要预测电网输出减少的情况下。

Method: 该方法首先计算点火时间与烟雾浓度场的特征分解，然后学习一个从输入系数的幂映射到输出系数的算子。将学习到的算子应用于上里奥格兰德流域的烟雾预测，实现了快速的预测速度。

Result: 在气溶胶光学深度（AOD）的预测上，该方法在达到与蒙特卡洛采样相当的准确度的同时，减少了耦合模型调用次数。在烟雾足迹确定方面，获得了65%的交并比（IoU）和0.95的曲线下面积（AUC），显著优于现有方法。

Conclusion: 该方法提供了一种高效且准确的烟雾预测方法，能够显著降低计算成本，并为长期燃料管理和气候变化场景下的电网影响评估提供支持。
