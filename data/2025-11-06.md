...
### [1] [Optimal Boundary Control of Diffusion on Graphs via Linear Programming](https://arxiv.org/abs/2511.03129)
*Harbir Antil, Rainald Löhner, Felipe Pérez*

Main category: math.OC

TL;DR: 本文提出了一种基于线性规划 (LP) 的框架，用于优化几何网络上的稳态扩散和通量，通过控制边界势来驱动内部通量，并满足物理上合理的约束。


<details>
  <summary>Details</summary>
Motivation: 许多规划、运输和扩散过程可以被建模为几何网络上的稳态边界控制问题，目标是优化边界势以实现所需的稳态配置，但传统的 PDE 约束优化问题复杂。

Method: 该框架利用线性规划 (LP) 求解，在加权、定向图上离散化扩散过程，通过边界势作为控制变量，并施加基于梯度界限的边缘通量上限和方向约束，最终得到一个有限维的 LP 问题。

Result: 该 LP 框架能够产生稳定且物理上一致的通量场，满足符号和守恒约束，并且在两个大型示例（体育场和城市街道网络）中验证了其有效性。

Conclusion: 该方法为解决扩散控制问题提供了一种通用且高效的工具，可推广到其他扩散控制现象，如热传输、化学扩散或行人流动，并与经典线性规划理论相结合。
...
TL;DR: 本文精确分析了随机特征模型中 Sobolev 训练的泛化误差，揭示了过参数化程度对训练方法选择的影响，并发现了在特定条件下模型通过插值噪声数据实现最优性能的情况。


<details>
  <summary>Details</summary>
Motivation: 现有理论对 Sobolev 训练（同时使用函数和梯度数据进行回归）在高度过参数化预测模型中的泛化误差影响知之甚少，尤其是在高维空间中。

Method: 结合统计物理中的复制方法和算子值自由概率理论中的线性化，研究人员推导了训练后的随机特征模型泛化误差的闭合形式描述，并使用 Monte Carlo 模拟验证了理论结果。

Result: 研究表明，对于单索引模型，补充梯度数据不一定能普遍提高预测性能，而是取决于过参数化程度。在特定条件下，模型通过插值噪声函数和梯度数据可以实现最优性能。

Conclusion: 该研究为理解和优化 Sobolev 训练提供了理论基础，揭示了模型泛化行为与过参数化程度、噪声水平以及正则化强度之间的复杂关系，并为基于梯度信息的训练方法提供了新的视角。
...
### [3] [Quantifying Weighted Morphological Content of Large-Scale Structures via Simulation-Based Inference](https://arxiv.org/abs/2511.03636)
*M. H. Jalali Kanafi, S. M. S. Movahed*

Main category: astro-ph.CO

TL;DR: 本文利用模拟数据和神经网络后验估计方法，比较了闵可夫斯基函数（MFs）和条件导数矩（CMD）两种高阶统计量对宇宙学参数的约束能力，发现CMD在捕捉各向异性特征方面优于MFs。


<details>
  <summary>Details</summary>
Motivation: 传统宇宙学推断方法依赖于低阶统计量，难以捕捉非高斯和非线性宇宙大尺度结构的信息。随着新一代巡天数据的涌现，需要更先进的统计工具和推断框架来充分利用这些数据。

Method: 研究人员利用Big Sobol Sequence (BSQ)模拟的晕目录，在z=0.5的红移下，采用无偏度似然推断框架（通过神经网络后验估计）对MFs和CMD的约束能力进行模拟预测分析。研究考察了不同的宇宙学参数值和平滑尺度。

Result: 结果表明，在给定的宇宙学参数和平滑尺度下，CMD对(Ωm, σ8)的约束精度优于MFs的各个阶数（分别提高了44%、52%、30%、45%、27%、17%和26%、17%）。MFs和CMD的联合使用进一步提高了精度。

Conclusion: CMD能够捕捉到MFs无法捕捉到的各向异性信息，表明CMD在宇宙学参数推断中具有潜在的应用价值。研究结果强调了发展能够有效利用非高斯信号的统计工具的重要性。
...
### [4] [Digitizing Spermatogenesis Lineage at Nanoscale Resolution In Tissue-Level Electron Microscopy](https://arxiv.org/abs/2511.02860)
*Li Xiao, Liqing Liu, Hongjun Wu, Jiayi Zhong, Yan Zhang, Junjie Hu, Sun Fei, Ge Yang, Tao Xu*

Main category: physics.bio-ph

TL;DR: 本文开发了DeepOrganelle工具，用于在组织层面进行纳米级分辨率的细胞器官定位和定量分析，揭示了精子发生过程中细胞器动态变化和相互作用。


<details>
  <summary>Details</summary>
Motivation: 现有“数字细胞”构建主要依赖单细胞和空间多组学技术，缺乏在组织层面进行细胞器网络与生理病理环境关联的系统性研究，亟需一种能够实现跨尺度细胞器分析的工具。

Method: DeepOrganelle基于轻量级的Mask2Former框架，实现了对不同细胞类型中细胞器的分割、提取、统计定量分析，以及在组织尺度上可视化和量化细胞器形态和相互作用的空间分布。该工具被应用于精子发生过程中的膜接触位点（MCSs）动态分析。

Result: DeepOrganelle系统性地量化了精子发生过程中12个发育阶段和24种状态的膜接触位点（MCSs）动态变化，揭示了细胞器类型及其相互作用差异化的时空梯度。特别发现了线粒体（Mito）-内质网（ER）接触的波浪状模式，并在X期紧缩期前显著增加，与维持双链断裂（DSB）修复所需的ATP供应有关。此外，还观察到血睾屏障的动态重塑以及精子发生I期前期血清雄激素结合蛋白细胞的细胞器地形阶段特异性重组。

Conclusion: DeepOrganelle工具为构建细胞图谱、理解细胞器功能和相互作用、以及研究组织发育和疾病机制提供了新的方法和视角，有望推动生物学和医学研究的进展。
