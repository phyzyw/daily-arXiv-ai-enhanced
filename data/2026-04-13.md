...
### [1] [EquiformerV3: Scaling Efficient, Expressive, and General SE(3)-Equivariant Graph Attention Transformers](https://arxiv.org/abs/2604.09130)
*Yi-Lun Liao, Alexander J. Hoffman, Sabrina C. Shen, Alexandre Duval, Sam Walton Norwood, Tess Smidt*

Main category: cs.LG

TL;DR: 本文提出了EquiformerV3，一种效率更高、表达能力更强、更具泛化性的SE(3)-equivariant图注意力Transformer模型。它在多个材料科学基准测试中取得了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 现有SE(3)-equivariant图神经网络在效率、表达能力和泛化性方面仍有提升空间，尤其是在大规模应用中。本文旨在解决这些问题。

Method: EquiformerV3在EquiformerV2的基础上进行了优化，包括软件实现优化（加速1.75倍）、引入了等ivariant合并层归一化、改进的feedforward网络超参数、平滑半径截断注意力机制，以及SwiGLU-S2激活函数以更好地模拟多体相互作用。

Result: EquiformerV3在OC20、OMat24和Matbench Discovery等基准测试中取得了最先进的结果，尤其是在需要能量守恒模拟和PES高阶导数的情况下表现出色。

Conclusion: EquiformerV3的改进使其能够更准确地建模平滑变化的势能表面，为分子动力学、催化剂设计和材料发现等领域提供了强大的工具。
...
### [2] [How does Chain of Thought decompose complex tasks?](https://arxiv.org/abs/2604.08872)
*Amrut Nadgir, Vijay Balasubramanian, Pratik Chaudhari*

Main category: cs.LG

TL;DR: 本文研究了Chain of Thought (CoT) 如何分解复杂任务，发现CoT的有效性与任务分解的树状结构深度有关，存在一个最佳深度，超过该深度反而会降低性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究对CoT的有效性存在争议，一些研究表明过长的推理路径会降低性能，而另一些研究则表明长推理路径可以提高性能。本文旨在解释这些观察，并提供CoT何时以及为何有效，以及机器应该进行多少推理的准则。

Method: 本文将语言任务建模为分类问题，并证明了在标准监督学习分类问题中，错误概率随类别数量呈幂律增长。然后，通过将任务分解为一系列更小的子问题来减少错误，并分析了子问题的最佳“度”（即类别数量）对整体性能的影响。

Result: 研究发现，存在一个临界度，低于该度时CoT会降低性能，高于该度时CoT可以减少错误，但存在一个最佳深度，超过该深度无法进一步降低错误。最佳深度与输入域的内在维度有关。

Conclusion: 本文通过将推理形式化为大型分类任务的分解，为CoT提供了一个简单的解释，并解释了最佳推理长度和推理轨迹结构的重要性等经验观察结果。
...
### [3] [Geometry-Induced Long-Range Correlations in Recurrent Neural Network Quantum States](https://arxiv.org/abs/2604.08661)
*Asif Bin Ayub, Amine Mohamed Aboussalah, Mohamed Hibat-Allah*

Main category: quant-ph

TL;DR: 本文提出了一种新的神经网络量子态（NQS）架构——扩张循环神经网络（dilated RNN），通过扩张连接引入长程依赖关系，同时保持较低的计算复杂度，有效解决了传统RNN在处理长程相关性量子态时的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统RNN架构偏向于有限长度相关性，难以有效逼近具有长程依赖关系的量子态。虽然Transformer架构可以解决这个问题，但计算和内存开销较大。因此，需要一种既能引入长程依赖关系，又能保持计算效率的解决方案。

Method: 通过在RNN中引入扩张连接，允许循环单元直接访问远处站点的信息，从而显式地编码长程依赖关系。研究人员分析了扩张如何改变相关性几何，并在线性化和微扰设置下展示了扩张可以诱导幂律相关性。此外，他们使用扩张RNN数值模拟了横向磁场伊辛模型和一维簇态。

Result: 分析表明扩张改变了相关性几何，可以诱导幂律相关性。数值结果表明，扩张RNN能够重现伊辛模型的幂律相关性，并准确逼近具有长程条件相关性的簇态，而传统RNN则表现出指数衰减。

Conclusion: 扩张RNN提供了一种简单有效的几何机制，用于构建对相关性敏感的自回归神经网络量子态，为解决量子多体系统中的长程相关性问题提供了一种有前景的方案。
...
TL;DR: 本文提出了CrossAbSense框架，通过不同结构的神经网络预测抗体设计的开发可行性，显著提高了预测准确率，并揭示了不同生物特性对链间相互作用的不同依赖程度。


<details>
  <summary>Details</summary>
Motivation: 抗体设计生成大量候选序列，但实验验证成本高昂。需要快速、可靠的计算方法（开发可行性预言）来筛选候选序列，解决计算设计与治疗现实之间的瓶颈。

Method: CrossAbSense框架使用特定属性的神经网络预言，结合冻结的蛋白质语言模型编码器和可配置的注意力解码器。通过系统性地搜索超参数，确定最适合预测每种属性的架构，包括仅自注意力、自+交叉注意力以及双向交叉注意力三种策略。

Result: 在GDPa1基准测试中，CrossAbSense在三种开发可行性测定中优于现有基线12-20%，在另外两种中表现出竞争力。研究发现，与初始假设相反，与聚集相关的特性仅依赖于单链序列特征，而表达量和热稳定性则需要显式地对重链和轻链之间的相互作用进行建模。

Conclusion: CrossAbSense框架展示了在抗体设计中减少实验筛选成本的潜力，揭示了生物特性与序列特征和链间相互作用之间的关系，为未来抗体设计和开发提供了新的见解。
...
### [5] [Active Learning for Generalizable Detonation Performance Prediction of Energetic Materials](https://arxiv.org/abs/2604.08744)
...
TL;DR: 本文提出了一种主动学习策略，结合密度泛函理论计算、热化学建模、消息传递神经网络和贝叶斯优化，用于高效预测和发现新型含能材料，并构建了迄今为止最大的潜在CHNO炸药数据库。


<details>
  <summary>Details</summary>
Motivation: 传统含能材料的发现过程缓慢且昂贵，计算方法受限于准确材料属性的获取。为了加速新型含能材料的发现，需要一种能够高效预测性能并探索广阔化学空间的方法。

Method: 研究人员采用主动学习策略，将密度泛函理论计算、热化学建模、消息传递神经网络和贝叶斯优化相结合，构建了一个高通量工作流程。该流程迭代地扩展训练数据集，通过选择性地添加新分子，在探索化学空间和利用高性能候选分子之间取得平衡。

Result: 该方法产生了迄今为止最大的公开CHNO炸药数据库，并构建了一个具有高预测准确性（R2 > 0.98）的通用替代模型。特征重要性分析表明，氧平衡是影响性能的主要因素，其次是局部电子结构、密度和特定官能团。

Conclusion: 该研究提供的替代模型、数据库和化学见解为高通量筛选和有针对性地发现新型含能材料奠定了基础，并为未来合成研究提供了明确的方向，有望扩展含能材料的设计空间。
...
### [6] [Transferable FB-GNN-MBE Framework for Potential Energy Surfaces: Data-Adaptive Transfer Learning in Deep Learned Many-Body Expansion Theory](https://arxiv.org/abs/2604.09320)
*Siqi Chen, Zhiqiang Wang, Yili Shen, Xianqi Deng, Xi Cheng, Cheng-Wei Ju, Jun Yi, Guo Ling, Dieaa Alhmoud, Hui Guan, Zhou Lin*

Main category: physics.chem-ph

TL;DR: 本文提出了一种名为FB-GNN-MBE的框架，将图神经网络（GNN）与多体展开理论（MBE）相结合，用于高效准确地预测复杂化学体系的势能面，并实现了跨体系的迁移学习。


<details>
  <summary>Details</summary>
Motivation: 传统的量子力学计算在模拟大型化学体系时计算量过大，而经典力场缺乏化学精度。因此，需要一种既能保证精度又能有效降低计算成本的方法来预测势能面。

Method: FB-GNN-MBE将体系分解为碎片，利用量子力学模型计算碎片的一体能，并使用图神经网络学习碎片间的相互作用关系。为了实现知识迁移，采用了师生学习协议，将一个在混合密度水团簇上训练的“教师”GNN的知识迁移到轻量级的“学生”GNN，后者在均匀密度水团簇上进行微调。

Result: FB-GNN-MBE在水、酚和混合基准测试中实现了化学级别的二体和三体能预测精度，并在水和酚二聚体的解离曲线预测中表现出色。迁移学习策略使得模型能够高效准确地预测不同大小水团簇的二体和三体能，无需重新训练。

Conclusion: FB-GNN-MBE框架优于传统的非GNN模型，具有高实用性，为大规模分子模拟提供了新的解决方案，有望加速复杂化学体系的理解和设计。
