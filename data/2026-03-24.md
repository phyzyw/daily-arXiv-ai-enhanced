...
### [1] [Characterizing High-Capacity Janus Aminobenzene-Graphene Anode for Sodium-Ion Batteries with Machine Learning](https://arxiv.org/abs/2603.22254)
*Claudia Islas-Vargas, L. Ricardo Montoya, Carlos A. Vital-José, Oliver T. Unke, Klaus-Robert Müller, Huziel E. Sauceda*

Main category: cond-mat.mtrl-sci

TL;DR: 本文利用机器学习力场（MLFF）和密度泛函理论计算，研究了氨基苯官能化Janus石墨烯（Na xAB）在钠离子电池中的储钠机制，并发现其具有高容量和快速离子扩散能力。


<details>
  <summary>Details</summary>
Motivation: 传统钠离子电池电极材料存在容量低、电压高、离子传输慢等问题。研究旨在寻找一种结构明确、高容量的钠离子电池电极材料，并探索机器学习力场在材料表征中的应用。

Method: 使用SpookyNet机器学习力场（MLFF）结合密度泛函理论计算，模拟了氨基苯官能化Janus石墨烯（Na xAB）在室温下的钠离子存储过程，分析了不同充放电状态下的存储机制。

Result: 研究发现Na xAB的钠离子存储机制包括氨基苯基位点吸附、Na n@AB m结构形成和层间填充三个阶段，具有~400 mAh g−1的容量、可忽略的体积变化和~10−6cm2s−1的钠离子扩散率，后者比硬碳高出两个到三个数量级。

Conclusion: 研究表明Janus氨基苯-石墨烯是一种有前景的高容量钠离子电池电极材料，并验证了基于MLFF的模拟在表征电极材料方面的有效性。
...
TL;DR: 本文提出了一种名为Nexerra𝑅1的化学语言模型，用于逆向设计具有目标性能的网状材料，特别是金属有机框架（MOFs），通过生成有机连接体实现从计算设计到可合成框架的直接转化。


<details>
  <summary>Details</summary>
Motivation: 传统的网状材料设计依赖于直觉和迭代实验，导致探索的化学空间有限，难以系统地发现具有特定性能的框架。现有计算方法仍然受限于已知的结构或组合组装的假设框架，难以指导真正新材料的发现。

Method: Nexerra𝑅1模型在分子构建块层面运行，支持无约束低连接连接体生成和受支架约束的对称多齿基序设计。结合流引导向分布目标，在保持化学有效性和组装可行性的同时，引导生成过程向应用相关目标靠拢。生成的连接体被组装成三维框架并进行结构优化，以产生与实验合成兼容的候选材料。

Result: 通过Nexerra𝑅1，研究人员成功重现了已知的MOFs，并提出了先前未报道的框架CU-525的实验合成方案，该方案完全在计算机上生成。

Conclusion: 这项研究建立了一种通用的网状材料逆向设计范例，可控的化学语言建模能够实现从计算设计到可合成框架的直接转化，为网状材料的快速发现和应用提供了新的途径。
...
### [3] [A Novel Method for Enforcing Exactly Dirichlet, Neumann and Robin Conditions on Curved Domain Boundaries for Physics Informed Machine Learning](https://arxiv.org/abs/2603.21909)
*Suchuan Dong, Yuchuan Zhang*

Main category: math.NA

TL;DR: 本文提出了一种系统性的方法，用于精确施加狄利克雷、诺伊曼和罗宾边界条件到具有任意曲率边界的通用四边形域上，并将其与极值学习机（ELM）技术相结合，用于科学机器学习。


<details>
  <summary>Details</summary>
Motivation: 在科学机器学习中，精确施加边界条件是关键技术挑战。传统的神经网络方法通常不进行插值，难以自动施加边界条件，尤其是在复杂几何形状的曲率边界上。

Method: 该方法基于将通用四边形域映射到标准域的精确映射，结合了理论函数连接（TFC）约束表达式和无穷次内插。针对诺伊曼或罗宾边界的兼容性约束，提出了处理不同情况（诺伊曼/罗宾边界与狄利克雷边界相交，以及两个诺伊曼/罗宾边界相互相交）的四步程序。

Result: 数值实验表明，该方法能够精确施加狄利克雷、诺伊曼和罗宾边界条件，边界条件误差达到机器精度，适用于各种具有复杂边界几何形状的二维域。

Conclusion: 该方法为科学机器学习提供了精确施加边界条件的新途径，尤其适用于复杂几何形状的物理问题求解，并与ELM技术相结合，进一步提升了科学计算的效率和精度。
...
### [4] [Suiren-1.0 Technical Report: A Family of Molecular Foundation Models](https://arxiv.org/abs/2603.21942)
*Junyi An, Xinyu Lu, Yun-Fei Shi, Li-Cheng Xu, Nannan Zhang, Chao Qu, Yuan Qi, Fenglei Cao*

Main category: physics.chem-ph

TL;DR: 本文提出了Suiren-1.0，一个用于准确建模各种有机系统的分子基础模型家族。该模型通过结合3D构象几何和2D统计系综空间，并在不同任务上取得了领先的性能。


<details>
  <summary>Details</summary>
Motivation: 现有分子模型在通用建模方面受限于科学复杂性和高质量监督数据的缺乏。Suiren-1.0旨在克服这些挑战，弥合微观结构与宏观可观测性之间的多尺度差距，并利用物理先验。

Method: Suiren-1.0包含三个变体（Suiren-Base, Suiren-Dimer, Suiren-ConfAvg）。Suiren-Base在70M的DFT数据集上进行预训练，Suiren-Dimer在13.5M的分子间相互作用样本上进行继续预训练。为了便于下游应用，提出了Conformation Compression Distillation (CCD)框架，将复杂的3D结构表示压缩成2D构象平均表示，从而得到轻量级的Suiren-ConfAvg。

Result: Suiren-1.0在各种任务上实现了最先进的结果，包括量子性质预测、分子间相互作用预测等。Suiren-ConfAvg能够从SMILES或分子图中生成高保真度的表示。

Conclusion: Suiren-1.0的发布为分子建模领域带来了新的突破，为后续研究提供了强大的工具和基准。所有模型和基准均已开源，促进了该领域的进一步发展。
...
### [5] [SPINONet: Scalable Spiking Physics-informed Neural Operator for Computational Mechanics Applications](https://arxiv.org/abs/2603.21674)
*Shailesh Garg, Luis Mandl, Somdatta Goswami, Souvik Chakraborty*

Main category: physics.comp-ph

TL;DR: 本文提出了SPINONET，一种受神经科学启发的、可分离的物理信息神经网络算子，旨在提高计算力学中物理信息算子学习模型的能源效率，尤其是在边缘设备等资源受限的环境中。


<details>
  <summary>Details</summary>
Motivation: 传统的物理信息算子学习模型，如DeepONet，在边缘设备等资源受限的环境中，由于重复的算子评估导致了巨大的计算和能源成本，因此需要更节能的解决方案。

Method: SPINONET通过架构感知的神经科学启发式设计，将易于回归的脉冲神经元整合到网络中，实现了稀疏、事件驱动的计算。该方法与物理信息训练兼容，并利用可分离的DeepONet框架，通过坐标分解和前向自动微分来提高效率。

Result: SPINONET在各种计算力学问题（包括时变和稳态问题）上进行了评估，其预测性能与传统的物理信息算子学习方法相当，即使在数据监督有限的情况下，也能在复杂场景中避免收敛到虚假解。

Conclusion: SPINONET通过分析其架构组件和设计选择，证明了其在降低计算负载和能源消耗方面的有效性，为在资源受限环境中部署物理信息算子学习模型提供了一种有前景的解决方案。
...
### [6] [Closed-form conditional diffusion models for data assimilation](https://arxiv.org/abs/2603.21291)
*Brianna Binder, Assad Oberai*

Main category: stat.ML

TL;DR: 本文提出了一种基于闭式条件扩散模型的自适应数据同化方法，利用核密度估计建模状态和观测的联合分布，无需显式了解系统和测量过程。


<details>
  <summary>Details</summary>
Motivation: 传统数据同化方法，如EnKF和粒子滤波器，在非线性、非高斯系统中存在精度和可扩展性问题。本文旨在利用扩散模型的优势，提供一种更准确、更灵活的数据同化方法。

Method: 该方法利用条件扩散模型，通过核密度估计（Kernel Density Estimation, KDE）来建模状态和观测的联合分布，从而高效地评估扩散模型的得分函数。这种方法允许在黑盒设置下进行数据同化，即无需对系统和测量过程有明确的了解。

Result: 在基于Lorenz-63和Lorenz-96系统的非线性数据同化问题中，该方法在小到中等集成大小下优于传统的EnKF和粒子滤波器。

Conclusion: 提出的方法结合了条件扩散模型的强大能力和黑盒适应性，为解决非线性、非高斯系统的数据同化问题提供了一种有前景的替代方案，有望在多个科学和工程领域得到应用。
...
### [7] [Interpreting the Synchronization Gap: The Hidden Mechanism Inside Diffusion Transformers](https://arxiv.org/abs/2603.20987)
*Emil Albrychiewicz, Andrés Franco Valiente, Li-Ching Chen, Viola Zixin Zhao*

Main category: cs.LG

TL;DR: 本文研究了扩散Transformer（DiTs）中同步间隙的机制，揭示了其作为一种内在架构特性存在，并与层深度和耦合强度密切相关。


<details>
  <summary>Details</summary>
Motivation: 尽管扩散模型在生成任务中取得了巨大成功，但其内部如何解决生成歧义的机制尚不清楚。现有理论预测存在同步间隙，但如何在实际的深度离散架构中体现尚不明确。

Method: 研究人员构建了一种通过嵌入生成轨迹并使用对称交叉注意力门控制耦合强度的DiT架构，并通过线性化分析和实验验证了注意力差异的分解。他们跟踪了每层的承诺时间和内部模式能量。

Result: 研究结果表明：(1) 同步间隙是DiTs的内在架构特性；(2) 强耦合时间隙完全消失；(3) 间隙严格局限于Transformer的最终层；(4) 全局低频结构在局部高频细节之前承诺。

Conclusion: 本文为理解DiTs如何解决生成歧义提供了机制解释，将物种化过渡定位到网络的终端层，为扩散模型的解释性研究提供了新的视角。
