...
### [1] [A Structure-Preserving Graph Neural Solver for Parametric Hyperbolic Conservation Laws](https://arxiv.org/abs/2604.15617)
*Jiamin Jiang, Shanglin Lv, Jingrun Chen*

Main category: physics.comp-ph

TL;DR: 本文提出了一种结构保持图神经网络求解器，用于解决参数化双曲守恒律问题，该方法结合了经典数值原理和图神经网络，实现了高精度、稳定性和计算效率。


<details>
  <summary>Details</summary>
Motivation: 传统数值方法计算成本高昂，难以应用于大规模参数分析和设计优化；现有的神经网络代理模型虽然推理速度快，但往往无法保证物理结构的正确性，导致非物理现象和不稳定。

Method: 该方法设计了一个学习的重建和通量算子，而不是黑盒状态更新器，从而内在保留了局部守恒和迎风特性。进一步，将消息传递图神经网络重新表述为高阶时空预测器，实现了大时间步长的保守和稳定神经网络更新，灵感来源于高阶导数推导方案。

Result: 在具有广泛参数变化的超音速流动基准测试中，该神经网络求解器在长时间范围内的稳定性和准确性上优于强代理基线，优于低阶离散化，并实现了比高分辨率模拟几个数量级的运行时加速。

Conclusion: 该研究提供了一种可解释且结构保持的图神经网络求解器，有效结合了经典数值方法和深度学习，为解决双曲守恒律问题提供了一种高效且可靠的替代方案，具有广泛的应用前景。
...
### [2] [Discovering quantum phenomena with Interpretable Machine Learning](https://arxiv.org/abs/2604.16015)
*Paulin de Schoulepnikoff, Hendrik Poulsen Nautrup, Hans J. Briegel, Gorka Muñoz-Gil*

Main category: quant-ph

TL;DR: 本文提出了一种结合可解释机器学习和符号方法的框架，用于从复杂的量子数据中自动发现物理规律，并揭示了之前未报道的现象。


<details>
  <summary>Details</summary>
Motivation: 现有机器学习方法在处理更通用的量子数据（如经典阴影、混合离散/连续数据）方面存在局限性，且难以自主发现新的物理现象，需要更具可解释性的方法。

Method: 该框架结合了变分自编码器（VAE）和符号回归技术。VAE用于从原始数据中学习有意义的潜在表示，符号回归则用于提取定义学习到的簇的紧凑解析表达式，从而揭示相结构。

Result: 该框架成功应用于实验性 Rydberg 原子快照、cluster Ising 模型的经典阴影以及混合离散-连续费米子数据，揭示了 Rydberg 阵列中的角序模式以及 Ising 模型中涌现的代数行为。

Conclusion: 该研究建立了一个通用的、可解释的框架，用于从各种量子数据集中自动发现物理定律，为从原始量子数据中提取有效理论开辟了道路，并提供了自动化发现有效描述和新现象的潜力。
...
TL;DR: PINNACLE是一个开源计算框架，用于经典和量子物理信息神经网络（PINNs），集成了现代训练策略、多GPU加速和混合量子-经典架构，旨在评估PINN性能并指导未来发展。


<details>
  <summary>Details</summary>
Motivation: 现有PINN框架缺乏统一的模块化工作流程，难以系统地评估不同架构和训练方法对性能的影响。此外，量子PINN的潜力尚未得到充分探索。

Method: PINNACLE框架支持多种增强技术，包括傅里叶特征嵌入、随机权重因子化、严格边界条件施加、自适应损失平衡、课程学习和二阶优化策略。该框架还扩展到混合量子-经典PINN，并对电路评估复杂度进行形式化估计，并支持多GPU并行计算。

Result: 研究量化了这些方法对收敛性、准确性和计算成本的影响，分析了分布式数据并行扩展的效率。结果表明PINN对架构和训练选择敏感，计算成本相对较高，但在某些情况下混合量子模型可以提高参数效率。

Conclusion: PINNACLE为基准测试物理信息学习方法提供了基础，通过对各种方法的权衡进行定量评估，指导未来PINN研究和开发。
...
### [4] [Evaluating the Progression of Large Language Model Capabilities for Small-Molecule Drug Design](https://arxiv.org/abs/2604.16279)
...
TL;DR: 本文评估了大型语言模型（LLMs）在小分子药物设计方面的能力进展，并展示了通过精心设计的评估任务和有针对性的后训练，可以显著提升LLMs在化学任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs在药物设计领域潜力巨大，但缺乏能够反映真实场景的基准测试，阻碍了其实用性。本文旨在评估现有LLMs的能力，并预测未来LLMs的发展趋势，为构建药物发现代理提供指导。

Method: 研究人员设计了一套化学相关的任务（分子性质预测、分子表示转换、分子设计），并将这些任务构建成强化学习（RL）环境。他们对三种模型家族进行了评估，并采用RL后训练方法提升模型性能。

Result: 研究发现，前沿模型在化学任务上的能力不断提高，但仍有改进空间，尤其是在数据量有限的实验设置中。RL后训练显著提升了模型性能，较小模型在后训练后可与领先模型相媲美。

Conclusion: 本文表明，通过结合精心设计的评估任务和有针对性的后训练，可以有效阐明并弥补LLMs在药物发现中的关键能力差距，为LLMs在药物设计中的实际应用提供了可行途径。
...
### [5] [Tabular foundation models for in-context prediction of molecular properties](https://arxiv.org/abs/2604.16123)
*Karim K. Ben Hicham, Jan G. Rittig, Martin Grohe, Alexander Mitsos*

Main category: cs.LG

TL;DR: 本文评估了表格基础模型（TFMs）在分子性质预测中的应用，证明了其在小到中等数据量下，无需任务特定训练即可实现高性能，并降低了计算成本。


<details>
  <summary>Details</summary>
Motivation: 传统的分子基础模型通常需要任务特定微调，且往往难以超越经典基线模型。本文旨在探索一种无需微调、更高效的分子性质预测方法，以应对小数据集的挑战。

Method: 研究人员将表格基础模型（TFMs）应用于分子性质预测，使用预训练的分子基础模型嵌入、经典描述符和指纹作为输入。他们评估了TFMs在标准化的制药基准和化学工程数据集上的性能，并比较了与微调方法的计算成本。

Result: 实验结果表明，TFMs在低到中等数据量下表现出优异的预测性能，计算成本显著降低。结合CheMeleon嵌入的TFMs在MoleculeACE任务中实现了高达100%的胜率，而RDKit2d和Mordred描述符也提供了强大的替代方案。

Conclusion: 本文的研究表明，使用TFMs进行上下文学习为分子性质预测提供了一种准确且经济高效的替代方案，尤其适用于实际应用中的小数据集场景，降低了对机器学习专业知识和计算预算的需求。
...
### [6] [ChemGraph-XANES: An Agentic Framework for XANES Simulation and Analysis](https://arxiv.org/abs/2604.16205)
*Vitor F. Grizzi, Thang Duc Pham, Luke N. Pretzie, Jiayi Xu, Murat Keceli, Cong Liu*

Main category: cond-mat.mtrl-sci

TL;DR: ChemGraph-XANES是一个代理框架，旨在简化XANES模拟和分析流程，通过自然语言任务规范、结构获取、参数选择和并行执行，实现XANES计算的可扩展性和自动化。


<details>
  <summary>Details</summary>
Motivation: 传统的XANES计算流程复杂，难以重复使用和扩展，尤其是在需要大量计算的场景下，例如高通量筛选和机器学习数据集生成。该研究旨在解决这一瓶颈，提高XANES计算的效率和可访问性。

Method: ChemGraph-XANES基于ASE、FDMNES、Parsl和LangGraph/LangChain构建，将XANES工作流程操作暴露为Python工具，并由大型语言模型（LLM）代理进行编排。框架支持文档检索增强的参数选择和自然语言请求，并利用任务并行性在高性能计算（HPC）系统上进行部署。

Result: 演示了基于文档的参数检索，证明了框架既能处理显式结构文件输入，也能处理化学级别的自然语言请求。通过任务并行执行，实现了XANES计算的可扩展性。

Conclusion: ChemGraph-XANES提供了一个可重复、可扩展的工作流程层，用于基于物理的XANES模拟、光谱整理和与代理兼容的计算光谱学，为下游分析和机器学习应用提供支持。
...
### [7] [Phase Transitions as the Breakdown of Statistical Indistinguishability](https://arxiv.org/abs/2604.15773)
*Taiyo Narita, Hideyuki Miyahara*

Main category: cond-mat.stat-mech

TL;DR: 本文提出了一种基于统计不可区分性的新颖相变表征方法，将相变定义为在参数微扰消失时统计不可区分性的崩溃，无需预先知道序参量。


<details>
  <summary>Details</summary>
Motivation: 传统相变表征依赖于序参量，但在自旋玻璃和拓扑相变等情况下难以构建。数据驱动方法依赖于训练过程，存在模型偏差和有限数据的不确定性。因此，需要一个系统、模型无关的相变识别框架。

Method: 作者将相变表述为假设检验，通过检验在参数分离消失时，两个概率分布是否仍然可区分。他们使用无分布的二样本游程检验，并将其应用于二维伊辛模型。

Result: 使用二样本游程检验，准确地识别了二维伊辛模型的临界点，无需预先知道序参量。Binder参数可以被解释为一种特定的假设检验形式。

Conclusion: 该研究表明相变与局部渐近理论之间存在深刻的联系，提供了一个通用、无序参量且模型无关的相变框架，并统一了传统方法。
