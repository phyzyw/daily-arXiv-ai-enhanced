...
### [1] [Neural Uncertainty Principle: A Unified View of Adversarial Fragility and LLM Hallucination](https://arxiv.org/abs/2603.19562)
*Dong-Xiao Zhang, Hu Lou, Jun-Jie Zhang, Jun Zhu, Deyu Meng*

Main category: cs.LG

TL;DR: 本文提出了神经网络不确定性原理（NUP），揭示了视觉模型中的对抗脆弱性和大型语言模型中的幻觉现象具有共同的几何根源，即输入和损失梯度的共轭可观测量受到不确定性约束。


<details>
  <summary>Details</summary>
Motivation: 现有方法针对视觉模型中的对抗脆弱性和大型语言模型中的幻觉分别采取了模态特定的解决方案，缺乏统一的视角。本文旨在提出一个统一的框架来诊断和缓解这些边界异常。

Method: 研究人员通过形式化神经网络不确定性原理（NUP），分析了输入-梯度相关性，并设计了单向探针来捕捉这种相关性。基于NUP理论，提出了ConjMask（高贡献输入组件掩码）和LogitReg（logit侧正则化）方法，并利用探针作为解码自由的风险信号。

Result: 研究发现，对抗脆弱性和幻觉都源于模型进入边界应力状态，输入和损失梯度之间存在不确定性约束。通过探针可以检测幻觉风险，掩码高耦合组件可以提高视觉模型的鲁棒性，而无需进行昂贵的对抗训练。

Conclusion: NUP将两种看似独立的失效分类转化为共享的不确定性预算视图，提供了一个统一的、实用的框架来诊断和缓解感知和生成任务中的边界异常，为模型可靠性分析提供了新的视角。
...
### [2] [Conditioning Protein Generation via Hopfield Pattern Multiplicity](https://arxiv.org/abs/2603.20115)
*Jeffrey D. Varner*

Main category: cs.LG

TL;DR: 本文提出了一种无需重新训练即可将蛋白质序列生成引导至特定功能子集的简单方法，通过调整采样器注意力的偏置来实现，解决了现有方法在数据稀缺时难以针对特定功能进行调控的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的蛋白质序列生成方法，如深度生成模型和蛋白质语言模型，通常需要大量训练数据和计算资源。即使是Stochastic Attention (SA) 这种无需训练的方法，也无法将生成过程引导至特定的功能子集，无法满足实验人员对特定功能的定向需求。

Method: 该方法通过在采样器的注意力logits中添加一个标量参数作为偏置，从而连续地将生成过程从整个家族转移到用户指定的子集。用户提供少量序列（例如，结合筛选的命中序列）和一个多重性比率，用于控制生成过程对这些序列的偏好。该方法对子集代表的性质（如结合、稳定性、特异性等）不敏感。

Result: 在五个Pfam家族（Kunitz, SH3, WW, Homeobox, 和Forkhead domains）的实验中，证实了分离度和间隙之间的单调关系。应用于omega-conotoxin肽，从23个已表征的结合剂中进行精心挑选，产生了超过一千个候选物，保留了主要药效团和所有实验鉴定出的结合决定因素。

Conclusion: 该研究表明，Stochastic Attention 能够使实践者在无需重新训练生成模型的情况下，将少量实验表征的序列扩展为多样化的候选库，为蛋白质工程提供了一种有效且灵活的工具。
...
### [3] [Physics-Informed Long-Range Coulomb Correction for Machine-learning Hamiltonians](https://arxiv.org/abs/2603.20007)
*Yang Zhong, Xiwen Li, Xingao Gong, Hongjun Xiang*

Main category: physics.comp-ph

TL;DR: 本文提出了一种基于变分分解的Physics-Informed长程库仑修正方法，并将其应用于机器学习哈密顿量，有效解决了传统机器学习哈密顿量在极性晶体和异质结构中忽略长程库仑相互作用的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的机器学习哈密顿量方法通常采用近视原理，忽略了长程库仑相互作用，这在极性晶体、低维材料和异质结构等系统中导致精度下降。因此，需要一种能够准确描述长程库仑相互作用的机器学习哈密顿量方法。

Method: 研究人员通过对静电能进行变分分解，推导了非正交原子轨道基中闭合形式的长程哈密顿量矩阵元素。他们将该框架集成到HamGNN-LR中，该框架结合了E(3)-等变消息传递和倒数空间Ewald求和，实现了高效的计算。

Result: 实验结果表明，基于物理的长程修正对于获得准确的电子结构计算至关重要。在极性ZnO薄片、CdSe/ZnS异质结构和GaN/AlN超晶格等体系上的基准测试显示，该方法将误差降低了2到3倍，并且具有强大的泛化能力。

Conclusion: 该研究提供了一个严谨的框架，将长程库仑相互作用纳入机器学习电子哈密顿量，有效解决了传统方法的局限性，为计算材料科学和凝聚态物理学提供了更准确、更高效的工具。
...
### [4] [Reinforcement-guided generative protein language models enable de novo design of highly diverse AAV capsids](https://arxiv.org/abs/2603.19473)
...
TL;DR: 本文提出了一种基于蛋白质语言模型和强化学习的框架，用于设计具有高多样性和功能的AAV衣壳蛋白，从而扩展基因治疗的潜力。


<details>
  <summary>Details</summary>
Motivation: AAV载体在基因治疗中应用广泛，但需要设计改进的衣壳蛋白以提高治疗效果。传统的蛋白质工程方法受限于巨大的序列空间，因此需要计算方法来指导实验。

Method: 研究人员开发了一个基于预训练蛋白质语言模型和强化学习的生成设计框架。通过在实验验证的衣壳蛋白序列上微调模型，并使用奖励函数同时促进预测的生存能力和序列新颖性来引导序列生成。

Result: 强化学习引导的生成方法能够探索更远的序列空间，同时保持高预测的生存能力，优于仅进行微调的方法。研究人员还提出了一种候选选择策略，综合考虑预测的生存能力、序列新颖性和生物物理性质。

Conclusion: 该工作建立了一个用于生成性探索蛋白质序列空间的框架，并推动了生成蛋白质语言模型在AAV生物工程中的应用，为设计新型AAV载体提供了有力的工具。
