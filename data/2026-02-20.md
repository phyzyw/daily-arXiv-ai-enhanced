...
### [1] [Universal Fine-Grained Symmetry Inference and Enforcement for Rigorous Crystal Structure Prediction](https://arxiv.org/abs/2602.17176)
*Shi Yin, Jinming Mu, Xudong Zhu, Lixin He*

Main category: cond-mat.mtrl-sci

TL;DR: 本文提出了一种新的晶体结构预测框架，利用大型语言模型直接生成符合晶体对称性的细粒度Wyckoff模式，并通过约束优化搜索严格执行代数一致性，从而避免了对现有数据库的依赖。


<details>
  <summary>Details</summary>
Motivation: 现有晶体结构预测(CSP)方法通常将晶体对称性视为软启发式或依赖于已知结构的数据库检索，这限制了物理保真度和发现新结构的潜力。为了克服这些限制，需要一种能够严格执行对称性约束并探索更大材料空间的方法。

Method: 该框架结合了大型语言模型、约束优化搜索和扩散骨干网络。首先，大型语言模型编码化学语义并直接生成细粒度Wyckoff模式。然后，通过约束优化搜索严格执行位点倍数和原子化学计量之间的代数一致性。最后，将对称一致的模板集成到扩散骨干网络中，以约束随机生成轨迹。

Result: 该框架在稳定性、独特性和新颖性(SUN)基准测试中实现了最先进的性能，并且在匹配性能方面也优于其他方法，证明了其有效性。

Conclusion: 该研究建立了一种新的晶体学空间严格探索范例，能够高效地扩展到以前未知的材料空间，无需依赖现有数据库或先验结构知识，为材料发现提供了新的途径。
...
### [2] [Synergizing Transport-Based Generative Models and Latent Geometry for Stochastic Closure Modeling](https://arxiv.org/abs/2602.17089)
*Xinghao Dong, Huchen Yang, Jin-long Wu*

Main category: cs.LG

TL;DR: 本文提出了一种基于传输的生成模型和潜在几何的随机闭合模型方法，通过在低维潜在空间中进行流匹配，实现了比传统扩散模型快两个数量级的采样速度，同时保持物理保真度。


<details>
  <summary>Details</summary>
Motivation: 传统的湍流闭合模型通常基于确定性假设，但在某些情况下，这种假设不成立。为了解决这个问题，需要探索随机建模方法，但随机闭合模型的开发和校准面临数据量大和模型结构复杂的问题。

Method: 本文将传输性生成模型与潜在几何相结合，在低维潜在空间中进行流匹配以实现快速采样。同时，通过联合训练方案和两种显式正则化约束（度量保持和几何感知）来控制潜在空间扭曲，确保采样闭合项的物理保真度。

Result: 实验结果表明，该方法实现了比迭代扩散方法快两个数量级的采样速度，并且学习到的随机闭合模型能够继承原始复杂动力系统的拓扑信息，无需大量训练数据。

Conclusion: 本文提出的方法为快速、高效地学习随机闭合模型提供了一种新的途径，有望应用于湍流、固体力学等复杂动力系统的建模，并为科学机器学习领域提供新的思路。
