...
### [1] [Detailed balance in large language model-driven agents](https://arxiv.org/abs/2512.10047)
*Zhuo-Yang Song, Qing-Hong Cao, Ming-xing Luo, Hua Xing Zhu*

Main category: cs.LG

TL;DR: 本文研究了大型语言模型驱动的智能体（LLM-driven agents）的宏观动态，发现其生成过程存在详细平衡现象，表明LLM可能通过学习潜在函数而非规则集来工作。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM驱动的智能体在解决复杂问题方面取得了成功，但缺乏一个理论框架来理解和统一它们的宏观动态。本文旨在建立复杂AI系统的宏观动力学理论，将AI智能体研究从工程实践提升到基于可预测和可量化的有效测量。

Method: 通过实验测量LLM生成的状态之间的转移概率，并基于最小作用量原理来估计LLM的潜在生成方向性。将LLM的生成过程视为嵌入在智能体框架中的马尔可夫转移过程。

Result: 统计上发现了LLM生成的转移过程存在详细平衡现象，表明LLM的生成可能并非通过学习规则集和策略，而是通过隐式学习潜在函数。

Conclusion: 该发现是LLM生成动态中第一个宏观物理定律，不依赖于特定模型细节。这表明LLM可能通过学习潜在函数来实现更强的泛化能力，并且这种现象可能超越不同的LLM架构和提示模板。
...
### [2] [A Model-Guided Neural Network Method for the Inverse Scattering Problem](https://arxiv.org/abs/2512.10123)
*Olivia Tsang, Owen Melia, Vasileios Charisopoulos, Jeremy Hoskins, Yuehaw Khoo, Rebecca Willett*

Main category: physics.comp-ph

TL;DR: 本文提出了一种模型引导的神经网络方法来解决反散射问题，该方法将可微分的PDE求解器嵌入到神经网络框架中，利用不同频率的波测量进行迭代重建，从而提高重建质量并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 传统的机器学习方法在处理非线性反散射问题时表现不佳，因为它们难以整合散射过程的物理知识。本文旨在解决这一问题，通过将物理知识显式地融入到机器学习框架中，提高反散射问题的求解效率和准确性。

Method: 该方法的核心是将可微分的PDE求解器作为前向模型嵌入到神经网络中。它采用迭代重建策略，利用不同频率的波测量，交替计算非线性最小二乘法的梯度并应用学习到的滤波操作，类似于算法展开和学习优化框架，并借鉴了递归线性化算法的思想。

Result: 实验结果表明，该方法能够在显著降低计算和采样成本的情况下，实现高质量的散射势重建。

Conclusion: 本文提出的模型引导的神经网络方法为解决非线性反散射问题提供了一种新的有效途径，有望在医学成像、遥感和无损检测等领域得到广泛应用。
...
### [3] [Tracking large chemical reaction networks and rare events by neural networks](https://arxiv.org/abs/2512.10309)
*Jiayu Weng, Xinyi Zhu, Jing Liu, Linyuan Lü, Pan Zhang, Ying Tang*

Main category: q-bio.MN

TL;DR: 本文提出了一种基于神经网络的新方法，通过优化算法和增强采样策略，显著提高了化学反应网络模拟的效率和精度，尤其是在处理高维系统和稀有事件时。


<details>
  <summary>Details</summary>
Motivation: 传统的化学主方程求解方法在处理大型化学反应网络，特别是高维系统和稀有事件时面临计算挑战。现有的神经网络方法效率有限，难以满足实际需求。

Method: 本文利用自回归神经网络，结合自然梯度下降和时间相关变分原理进行优化，并采用增强采样策略来捕捉稀有事件。该方法被应用于处理复杂的反应网络，包括MAPK级联网络和空间扩展的反应-扩散系统。

Result: 实验结果表明，该方法在计算速度上实现了5-22倍的提升，并且在处理具有挑战性的反应网络时，精度高于之前的神经网络方法。该方法成功模拟了迄今为止最大的生物网络，并扩展到二维反应-扩散系统。

Conclusion: 本文提出的方法为高效建模化学反应网络提供了新的途径，尤其适用于处理高维系统和稀有事件，具有广泛的应用前景。
