...
### [1] [Rank Collapse, Fixed Points, and the Renormalization Group Structure of MLP Residual Networks](https://arxiv.org/abs/2606.10324)
*Parviz Haggi-Mani, Irina Rish*

Main category: cs.LG

TL;DR: 本文通过对纯 MLP 残差堆栈的经验研究，首次提供了量化的、位置级别的证据，表明 MLP 残差网络实现了选择性的粗粒化过程，其行为受输入分布的谱结构控制。


<details>
  <summary>Details</summary>
Motivation: 现有研究对深度神经网络与重整化群 (RG) 理论的类比主要停留在定性描述层面，缺乏量化分析。本文旨在超越定性描述，对这种类比进行经验研究。

Method: 研究人员使用训练在合成马尔可夫链序列上的纯 MLP 残差堆栈，这些序列具有已知的谱特性，并进行 masked token prediction。他们分析了残差流的有效秩、层间核漂移以及它们与网络深度的关系。

Result: 研究发现：(1) 残差流的有效秩随着深度单调递减；(2) 秩坍塌是选择性的，仅发生在具有短相关长度的链上；(3) 层间核漂移集中在特定的层过渡处，其余网络接近固定点。

Conclusion: 这些发现支持了 MLP 残差网络实现选择性粗粒化过程的观点，并为大型语言模型中的 RG 计划提供了新的见解。研究人员建议进行扰动衰减实验以提取 RG 缩放指数。
...
### [2] [Algorithmic and Minimax Complexities in Kernel Bandits](https://arxiv.org/abs/2606.11171)
*Yunbei Xu*

Main category: cs.LG

TL;DR: 本文将GP-UCB和DEC方法统一到一个框架下，并提出了一种新的算法，证明了在过参数化模型中，算法信息比类宽最小最大风险或DEC系数更能提供信息。


<details>
  <summary>Details</summary>
Motivation: 长期以来，GP-UCB和DEC方法被认为是不同的理论，本文旨在将它们统一起来，并探讨它们之间的区别和联系，从而更深入地理解kernel bandits中的信息理论最优性。

Method: 本文提出了MAIR框架，利用异构正定算法先验，统一了GP-UCB分析和MAMS算法，并提出了一种safeguarded master算法，结合了它们的优点。通过hub-cloud分离的构造，比较了算法的性能。

Result: 研究表明，算法信息和类宽最小最大风险系数回答了不同的问题，并且可以导致不同的差距。在kernel bandits中，这种区别变得数学上清晰可见。safeguarded master算法在结构化真值上表现良好。

Conclusion: 本文提供了一个统一的视角来理解kernel bandits中的算法信息和类宽最小最大风险，为进一步研究信息理论最优性提供了新的思路，并澄清了GP-UCB最优性讨论。
...
### [3] [LLM-Guided Neural Architecture Search for Robust Co-Design of Physical Neural Networks](https://arxiv.org/abs/2606.10294)
*Tyler King, Timothee Leleu*

Main category: cs.LG

TL;DR: 本文提出了一种名为UH-NAS的LLM引导的神经架构搜索框架，旨在针对非传统硬件进行鲁棒的架构协同设计，同时优化准确率和推理能耗。


<details>
  <summary>Details</summary>
Motivation: 现有神经架构搜索方法通常针对单一硬件家族，难以进行跨平台比较和泛化。在非传统硬件上，设备非理想特性对性能影响显著，需要更灵活的架构设计方法。

Method: UH-NAS将语言模型作为进化算子，集成到硬件感知的神经架构搜索框架中。它通过可互换的后端、能耗模型、物理约束和非理想性模拟器，实现跨异构平台的一致搜索流程，并利用LLM生成的系统提示和硬件知识库引导搜索。

Result: 在光子MZI硬件上测试，UH-NAS发现的架构比传统基线更具多样性和鲁棒性，并且优于现有的LLM到NAS方法。实验还表明，系统提示和架构与硬件协同设计对于新兴计算平台至关重要。

Conclusion: UH-NAS提供了一种硬件无关的、LLM引导的神经架构搜索方法，能够有效地解决非传统硬件上的架构协同设计问题，为新兴计算平台的设计和优化提供了新的思路。
...
### [4] [Integrating Out, Twice:The Open-System Case That Neural-Network Ensemble Theory Is Missing](https://arxiv.org/abs/2606.09950)
*Jin Lei*

Main category: cs.LG

TL;DR: 本文将神经网络集成理论与核物理中的开放系统理论联系起来，指出当前神经网络集成理论忽略了开放系统的关键特征，并尝试将核物理中的方法应用于神经网络，但结果不尽如人意。


<details>
  <summary>Details</summary>
Motivation: 神经网络集成理论通常处理的是封闭系统，而现实世界的问题往往是开放系统。本文旨在探讨神经网络集成理论中开放系统的缺失，并借鉴核物理中处理开放系统的有效方法。

Method: 作者将神经网络集成和核物理中的费舍巴克投影方法进行类比，利用高斯分布的代数和块反演，构建了封闭系统和开放系统的对应关系。随后，作者尝试将开放系统框架应用于截断的注意力图、token级别的转移算子和稀疏专家路由器等神经网络结构进行测试。

Result: 在神经网络上的开放系统测试结果主要为负面。虽然开放性确实存在，但其独特的特性要么是所选划分的伪影，要么是被训练目标固定在较低水平，或者可操作的不确定性只是表征性的，而非开放系统所应有的。

Conclusion: 本文指出，神经网络要真正实现开放系统处理，需要具有连续谱和波动动力学而非弛豫动力学的消除扇区，这表明当前主流学习方法在处理开放系统方面存在局限性。本文的主要发现是负面结果，但其价值在于指明了未来研究的方向。
...
### [5] [Flexible Kernels for Protein Property Prediction](https://arxiv.org/abs/2606.11057)
*Martin Jankowiak, Yerdos Ordabayev, Rudraksh Tuwani, Henry N. Ward, Hunter Nisonoff, James M. McFarland, Gevorg Grigoryan*

Main category: cs.LG

TL;DR: 本文提出了一种灵活的序列核函数，利用进化替代矩阵和局部线性，结合高斯过程进行蛋白质性质预测，在数据效率和多任务学习方面表现优异，并能有效整合结构信息。


<details>
  <summary>Details</summary>
Motivation: 蛋白质性质预测对于蛋白质设计至关重要，但从稀疏实验数据中准确预测仍然是一个挑战。现有方法在数据效率、不确定性估计和多任务学习方面存在不足。

Method: 本文提出了一种基于高斯过程的序列核函数，该函数利用进化替代矩阵和局部线性，并能学习结构感知的替代矩阵，从而整合结构信息。通过多任务学习，将该方法应用于多个蛋白质性质预测任务。

Result: 实验表明，提出的序列核函数在高斯过程框架下，在多个蛋白质性质预测任务中优于依赖于大型基础模型embedding的替代方法。结构条件核函数在多任务学习中表现出色，显著优于局部监督学习方法。

Conclusion: 本文提出的灵活核函数为蛋白质性质预测提供了一种数据高效、具有高质量不确定性估计且适用于多任务学习的有效方法，为蛋白质设计工具箱的构建提供了新的思路。
...
### [6] [An adaptive framework for the axisymmetric pulsar magnetosphere using physics-informed Kolmogorov-Arnold networks](https://arxiv.org/abs/2606.10686)
*Spyros Rigas, Ioannis Contopoulos, Georgios Alexandridis, Antonios Nathanail*

Main category: physics.comp-ph

TL;DR: 本文提出了一种基于Kolmogorov–Arnold网络（KAN）的自适应框架，用于求解轴对称脉冲星磁ospheres，显著提高了计算效率和精度，并能有效解决空间尺度差异问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于物理信息神经网络（PINN）的脉冲星磁ospheres求解方法存在手动调参多、精度有限、训练时间长等问题，且难以有效解决空间尺度差异。

Method: 研究人员引入了基于Kolmogorov–Arnold网络（KAN）的领域特定神经网络架构，并设计了自动化的自适应训练流程和基于物理的收敛判据，取代了手动调参。该框架将偏微分方程（PDE）及其边界条件嵌入到神经网络的损失函数中。

Result: 该方法在单精度下实现了小于20分钟的收敛时间，并将PDE残差的均方误差降低到10⁻⁶，比现有方法提高了两个数量级。同时，能够可靠地解析出减小80%的恒星半径，有效解决了空间尺度差异问题，并提供了一个连接到无穷远通量的修正方程。

Conclusion: 该研究提供了一种高效、高精度且可自适应的脉冲星磁ospheres求解框架，并将其开源为PulsarX库，为脉冲星磁ospheres的研究提供了新的工具和方法。
...
### [7] [Towards Critical Branching Mechanism in Recurrent Neural Networks](https://arxiv.org/abs/2606.10384)
*Feixiang Ren, Ling Feng*

Main category: nlin.AO

TL;DR: 本研究分析了训练好的LSTM网络中的隐藏状态动态，发现小规模网络表现出接近临界状态的特征，如无标度雪崩统计和接近于1的分支参数，而大规模模型则表现出亚临界状态。研究提出了一种混合分支过程框架来解释亚临界分支与1/fβ噪声的共存。


<details>
  <summary>Details</summary>
Motivation: 借鉴生物神经系统中的临界性概念，研究旨在探索人工神经网络（特别是LSTM网络）中临界行为的起源和相关性，以及解释其在训练过程中如何产生。

Method: 研究人员分析了训练好的LSTM网络中的隐藏状态动态，定义了雪崩事件，并分析了其统计特性，将LSTM雪崩与分支过程建立直接联系。同时，提出了混合分支过程框架。

Result: 研究发现，小规模LSTM网络在训练的特定阶段表现出接近临界状态的特征，包括无标度雪崩统计和接近于1的分支参数；而大规模模型则表现出亚临界状态。1/fβ噪声的出现与潜在的分支过程密切相关。

Conclusion: 该研究表明，LSTM网络中的临界行为是一种与容量相关的动态机制，为理解人工神经网络中的临界现象提供了一种新的视角，并为ANN的设计和优化提供了潜在的指导。
...
### [8] [A Constrained Natural-Language Interface for Variational Multi-Physics Finite Element Simulations in FEniCS](https://arxiv.org/abs/2606.10928)
*Nilay Upadhyay, Wesley F. Reinhart*

Main category: cs.CE

TL;DR: 本文提出了一种受限的自然语言接口，用于在FEniCS中进行多物理场有限元模拟。该接口将LLM限制在前端任务，避免了LLM直接生成求解器代码带来的可靠性问题。


<details>
  <summary>Details</summary>
Motivation: 传统的有限元分析工作流程繁琐，且容易出错。大型语言模型(LLM)可以简化设置有限元模拟的工作，但直接生成求解器代码可能导致可靠性问题，因此需要一种更安全的方法。

Method: 该系统将用户提示解析为结构化的JSON规范，LLM仅生成非目录几何体的Gmsh代码，并使用重试反馈进行修复。它不生成FEniCS求解器模板、弱形式或数值求解器核心，而是使用预先编写的FEniCS/UFL模板，并通过确定性映射器选择合适的模板。

Result: 在两个和三维的分析解和已发表的基准测试中，平滑情况的精度达到亚百分之一，非线性情况的精度在2-5%范围内。LLM前端的解析准确率达到100%，自定义几何形状基准测试的首次成功率和最终成功率均为90%。

Conclusion: 该研究提供了一种受限的、可靠的自然语言驱动的变分模拟架构，避免了LLM直接生成求解器代码的风险，并展示了在复杂几何形状（如L形支架）上进行多物理场模拟的能力。
...
### [9] [Non-linear mechanical field reconstruction coupling recurrent neural networks with physics-informed graph neural networks](https://arxiv.org/abs/2606.10909)
*Manuel Ricardo Guevara Garban, Yves Chemisky, Étienne Prulière, Michaël Clément, Martin Abendroth, Björn Kiefer*

Main category: cs.CE

TL;DR: 本文提出了一种结合LSTM和GNN的框架，用于高效重建非线性材料微结构中的局部应力场，显著加速了多尺度模拟，并具有良好的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 传统多尺度模拟计算成本高昂，尤其是在考虑非线性材料行为（如塑性）时。为了解决这一问题，需要一种更高效的方法来重建局部应力场。

Method: 该方法采用LSTM网络编码宏观应力-应变序列，捕捉路径相关的本构响应；并使用物理信息图神经网络（GNN）重建时空分辨率的应力场。引入了一种相对权重策略，平衡数据驱动的重建损失和基于离散散度的平衡惩罚，以解决尺度不匹配问题。

Result: 该模型在10,000条非比例加载路径上训练，实现了比有限元模拟快三个数量级的速度提升，并且能够泛化到训练长度两倍的加载序列，累积误差为1.9%。模型对网格类型和分辨率具有鲁棒性。

Conclusion: 该研究提供了一种高效且通用的局部应力场重建方法，可用于多尺度优化和设计复杂结构材料，尤其是在考虑非线性材料行为时，具有重要的工程应用价值。
...
### [10] [POPSICLE: Benchmark Datasets for Segmentation and Localization in CryoET](https://arxiv.org/abs/2606.10255)
...
TL;DR: POPSICLE是一个用于冷冻电镜（cryoET）分割和定位的基准数据集套件，旨在解决当前cryoET机器学习评估缺乏标准化和通用性问题。


<details>
  <summary>Details</summary>
Motivation: 现有的cryoET数据集规模小、任务特定且孤立，限制了机器学习方法之间可靠的比较。缺乏标准基准阻碍了cryoET领域机器学习的发展。

Method: POPSICLE从CryoET数据门户构建，包含真核和原核系统、纯化和原位样本，涵盖了密集体素级分割和稀疏定位任务。该基准数据集是一个动态资源，可以随着新数据集和注释的可用性而扩展。

Result: 基线实验表明，不同任务的模型排名存在显著差异，强调了需要针对cryoET的独特特性定制基准，而不是采用来自其他生物医学成像领域的评估方法。

Conclusion: POPSICLE为cryoET中的可重复的机器学习评估提供了一个开放和可扩展的基础，促进了该领域机器学习研究的进步，并鼓励开发能够跨任务泛化的模型。
