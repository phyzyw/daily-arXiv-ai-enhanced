...
### [1] [Information-Geometric Decomposition of Generalization Error in Unsupervised Learning](https://arxiv.org/abs/2604.12340)
*Gilhan Kim*

Main category: stat.ML

TL;DR: 本文将无监督学习中的泛化误差分解为模型误差、数据偏差和方差三个非负分量，并以ϵ-PCA为例进行分析，揭示了最优模型选择与数据偏差之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 在监督学习中存在偏差-方差权衡，但在无监督学习中缺乏类似的理论框架。本文旨在建立无监督学习泛化误差的理论分解，并找到最小化泛化误差的模型复杂度。

Method: 利用信息几何中的广义勾股定理和对偶e-混合方差恒等式，对Kullback-Leibler泛化误差进行分解。针对非e-flat模型，通过技术上的e-flat重构进行分析，并结合随机矩阵理论。

Result: 对ϵ-PCA进行了分析，发现最优秩为λ∗cut=ϵ，表明模型误差增益与数据偏差成本之间存在边际率平衡。 得到了一个包含三个阶段（保留所有、内部、崩溃）的边界比较相图，并计算了崩溃阈值ϵ ∗(α)。

Conclusion: 本文为无监督学习泛化误差提供了理论分解，揭示了模型复杂度选择的内在机制，并为相关领域的研究提供了新的视角和工具。
...
### [2] [Distinct mechanisms underlying in-context learning in transformers](https://arxiv.org/abs/2604.12151)
*Cole Gibson, Wenping Cui, Gautam Reddy*

Main category: cs.LG

TL;DR: 本文研究了Transformer模型中上下文学习的能力，揭示了其背后的四个算法阶段和两种不同的机制，并阐明了数据多样性对这些阶段的影响。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习需要参数调整来适应特定数据集，而Transformer模型展现出上下文学习能力，无需额外参数调整即可泛化到新数据。本文旨在理解这种能力背后的机制，以及数据特征如何影响其表现。

Method: 研究人员在Transformer模型上训练，使用有限的离散马尔可夫链数据集，分析了模型在不同数据多样性下的行为。他们识别了四个算法阶段，并研究了多层子电路如何实现上下文自适应计算，并构建了对称约束理论来解释训练动态。

Result: 研究发现Transformer模型展现出四个算法阶段：记忆与泛化、1点统计与2点统计的结合。这些阶段由多层子电路实现，揭示了两种不同的机制。数据多样性（K）决定了记忆和泛化之间的边界，分别由K∗1和K∗2设定。研究还识别了损失景观的关键特征，这些特征允许网络泛化。

Conclusion: 本文揭示了Transformer模型实现上下文学习的独特子电路，并阐明了数据多样性对不同机制选择的影响。研究结果有助于理解Transformer模型的泛化能力，并为未来设计更强大的、具有上下文自适应能力的模型提供指导。
...
### [3] [Fast and principled equation discovery from chaos to climate](https://arxiv.org/abs/2604.11929)
*Yuzheng Zhang, Weizhen Li, Rui Carvalho*

Main category: cs.LG

TL;DR: 本文提出了一种名为Bayesian-ARGOS的新框架，该框架结合了快速的频率学筛选和集中的贝叶斯推断，实现了在嘈杂数据中高效、自动且具有统计严谨性的方程发现。


<details>
  <summary>Details</summary>
Motivation: 现有基于库的稀疏回归方法在自动化、统计严谨性和计算效率之间存在权衡，难以同时满足这三个目标。因此，需要一种能够平衡这些需求的方程发现方法。

Method: Bayesian-ARGOS 框架结合了快速的频率学筛选和集中的贝叶斯推断，实现了自动化方程发现并对不确定性进行原则性量化。该框架还集成了表示学习（SINDy-SHRED）以处理高维数据。

Result: 在七个混沌系统上的测试表明，Bayesian-ARGOS 在大多数情况下优于两种最先进的方法，在数据效率和噪声容忍度方面优于 SINDy，并且计算成本降低了两个数量级。此外，该框架还提供了影响分析和多重共线性检测等统计诊断工具。

Conclusion: Bayesian-ARGOS 提供了一种原则性、自动化且计算高效的途径，可以将稀缺和嘈杂的观测结果转化为可解释的控制方程，适用于从基准混沌系统到全球气候模式的各种规模。
...
### [4] [XANE(3): An E(3)-Equivariant Graph Neural Network for Accurate Prediction of XANES Spectra from Atomic Structures](https://arxiv.org/abs/2604.12140)
*Vitor F. Grizzi, Luke N. Pretzie, Jiayi Xu, Cong Liu*

Main category: cs.LG

TL;DR: 本文提出了XANE(3)，一种基于E(3)-等变图神经网络，能够直接从原子结构预测XANES光谱，显著提高了预测精度和效率。


<details>
  <summary>Details</summary>
Motivation: 传统的XANES光谱计算依赖于计算密集型的第一性原理方法，这在模拟复杂材料（如纳米催化剂）的结构分布时成为瓶颈。因此，需要一种快速、准确的替代方案。

Method: XANE(3)模型结合了张量积消息传递、球谐函数边特征、吸收体查询注意力池化、自定义等变层归一化、自适应门控残差连接和基于多尺度高斯基函数的谱读出。训练过程中采用复合目标函数，包括点对点光谱重建以及一阶和二阶导数匹配。

Result: 在5941个氧化铁表面晶面FDMNES模拟数据集上，XANE(3)模型实现了0.001的均方误差，准确地重现了主边缘结构、相对峰强度、前沿特征和后沿振荡。消融研究表明，导数感知目标函数、自定义等变归一化、吸收体条件注意力池化、自适应门控残差混合和全局背景项均能提高性能。

Conclusion: XANE(3)作为XANES模拟的准确高效替代方案，为加速光谱预测、机器学习辅助光谱学和数据驱动材料发现提供了有前景的途径。
...
### [5] [TriFit: Trimodal Fusion with Protein Dynamics for Mutation Fitness Prediction](https://arxiv.org/abs/2604.12026)
*Seungik Cho*

Main category: cs.LG

TL;DR: TriFit是一种融合序列、结构和蛋白质动态的三模态框架，用于预测氨基酸突变对蛋白质功能的影响。它通过混合专家模型自适应地融合不同模态，并在ProteinGym基准测试中取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有突变效应预测方法通常忽略了蛋白质动态信息，而蛋白质动态（如残基柔韧性、相关运动和协同耦合）是影响突变耐受性的重要因素。此外，现有融合策略对所有模态一视同仁，而不同突变类型对序列、结构和动态信息的依赖程度不同。

Method: TriFit利用Masked Marginal Scoring (ESM-2)、AlphaFold2预测的Cα几何结构和高斯网络模型(GNM)提取序列、结构和动态嵌入。然后，通过一个四专家混合专家(MoE)模块融合这些嵌入，并使用三模态交叉模态对比学习进行训练，以在统一的潜在空间中对齐互补表示。

Result: 在ProteinGym基准测试上，TriFit实现了AUROC 0.897±0.0002，优于所有监督基线（包括Kermut 0.864和ProteinNPT 0.844）和零样本模型ESM3 (0.769)。动态信息贡献了最大的边际提升，并且模型输出具有良好的校准性。

Conclusion: TriFit的成功表明，将蛋白质动态信息纳入突变效应预测模型中可以显著提高预测准确性。自适应融合机制能够更好地利用不同模态的信息，为理解遗传疾病和工程化治疗蛋白质提供了新的工具。
...
### [6] [Towards grounded autonomous research: an end-to-end LLM mini research loop on published computational physics](https://arxiv.org/abs/2604.12198)
*Haonan Huang*

Main category: physics.comp-ph

TL;DR: 本文提出了一种端到端的LLM小型研究循环，用于自动执行计算物理研究，并在规模和深度两个方面进行了测试，结果表明LLM能够自主阅读、重现、批判和扩展已发表的论文，甚至能够发现并修正原始论文的结论。


<details>
  <summary>Details</summary>
Motivation: 现有LLM自主代理主要应用于机器学习研究，而真实物理科学研究需要更深层次的推理、对物理真理的约束以及对现有文献的依赖。本文旨在探索LLM是否能够自主完成计算物理领域的小型研究循环。

Method: 研究人员构建了一个小型研究循环，包括阅读论文、重现结果、批判评估和扩展研究。该循环在两个方面进行测试：规模（在111篇开放获取的计算物理论文上）和深度（针对一篇Nature Communications论文）。实验使用Claude Opus 4.6配置，并基于密度泛函理论（DFT）和Quantum ESPRESSO生态系统。

Result: 在规模测试中，LLM能够重现约四分之三的计算结果，并自主提出约42%论文的实质性方法论问题，其中97.7%需要执行才能发现。在深度测试中，LLM针对一篇论文进行了额外的计算，并生成了一篇可发表的评论，修正了原始论文的结论。

Conclusion: 本文展示了“ grounded autonomous research” 的可行性，即AI通过阅读、重现、批判和扩展已发表的工作来进行真实世界的科学研究，并强调了计算执行在科学发现中的重要性，而非仅仅依赖于文本阅读。
...
### [7] [Inverse Design of Inorganic Compounds with Generative AI](https://arxiv.org/abs/2604.11827)
*Hannes Kneiding, Lucía Morán-González, Nishamol Kuriakose, Ainara Nova, David Balcells*

Main category: physics.chem-ph

TL;DR: 本文综述了生成式人工智能在无机化合物设计中的应用，分析了其面临的挑战以及应对策略，并展望了未来的发展方向。


<details>
  <summary>Details</summary>
Motivation: 尽管机器学习在化学领域取得了显著进展，但生成式人工智能在无机化合物设计中的应用仍然有限。本文旨在分析这些挑战，并探讨如何利用生成式人工智能设计各种无机体系，包括分子、晶体、过渡金属配合物和微孔材料。

Method: 本文通过分析生成式人工智能方法如何演变为数据表示-模型流程，来解决无机化合物的复杂性，包括化学成分、几何形状、对称性和电子结构等。同时，讨论了未来发展方向，如基准标准化和可合成性指标的开发。

Result: 本文总结了生成式人工智能在过渡金属配合物和金属有机框架等不同无机体系中的应用进展，并指出了当前面临的挑战。

Conclusion: 生成式人工智能为无机化合物的设计提供了强大的工具，有望加速新材料的发现和优化。未来的研究应侧重于基准标准化和可合成性评估，以推动该领域的发展。
