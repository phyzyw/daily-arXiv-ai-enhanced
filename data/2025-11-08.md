...
### [1] [Machine Learning for Electron-Scale Turbulence Modeling in W7-X](https://arxiv.org/abs/2511.04567)
*Ionut-Gabriel Farcas, Don Lawrence Carl Agapito Fernando, Alejandro Banon Navarro, Gabriele Merlo, Frank Jenko*

Main category: physics.plasm-ph

TL;DR: 本文利用机器学习构建了针对W7-X星状器电子温度梯度(ETG)湍流的降阶模型，用于预测ETG热通量，以加速等离子体剖面预测和参数扫描等任务。


<details>
  <summary>Details</summary>
Motivation: 高保真模拟计算成本高昂，限制了其在快速剖面预测、不确定性量化、参数扫描和设计优化等任务中的应用。因此，需要高效且可靠的降阶模型。

Method: 研究人员使用回归和主动机器学习方法，基于三个等离子体参数（归一化电子温度径向梯度 ωTe、归一化电子温度和密度径向梯度之比 ηe、电子-离子温度比 τ）构建了七个径向位置的模型。通过从现有模拟数据库中选择最具信息量的点来迭代细化训练集。

Result: 模型在超过393个点的样本外数据集上表现出鲁棒的性能和可比拟于原始参考模拟的预测精度，即使超出训练域也能有效预测。评估了通用降阶模型的预测能力。

Conclusion: 该研究为星状器ETG湍流的降阶建模提供了一种有效的方法，有望加速等离子体物理研究和核聚变装置的设计优化。
...
### [2] [Uncertainties in Physics-informed Inverse Problems: The Hidden Risk in Scientific AI](https://arxiv.org/abs/2511.04564)
*Yoh-ichi Mototake, Makoto Sasaki*

Main category: physics.comp-ph

TL;DR: 本文研究了基于物理信息机器学习 (PIML) 中系数函数估计的不确定性问题，并提出了一种量化和分析框架，强调了在科学AI中选择物理上合理的解的重要性。


<details>
  <summary>Details</summary>
Motivation: 传统的机器学习模型通常基于预测性能进行评估，而物理学更注重解的物理意义。本文指出，仅依赖预测准确性可能导致错误的物理解释，并借鉴开普勒太阳中心模型的例子，强调了数据驱动模型推断中固有不确定性的存在。

Method: 本文提出了一种量化和分析PIML中系数函数估计不确定性的框架，并将其应用于磁流体力学模型的简化版本，利用几何约束进行验证。

Result: 研究结果表明，存在不确定性，但通过引入几何约束，可以对简化模型进行唯一识别。

Conclusion: 本文强调了在科学AI中，在数据驱动模型推断中考虑物理意义的重要性，并提供了一种评估和处理此类不确定性的方法，有助于避免因过度依赖预测性能而导致的错误结论。
...
### [3] [Uncertainty Quantification for Reduced-Order Surrogate Models Applied to Cloud Microphysics](https://arxiv.org/abs/2511.04534)
*Jonas E. Katona, Emily K. de Jong, Nipun Gunawardena*

Main category: cs.LG

TL;DR: 本文提出了一种适用于降阶模型（ROM）的、无需修改模型架构或训练过程的后验、模型无关的不确定性量化框架，并将其应用于云微物理学中的降阶模型。


<details>
  <summary>Details</summary>
Motivation: 现有降阶模型缺乏鲁棒的不确定性量化方法，且现有方法通常与特定架构或训练过程相关，限制了其通用性。云微物理学参数化中的不确定性是未来气候预测的主要来源之一。

Method: 该框架利用保形预测（Conformal Prediction，CP），一种分布自由的方法，对ROM的多个组件（潜在动力学、重构和端到端预测）进行统计预测区间的估计，无需修改底层架构或训练过程。

Result: 在云微物理学中的降阶模型上进行了验证，准确预测了液滴大小分布的演变，并量化了ROM管道中的不确定性。

Conclusion: 该方法提供了一种通用的、模型无关的不确定性量化方法，可以帮助实践者更可靠地评估ROM架构的各个组件，从而提高对降阶模型的信任度。
