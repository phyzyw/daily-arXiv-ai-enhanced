...
### [1] [Deep Learning of Solver-Aware Turbulence Closures from Nudged LES Dynamics](https://arxiv.org/abs/2604.23874)
*Ashwin Suriyanarayanan, Melissa Adrian, Dibyajyoti Chakraborty, Romit Maulik*

Main category: physics.flu-dyn

TL;DR: 本文提出了一种基于连续数据同化（nudging）的深度学习湍流闭合模型训练方法，该方法结合了a-prioritraining的效率和a-posterioritraining的准确性，避免了求解器反向传播的计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习湍流闭合模型存在训练不稳定、计算成本高、难以广泛应用以及泛化能力受限等问题。传统的a-posterioritraining方法需要修改求解器，且对数值方案敏感。

Method: 该研究采用连续数据同化（nudging）方法，将DNS数据视为稀疏观测，进行a-prioritraining，使深度学习模型学习所需的强制力，从而捕捉真实统计量。该方法无需反向传播，并能适应不同的数值和时间方案。

Result: 实验结果表明，该模型能够成功学习所需的强制力，在保持长期稳定性的同时，适应不同的数值和时间方案，并能分析数值离散误差对模型行为的影响，其预测结果优于传统闭合模型。

Conclusion: 该研究提供了一种高效且稳定的深度学习湍流闭合模型训练方法，有望提高LES模拟的准确性和泛化能力，为湍流模拟领域提供新的思路。
...
### [2] [Computational Design and Experimental Validation of Photoactive PARP1 Inhibitors](https://arxiv.org/abs/2604.24634)
...
TL;DR: 该研究利用计算方法和实验验证，成功设计并合成了光激活的PARP1抑制剂，实现了在绿光照射下对PARP1的抑制能力增强。


<details>
  <summary>Details</summary>
Motivation: 现有治疗方法副作用严重，光激活药物有望实现对局部疾病的精准治疗，但其开发面临光物理和生物学性质的复杂优化问题。

Method: 研究人员使用基于原子模拟和机器学习的方法，筛选了500万个潜在的光激活配体。该流程结合了蛋白质-配体对接、机器学习力场和量子化学计算预测pKa、吸收光谱和热半衰期、基于图的代理模型进行额外化合物筛选、机器学习非绝热动力学估计量子产率以及自由能扰动（FEP）优化结合预测。

Result: 合成的10个候选化合物中，1个化合物在绿光照射下对PARP1的抑制作用提高了15倍（208.8±28.3µM vs 14.4±1.9µM）。

Conclusion: 该研究验证了计算引导的筛选策略在识别红移PARP1光抑制剂方面的有效性，并指出了在水性介质中快速热弛豫等当前局限性。
...
### [3] [Enhancing molecular dynamics with equivariant machine-learned densities](https://arxiv.org/abs/2604.24563)
*Mihail Bogojeski, Muhammad R. Hasyim, Leslie Vogt-Maranto, Klaus-Robert Müller, Kieron Burke, Mark E. Tuckerman*

Main category: physics.chem-ph

TL;DR: 本文提出了一种名为DenSNet的新方法，利用机器学习直接预测电子密度，从而在分子动力学模拟中同时获得能量、力以及光谱和电子性质。


<details>
  <summary>Details</summary>
Motivation: 传统的机器学习原子间势（MLIP）主要关注能量和力，无法直接获得光谱等电子性质。为了解决这个问题，并提供一个统一的框架来预测各种性质，需要一种能够直接学习电子结构的机器学习方法。

Method: DenSNet采用SE(3)-等变神经网络预测原子中心高斯基组的密度系数，并结合∆-学习策略加速训练。然后，另一个等变网络将预测的密度映射到总能量，形成一个统一的框架。该方法基于Hohenberg-Kohn定理，将核构型映射到电子密度。

Result: 在乙醇、乙硫醇和间苯二酚等分子上验证了DenSNet，其机器学习轨迹中的红外光谱与实验数据高度吻合。在聚噻吩低聚物上进行了可扩展性测试，成功预测了长链（高达12个单体）的红外光谱，与DFT计算结果一致。

Conclusion: DenSNet的出现为大规模分子模拟中可转移地预测光谱和电子性质开辟了一条实用途径，重新强调了电子密度作为核心学习量的作用。
...
### [4] [SolarTformer: A Transformer Based Deep Learning Approach for Short Term Solar Power Forecasting](https://arxiv.org/abs/2604.24306)
*Ankan Basu, Jyotiraditya Roy, Aditya Datta, Prayas Sanyal, Sumanta Banerjee*

Main category: cs.LG

TL;DR: 本文提出了一种基于Transformer架构的深度学习模型SolarTformer，用于短期太阳能发电预测，显著优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 准确预测太阳能发电对于将可再生能源有效整合到电网至关重要。传统预测方法存在复杂、耗时、成本高昂以及难以应用于农村地区的局限性。

Method: SolarTformer模型利用自注意力机制捕捉太阳辐照度的时空依赖性，并结合电站特定元数据进行训练，以提高泛化能力。模型基于Transformer架构进行修改，并对时间序列数据进行循环编码。

Result: 实验结果表明，SolarTformer在相同数据集上显著优于现有模型，在晴天和阴天均表现出良好的性能，具有较高的鲁棒性和泛化能力。

Conclusion: SolarTformer的成功应用表明，基于注意力机制的架构可以有效提高太阳能发电预测的准确性，有助于更可靠地管理可再生能源。
...
### [5] [Predicting Wind Loads on Container Ships in Harbor Environments through Multi-Fidelity Modeling](https://arxiv.org/abs/2604.22882)
*Matilde Fiore, Andrea Bresciani, Miguel Alfonso Mendez, Jeroen van Beeck*

Main category: cs.LG

TL;DR: 本文提出了一种多保真度代理建模框架，用于预测集装箱船在港口环境中的风载荷，显著提高了预测精度并减少了高保真CFD模拟的依赖性。


<details>
  <summary>Details</summary>
Motivation: 现代集装箱船的风阻面积增大，导致风载荷增加，现有的经验模型准确性不足且未考虑周围结构的影响，因此需要更准确的风载荷预测方法。

Method: 该研究结合经验相关性、简化和详细的CFD模型，采用递归共克里金法融合不同保真度的信息，并通过灵敏度分析和序列采样构建训练数据库。

Result: 多保真度方法显著提高了预测精度，优于单保真度模型和传统经验相关性，并能捕捉到风载荷与关键几何参数之间的依赖关系。

Conclusion: 该框架提供了一种稳健高效的工具，可用于工程应用，为智能港口的自动化系泊系统提供准确的风载荷预测。
...
### [6] [Agentic AI platforms for autonomous training and rule induction of human-human and virus-human protein-protein interactions](https://arxiv.org/abs/2604.23924)
*Hung N. Do, Jessica Z. Kubicek-Sutherland, Oscar A. Negrete, S. Gnanakaran*

Main category: cs.AI

TL;DR: 本文提出了两种自主训练和规则诱导的智能体AI平台，分别用于预测人类-人类和病毒-人类蛋白-蛋白相互作用，并取得了高准确率和可解释性。


<details>
  <summary>Details</summary>
Motivation: 蛋白-蛋白相互作用(PPI)预测面临数据稀缺和缺乏负数据标签的挑战，现有机器学习模型也存在数据泄露问题。因此，需要更有效的方法来预测PPI并提供可解释性。

Method: 设计了两个智能体AI平台：一个用于自主训练预测机器学习模型，另一个用于诱导明确的通用规则。第一个平台由五个AI智能体组成，负责数据收集、验证、特征嵌入、模型设计和训练。第二个平台利用蛋白嵌入、物理化学自协方差描述符等信息诱导规则，并与机器学习模型的SHAP特征进行比对。

Result: 人类-人类PPI预测准确率达到87.3%，病毒-人类PPI预测准确率达到86.5%。诱导的规则与机器学习模型识别的关键特征一致。

Conclusion: 该研究展示了智能体AI在数据规划、执行、规则诱导和机器学习解释方面的能力，为PPI预测和相关应用开辟了新的可能性。
