...
### [1] [Fine-tuning DeepSeek-OCR-2 for Molecular Structure Recognition](https://arxiv.org/abs/2604.03476)
*Haocheng Tang, Xingyu Dang, Junmei Wang*

Main category: cs.CV

TL;DR: 本文将DeepSeek-OCR-2模型应用于分子结构识别任务，通过两阶段的逐步微调策略，实现了与现有图像到序列模型相当的准确率，但仍低于图像到图模型的性能。


<details>
  <summary>Details</summary>
Motivation: 传统OCSR模型存在局限性，而大型多模态语言模型在文档理解方面表现出潜力。本文旨在探索如何利用DeepSeek-OCR-2模型进行分子光学识别，并克服直接全参数微调的训练不稳定问题。

Method: 作者将分子光学识别任务定义为图像条件下的SMILES生成问题，并提出了一种两阶段的逐步监督微调策略：首先使用LoRA进行参数高效的微调，然后进行选择性的全参数微调，采用分步学习率。训练数据集结合了PubChem的合成渲染图像和USPTO-MOL的专利图像。

Result: 微调后的MolSeek-OCR模型在精确匹配方面表现出与最佳图像到序列模型相当的性能，但在严格的序列级别保真度方面仍存在挑战。强化学习的后训练方法未能显著提升模型性能。

Conclusion: 本文展示了DeepSeek-OCR-2模型在分子结构识别中的应用潜力，并强调了在VLM-based分子识别中保持序列级别保真度的重要性。研究结果为未来利用大型语言模型进行化学信息提取提供了新的思路。
...
### [2] [Hybrid Fourier Neural Operator for Surrogate Modeling of Laser Processing with a Quantum-Circuit Mixer](https://arxiv.org/abs/2604.04828)
*Mateusz Papierz, Asel Sagingalieva, Alix Benoit, Toni Ivas, Elia Iseli, Alexey Melnikov*

Main category: quant-ph

TL;DR: 本文提出了一种混合量子-经典傅里叶神经网络算子（HQ-LP-FNO），通过用紧凑的共享模式变分量子电路（VQC）混合器替换传统傅里叶神经网络算子中的部分密集谱块，以提高三维激光加工模拟的代理模型效率和精度。


<details>
  <summary>Details</summary>
Motivation: 传统傅里叶神经网络算子（FNO）在三维问题中参数量大，限制了其在实时控制和不确定性量化中的应用。研究动机在于寻找更紧凑的谱混合策略，以降低参数量并提高效率。

Method: HQ-LP-FNO将传统FNO中的部分密集谱块替换为共享模式的VQC混合器，并设计了参数匹配的经典瓶颈控制。该模型在三维高能激光加工模拟中进行评估，耦合了热传导、熔池对流、自由表面变形和相变等物理现象。

Result: 实验结果表明，HQ-LP-FNO相对于纯经典FNO基线，减少了15.6%的可训练参数，同时将相分数绝对误差降低了26%，相对温度MAE从2.89%降低到2.56%。VQC的分配比例对温度指标有显著影响，表明经典-量子划分存在最优解。

Conclusion: 研究表明，基于VQC的参数高效谱混合可以提高复杂多物理场问题的神经网络算子代理模型，并为基于物理的混合量子算子学习建立了一种可控的评估协议。该方法为实时控制和不确定性量化提供了新的可能性。
...
### [3] [From Paper to Program: A Multi-Stage LLM-Assisted Workflow for Accelerating Quantum Many-Body Algorithm Development](https://arxiv.org/abs/2604.04089)
*Yi Zhou*

Main category: physics.comp-ph

TL;DR: 本文提出了一种多阶段LLM辅助工作流程，通过生成中间LaTeX规范，显著提高了量子多体算法开发的效率和可靠性，将开发周期从数月缩短至24小时。


<details>
  <summary>Details</summary>
Motivation: 传统上，将量子多体理论转化为可扩展软件需要耗费大量时间，直接使用LLM进行“论文到程序”的转换容易出错，缺乏空间推理能力，导致代码幻觉和内存瓶颈。

Method: 该方法采用多阶段工作流程，模拟物理研究小组。首先，LLM-0从文献中提取信息；其次，LLM-1生成数学严谨的LaTeX规范（作为中间蓝图），约束LLM-2生成精确的、无矩阵的O(D3)操作；最后，LLM-2根据LaTeX规范生成代码。

Result: 使用该框架成功生成了DMRG引擎，准确捕捉了Spin-1/2 Heisenberg模型的临界纠缠标度以及Spin-1 AKLT模型的对称保护拓扑序。在16种不同基础模型组合的测试中，成功率达到100%。

Conclusion: 该框架提供了一种高度可重复的加速计算物理研究的范例，通过结构化的指导和中间规范，有效解决了LLM在复杂科学计算中的局限性，极大地缩短了算法开发周期。
...
### [4] [Non-Equilibrium Stochastic Dynamics as a Unified Framework for Insight and Repetitive Learning: A Kramers Escape Approach to Continual Learning](https://arxiv.org/abs/2604.04154)
*Gunn Kim*

Main category: cond-mat.stat-mech

TL;DR: 本文将持续学习中的稳定性-可塑性困境和洞察与重复学习之间的区别统一到一个非平衡统计物理框架下，利用Kramers逃逸速率模型，揭示了EWC惩罚项的物理机制以及洞察和重复学习的本质差异。


<details>
  <summary>Details</summary>
Motivation: 持续学习系统面临稳定性-可塑性困境，且现有方法缺乏物理基础解释；洞察和重复学习缺乏统一的理论描述。

Method: 将学习系统状态建模为在双阱能量景观上随时间依赖有效温度T(t)的Langevin动力学演化，通过Fokker-Planck方程和Kramers逃逸速率k= (ω 0ωb/2π)e−∆E/T分析系统行为，并分别对应EWC惩罚项和不同温度协议。

Result: 识别EWC惩罚项为能量势垒，其高度随任务数线性增长，导致过渡速率呈指数衰减；洞察事件对应于T(t)的瞬时峰值，驱动快速势垒穿越；重复学习则在适度升高但固定的温度下进行，通过持续的随机扩散实现过渡。

Conclusion: 本文建立了一个基于物理基础的持续学习可塑性框架，揭示了可塑性的失败机制，并为人工智能中自适应噪声调度提供了设计原则。
...
### [5] [Towards protein folding pathways by reconstructing protein residue networks with a policy-driven model](https://arxiv.org/abs/2604.04677)
*Susan Khor*

Main category: q-bio.BM

TL;DR: 该研究利用网络动力学(ND)方法，通过策略驱动的模型重建蛋白质残基网络，并发现重建的网络与蛋白质折叠速率之间存在高度相关性，提示了蛋白质折叠过程中起始点和环境条件的重要性。


<details>
  <summary>Details</summary>
Motivation: 现有研究在模拟蛋白质折叠方面存在局限性，尤其是在多状态折叠蛋白的模拟上。本研究旨在改进ND方法，使其能够更准确地预测蛋白质折叠速率，并探索蛋白质折叠的潜在途径。

Method: 研究人员改进了网络动力学(ND)方法，通过策略驱动的模型重建蛋白质残基网络。该方法使用节点选择和边恢复策略，并结合欧式距离搜索(EDS)算法识别短切网络(SCN)。通过调整策略和随机种子，优化ND方法，使其重建的PRN的SCN达到峰值能量，并与已发表的折叠速率进行比较。

Result: 重建的蛋白质残基网络与已发表的折叠速率之间存在高度相关性（Pearson相关系数 < -0.83），且在折叠家族层面也表现出强相关性。结果表明，合适的策略和随机种子对ND方法的成功至关重要。

Conclusion: 该研究结果表明，通过重建蛋白质残基网络，可以有效地预测蛋白质折叠速率，并可能揭示蛋白质折叠的潜在途径。研究结果强调了蛋白质折叠过程中起始点和环境条件的重要性，并为进一步研究蛋白质折叠机制提供了新的思路。
